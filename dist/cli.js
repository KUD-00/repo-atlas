...
function status(root, args) {
    ...
    const show = (label, st, max = 15) => {
        ...
        for (const e of list.slice(0, max))
            console.log(`  ${e.type === 'dir' ? 'D' : 'F'} ${e.path || '(root)'}${fmtDelta(e.delta)}`);
        if (list.length > max)
            console.log(`  … and ${list.length - max} more (use --json for the full list)`);
    };
    ...
    if (result.audits.length) {
        ...
        for (const a of result.audits) {
            if (a.status === 'fresh' && !a.findingsWithDrift)
                continue;
            if (a.invalidReason) {
    ...
    if (result.readability) {
        ...
        for (const file of readability.changedFiles.slice(0, 10))
            console.log(`  R ${file} — changed since readability scan`);
        for (const file of readability.missingFiles.slice(0, 10))
            console.log(`  R ${file} — gone since readability scan`);
        for (const file of readability.failedFiles.slice(0, 10))
            console.log(`  R ${file} — could not hash current bytes`);
        if (readability.changedFiles.length > 10)
            console.log(`  … and ${readability.changedFiles.length - 10} more changed (use --json for the full list)`);
    }
    ...
    if (result.brokenRefs.length) {
        ...
        if (result.brokenRefs.length > 20)
            console.log(`  … and ${result.brokenRefs.length - 20} more`);
    }
}
...
function stamp(root, args) {
    ...
    for (const p of targets) {
        ...
        if (note.hash === current.hash && note.anchor)
            continue;
...
function check(root, args) {
    ...
    if (args.includes('--json')) {
        const pick = (k) => result.findings.filter((f) => f.kind === k);
        console.log(JSON.stringify({
            summary,
...
function readability(root, args) {
    ...
    if (canonicalTarget) {
        ...
        assertReadabilityReportOutput(root, target);
    ...
    if (target !== null) {
        ...
        if (fs.existsSync(target)) {
            ...
            if (!isSupportedReadabilityReport(prev))
                throw new Error(`refusing to overwrite unsupported readability report: ${target}`);
            ...
            if (trend.changedFiles || trend.addedFiles.length || trend.removedFiles.length) {
                ...
                for (const w of trend.worsened.slice(0, 5))
                    log(`  ↓ ${w.path}  ${w.before.toFixed(1)} → ${w.after.toFixed(1)}`);
                for (const im of trend.improved.slice(0, 5))
                    log(`  ↑ ${im.path}  ${im.before.toFixed(1)} → ${im.after.toFixed(1)}`);
            }
        }
        if (canonicalTarget)
            writeCanonicalReadabilityReport(root, report);
        else
            writeReadabilityReport(root, target, report);
        if (canonicalTarget)
            writeReadabilityAuditLedger(root, report);
        log(`wrote ${target}`);
    }
    ...
    if (json)
        ...
        console.log(formatReadabilitySummary(report, top));
}
function quality(root, args) {
    const config = loadConfig(root) ?? {};
    const json = args.includes('--json');
    ...
    const ingest = [
        ...flagValues('--ingest').map((file) => ({ tool: 'generic', file })),
        ...flagValues('--ingest-eslint').map((file) => ({ tool: 'eslint', file })),
        ...flagValues('--ingest-knip').map((file) => ({ tool: 'knip', file })),
    ];
    ...
    if (json)
        ...
        console.log(formatQualitySummary(report, top));
