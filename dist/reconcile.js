...
function pickBest(from, candidates) {
    return [...candidates].sort((a, b) => suffixScore(from, b) - suffixScore(from, a) || (a < b ? -1 : 1))[0];
}
export function detectMoves(root, scanResult, entries, orphans, notes) {
    const missingFiles = new Set(entries.filter((e) => e.status === 'missing' && e.type === 'file').map((e) => e.path));
    const missingDirs = new Set(entries.filter((e) => e.status === 'missing' && e.type === 'dir').map((e) => e.path));
    ...
    if (orphanFiles.length && missingFiles.size) {
        ...
        for (const [anchor, fromPaths] of byAnchor) {
            ...
            for (const line of out.split('\n')) {
                ...
                if (!fromPaths.has(from) || claimedFrom.has(from))
                    continue;
                if (!missingFiles.has(to) || claimedTo.has(to))
                    continue;
    ...
    for (const [anchor, anchored] of byAnchor) {
        ...
        for (const e of anchored) {
            ...
            for (const [p, [a, r]] of stat) {
                if (e.type === 'file' ? p !== e.path : (e.path !== '' && p !== e.path && !p.startsWith(e.path + '/')))
                    continue;
                added += a;
                removed += r;
                files++;
