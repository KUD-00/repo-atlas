...
export function registerAuditUniqueBlobBytes(currentBytes, additionalBytes, maxBytes = AUDIT_V3_UNIQUE_BLOB_BYTE_LIMIT) {
    ...
    if (currentBytes > maxBytes || additionalBytes > maxBytes - currentBytes) {
        throw new Error(`unique exact-source blob byte limit of ${maxBytes} bytes exceeded`);
    }
...
function computeCodexFingerprint(targetId, ruleId, anchor, instance) {
...
export function computeAuditCanonicalDigest(value) {
...
function assertIdentityText(value, label) {
    if (typeof value !== 'string' ||
        ...
        throw new Error(`${label} must be bounded nonempty identity text without NUL or lone Unicode surrogates`);
    }
}
...
function assertGitBlob(value, label) {
    if (!GIT_BLOB_RE.test(value))
        throw new Error(`${label} must be a canonical Git blob digest`);
}
function assertSecuritySlug(value) {
    if (!SECURITY_SLUG_RE.test(value)) {
        throw new Error('audit slug must begin with security- and use lowercase kebab-case');
    }
}
function assertPattern(value, label) {
    if (typeof value !== 'string' ||
        value.length === 0 ||
        value.length > TEXT_LIMIT ||
        value.includes('\0') ||
...
function assertSemanticSelector(value, label) {
    if (typeof value !== 'string' ||
        value.length === 0 ||
        value.length > TEXT_LIMIT ||
        value.includes('\0') ||
...
export function computeAtlasFingerprint(input) {
    ...
    if (input.domain !== 'security')
        throw new Error('Atlas V3 fingerprint domain must be security');
    assertIdentityText(input.ruleId, 'ruleId');
    assertIdentityText(input.anchor, 'anchor');
    if (input.instance !== undefined)
        assertIdentityText(input.instance, 'instance');
...
export function computeAtlasObservationId(input) {
    ...
    return stableAuditId('aobs', 'atlas-observation/v1', [
        input.slug,
        input.adapter,
        input.runId,
...
export function computeAtlasOccurrenceId(observationId, fingerprint) {
    if (!OBSERVATION_ID_RE.test(observationId))
        throw new Error('invalid Atlas observationId');
    if (!FINGERPRINT_RE.test(fingerprint))
        throw new Error('invalid Atlas fingerprint');
    return stableAuditId('atocc', 'atlas-occurrence/v1', [observationId, fingerprint]);
}
export function computeExactScopeIdentityDigest(input) {
    ...
    for (const pattern of input.includePaths)
        assertPattern(pattern, 'include path');
    for (const pattern of input.excludePaths)
        assertPattern(pattern, 'exclude path');
...
export function computeSemanticScopeIdentityDigest(input) {
    if (![
        'repository',
        'scoped_path',
        'diff',
    ...
    }).sort((left, right) => utf16Compare(left.pattern, right.pattern) || utf16Compare(left.reason, right.reason));
...
export function computeAuditInventoryDigest(files) {
...
export function computeAuditHistoryEntryDigest(entry) {
...
function arrayAt(value, pointer) {
    if (!Array.isArray(value))
        invalid('invalid-type', pointer, 'must be an array');
    return value;
}
function exactKeys(value, required, optional, pointer) {
    ...
    for (const key of Object.keys(value)) {
        if (!allowed.has(key))
            invalid('unknown-member', `${pointer}/${key}`, 'unknown member');
    }
    for (const key of required) {
        if (!Object.hasOwn(value, key))
            invalid('missing-member', `${pointer}/${key}`, 'required member is missing');
    }
}
function stringAt(value, pointer, nonempty = true) {
    if (typeof value !== 'string' ||
...
function gitBlobAt(value, pointer) {
...
function uniqueStringsAt(value, pointer) {
    const rows = arrayAt(value, pointer).map((row, index) => stringAt(row, `${pointer}/${index}`));
...
function rawSha256At(value, pointer) {
...
function remoteAt(value, pointer) {
    const text = stringAt(value, pointer);
    if (/[\u0000-\u001f\u007f\\]/u.test(text)) {
    ...
    if (!text.includes('://') ||
        ...
        parsed.hostname === '' ||
        parsed.href !== text) {
        invalid('invalid-remote', pointer, 'remote must be a canonical absolute URL without credentials, query, or fragment');
    }
    return text;
}
function extensionJsonValueAt(value, pointer) {
    let members = 0;
    const seen = new Set();
    const visit = (candidate, candidatePointer, depth) => {
        ...
        if (typeof candidate === 'number') {
            if (!Number.isFinite(candidate) ||
                ...
                invalid('invalid-extension-value', candidatePointer, 'number must be finite and integer-valued numbers must be safe integers');
            }
            return candidate;
        }
        ...
        try {
            ...
            for (const key of Object.keys(descriptors).sort(utf16Compare)) {
                ...
                result[key] = visit(descriptor.value, `${candidatePointer}/${key.replace(/~/gu, '~0').replace(/\//gu, '~1')}`, depth + 1);
            }
            return result;
        }
        finally {
...
function configRepositoryId(root) {
...
function gitBlobBytes(root, blob, pointer, context) {
    ...
    try {
        context.uniqueBlobBytes = registerAuditUniqueBlobBytes(context.uniqueBlobBytes, loaded.byteLength);
    }
    catch (error) {
        invalid('unique-blob-byte-limit', pointer, error instanceof Error
            ? error.message
...
function snippetFromBlob(bytes, startLine, endLine, pointer) {
    let text;
    try {
    ...
    if (endLine > lines.length)
        invalid('invalid-line', pointer, 'line range exceeds the source blob');
    return lines.slice(startLine - 1, endLine).join('\n');
}
function parseProducer(value, pointer) {
    const producer = recordAt(value, pointer);
    const kind = enumAt(producer.kind, ['grok-cli', 'codex-security', 'migration', 'manual'], `${pointer}/kind`);
    const common = [
        'kind',
        'name',
        'version',
    ...
    if (kind === 'codex-security') {
        ...
        exactKeys(producer, [...common, 'ruleset'], ['prompt', 'effectiveConfigDigest', 'environmentPolicyDigest', 'transcriptDigest'], pointer);
    }
    ...
    if (kind === 'codex-security') {
        enumAt(producer.identityBasis, ['codex-contract'], `${pointer}/identityBasis`);
        ...
        enumAt(contract.namespace, ['codex-security/1.0'], `${pointer}/sourceContract/namespace`);
        enumAt(contract.status, ['completed'], `${pointer}/sourceContract/status`);
        ...
        if (sealedAt !== completedAt) {
            invalid('timestamp-mismatch', `${pointer}/sourceContract/sealedAt`, 'must be byte-for-byte equal to completedAt');
        }
        ...
        if (contract.coverageRef !== 'coverage.json') {
            invalid('invalid-path', `${pointer}/sourceContract/coverageRef`, 'must equal coverage.json for Codex Security 1.0');
        }
        if (contract.findingsRef !== 'findings.json') {
            invalid('invalid-path', `${pointer}/sourceContract/findingsRef`, 'must equal findings.json for Codex Security 1.0');
        }
        ...
        if (identityDigest !== expectedIdentity) {
            invalid('identity-mismatch', `${pointer}/identityDigest`, 'does not match the Codex contract identity');
        }
        return {
            kind,
            adapter,
    ...
    enumAt(producer.identityBasis, ['ruleset'], `${pointer}/identityBasis`);
    ...
    for (const key of ['effectiveConfigDigest', 'environmentPolicyDigest', 'transcriptDigest']) {
        if (producer[key] !== undefined)
            sha256At(producer[key], `${pointer}/${key}`);
    }
    if (producer.prompt !== undefined) {
        const prompt = recordAt(producer.prompt, `${pointer}/prompt`);
        exactKeys(prompt, ['builtinVersion', 'digest'], ['extraPath', 'extraDigest'], `${pointer}/prompt`);
...
function parseTarget(root, producerKind, value, pointer) {
    const target = recordAt(value, pointer);
    const kind = enumAt(target.kind, ['git-revision', 'git-worktree', 'git-diff', 'directory-snapshot'], `${pointer}/kind`);
    const common = [
        'kind',
        'repositoryId',
        'targetId',
    ...
    if (producerKind === 'codex-security') {
        ...
        exactKeys(target, [...common, 'displayName', 'sourceKind', ...requiredByKind[kind], ...(basis === 'snapshot'
                ? ['snapshotDigest', 'sourceSnapshotDigest']
                : [])], ['remote', ...optionalByKind[kind]], pointer);
    }
    else {
        const requiredByKind = {
        ...
        exactKeys(target, [...common, 'snapshotDigest', ...requiredByKind[kind]], [...metadata, ...(kind === 'git-worktree' ? ['revision'] : [])], pointer);
    }
    ...
    if (target.remote !== undefined)
        remoteAt(target.remote, `${pointer}/remote`);
    if (producerKind === 'codex-security') {
        const sourceKind = enumAt(target.sourceKind, ['git_revision', 'git_worktree', 'git_diff', 'directory_snapshot'], `${pointer}/sourceKind`);
        const expectedKind = {
        ...
        if (kind !== 'git-revision' && basis !== 'snapshot') {
            invalid('target-basis-mismatch', `${pointer}/identityBasis`, 'completed Codex worktree, diff, and directory targets require snapshot identity');
        }
        if (basis === 'snapshot') {
            const sourceSnapshot = stringAt(target.sourceSnapshotDigest, `${pointer}/sourceSnapshotDigest`);
            const sourceMatch = /^codex-security-snapshot\/v1:(sha256:[0-9a-f]{64})$/u.exec(sourceSnapshot);
            if (!sourceMatch) {
                invalid('invalid-digest', `${pointer}/sourceSnapshotDigest`, 'must be a Codex Security snapshot digest');
            }
            ...
            if (snapshotDigest !== sourceMatch[1] || identityDigest !== snapshotDigest) {
                invalid('identity-mismatch', `${pointer}/identityDigest`, 'snapshot identity must equal the normalized source snapshot digest');
            }
        }
        else {
            ...
            for (const key of ['sourceRevision', 'sourceBaseRevision', 'sourceHeadRevision']) {
                if (typeof target[key] === 'string')
                    identityMaterial[key] = target[key];
            }
            const expectedIdentity = computeAuditCanonicalDigest(identityMaterial);
            if (identityDigest !== expectedIdentity) {
                invalid('identity-mismatch', `${pointer}/identityDigest`, 'does not match the source revision-coordinate identity');
            }
        }
        return { repositoryId, targetId, identityDigest };
    }
    enumAt(target.identityBasis, ['snapshot'], `${pointer}/identityBasis`);
    const snapshotDigest = sha256At(target.snapshotDigest, `${pointer}/snapshotDigest`);
    ...
    if (kind === 'git-revision') {
        if (dirty)
            invalid('invalid-dirty-claim', `${pointer}/dirty`, 'git revision must be clean');
        ...
        if (dirty)
            invalid('invalid-dirty-claim', `${pointer}/dirty`, 'git diff must be clean');
...
function parseFileReceipt(root, value, pointer, context) {
    ...
    const outcome = enumAt(file.outcome, ['clean', 'findings', 'unknown'], `${pointer}/outcome`);
    const findingOccurrenceIds = uniqueStringsAt(file.findingOccurrenceIds, `${pointer}/findingOccurrenceIds`);
    ...
    if (hasReviewedAt) {
        ...
        const precision = enumAt(file.reviewedAtPrecision, ['timestamp', 'date'], `${pointer}/reviewedAtPrecision`);
    ...
    if (file.reviewedBy !== undefined)
        stringAt(file.reviewedBy, `${pointer}/reviewedBy`);
    if (file.ruleset !== undefined)
        stringAt(file.ruleset, `${pointer}/ruleset`);
    return {
        receipt: file,
        bytes,
    };
}
function parseExplicitExclusions(value, pointer) {
    ...
    const parsed = rows.map((row, index) => {
        ...
        try {
            ...
            invalid('invalid-path', `${itemPointer}/pattern`, error instanceof Error ? error.message : String(error));
        }
        return {
            pattern,
...
function parseSemanticCoverage(value, pointer) {
    ...
    exactKeys(coverage, [
        ...
        'explicitExclusions',
        'deferred',
    ], ['openQuestions'], pointer);
    const mode = enumAt(coverage.mode, ['repository', 'scoped_path', 'diff', 'commit', 'branch_diff', 'working_tree', 'deep_repository', 'unit', 'custom'], `${pointer}/mode`);
    const completeness = enumAt(coverage.completeness, ['complete', 'partial', 'unknown'], `${pointer}/completeness`);
    const inventoryStrategy = enumAt(coverage.inventoryStrategy, ['repository', 'scoped_path', 'diff', 'directory', 'custom', 'unit'], `${pointer}/inventoryStrategy`);
    const explicitExclusions = parseExplicitExclusions(coverage.explicitExclusions, `${pointer}/explicitExclusions`);
    ...
    for (const [index, row] of surfaces.entries()) {
        ...
        exactKeys(surface, ['id', 'label', 'disposition', 'receiptRefs'], ['riskArea', 'notes'], itemPointer);
        const id = stringAt(surface.id, `${itemPointer}/id`);
        if (surfaceIds.has(id))
            invalid('duplicate', `${itemPointer}/id`, 'duplicate semantic surface ID');
        surfaceIds.add(id);
        stringAt(surface.label, `${itemPointer}/label`);
        const disposition = enumAt(surface.disposition, ['reported', 'no_issue_found', 'rejected', 'not_applicable', 'needs_follow_up'], `${itemPointer}/disposition`);
        ...
        if (surface.riskArea !== undefined)
            stringAt(surface.riskArea, `${itemPointer}/riskArea`);
        if (surface.notes !== undefined)
            stringAt(surface.notes, `${itemPointer}/notes`);
    }
    ...
    for (const [index, row] of deferred.entries()) {
        ...
        if (deferredIds.has(id))
            invalid('duplicate', `${itemPointer}/id`, 'duplicate deferred ID');
        deferredIds.add(id);
        ...
        if (item.surfaceIds !== undefined) {
            ...
            for (const [surfaceIndex, surfaceId] of ids.entries()) {
                if (!surfaceIds.has(surfaceId)) {
                    invalid('unknown-reference', `${itemPointer}/surfaceIds/${surfaceIndex}`, 'must reference a semantic surface');
                }
            }
        }
    }
    ...
    if (completeness === 'complete' && (deferred.length !== 0 || needsFollowUp)) {
        invalid('semantic-coverage-closure', pointer, 'complete semantic coverage forbids deferred work and needs_follow_up surfaces');
    }
    return { mode, completeness, inventoryStrategy, explicitExclusions, findingRefs };
}
function parseThreatModel(value, pointer) {
    const model = recordAt(value, pointer);
    exactKeys(model, ['summary'], [
        'assets',
        'trustBoundaries',
...
function parseSourceArtifacts(value, pointer) {
    const rows = arrayAt(value, pointer);
    ...
    for (const [index, row] of rows.entries()) {
        ...
        const integrityKind = enumAt(artifact.integrityKind, ['producer-manifest', 'adapter-bundle'], `${itemPointer}/integrityKind`);
        exactKeys(artifact, [
            'path',
            'sha256',
            'mediaType',
        ...
        const retainedInAtlas = booleanAt(artifact.retainedInAtlas, `${itemPointer}/retainedInAtlas`);
...
function resolveObservationPointer(observation, jsonPointer) {
...
function validateCodexArtifactJoins(observation, artifacts) {
    const required = [
        {
    ...
    for (const [surfaceIndex, candidate] of coverage.surfaces.entries()) {
        for (const [refIndex, value] of candidate.receiptRefs.entries()) {
            if (!value.startsWith('artifacts/')) {
                invalid('invalid-reference', `/semanticCoverage/surfaces/${surfaceIndex}/receiptRefs/${refIndex}`, 'Codex coverage receipt references must be artifact paths');
            }
            required.push({
                path: value,
    ...
    for (const [findingIndex, candidate] of observation.findings.entries()) {
        const codeEvidence = candidate.codeEvidence;
        for (const [evidenceIndex, evidence] of (codeEvidence ?? []).entries()) {
            if (evidence.evidenceBasis !== 'sealed-producer-snippet')
                continue;
            const sourceSeal = evidence.sourceSeal;
            required.push({
                path: sourceSeal.artifactPath,
                pointer: `/findings/${findingIndex}/codeEvidence/${evidenceIndex}/sourceSeal/artifactPath`,
                integrityKind: 'producer-manifest',
            });
        }
        const artifactRefs = candidate.artifactRefs;
        for (const [artifactIndex, artifactRef] of (artifactRefs ?? []).entries()) {
            required.push({
                path: artifactRef.sourceArtifactPath,
                pointer: `/findings/${findingIndex}/artifactRefs/${artifactIndex}/sourceArtifactPath`,
                integrityKind: artifactRef.integrityKind,
            });
        }
        const validation = candidate.validation;
        if (validation?.artifactRefs !== undefined) {
            for (const [artifactIndex, artifactPath] of validation.artifactRefs.entries()) {
                required.push({
                    path: artifactPath,
                    pointer: `/findings/${findingIndex}/validation/artifactRefs/${artifactIndex}`,
                });
            }
        }
    }
    const forwardReferences = new Set(required.map((reference) => `${reference.path}\0${reference.pointer}`));
    for (const artifact of artifacts.values()) {
        if (artifact.integrityKind === 'producer-manifest' &&
            artifact.integrityIndex !== 'scan-manifest.json') {
            invalid('artifact-integrity-mismatch', '/sourceArtifacts', `${artifact.path} must be indexed by scan-manifest.json`);
        }
        for (const backlink of artifact.referencedBy) {
            if (!forwardReferences.has(`${artifact.path}\0${backlink}`)) {
                invalid('artifact-backlink-mismatch', '/sourceArtifacts', `${backlink} is not a recognized forward artifact reference`);
            }
            if (resolveObservationPointer(observation, backlink) !== artifact.path) {
                invalid('artifact-backlink-mismatch', '/sourceArtifacts', `${backlink} must resolve to the exact artifact path ${artifact.path}`);
            }
        }
    }
    for (const reference of required) {
        ...
        if (!artifact ||
            ...
            invalid('artifact-reference-mismatch', reference.pointer, `must resolve to a ${reference.integrityKind ?? 'source'} artifact`);
        }
        if (!artifact.referencedBy.includes(reference.pointer)) {
            invalid('artifact-backlink-mismatch', reference.pointer, 'source artifact must contain the exact leaf backlink');
        }
    }
}
function parseExtensions(value, pointer, codexSource, keys = new Set()) {
    const rows = arrayAt(value, pointer);
    for (const [index, row] of rows.entries()) {
        const itemPointer = `${pointer}/${index}`;
        ...
        if (codexSource &&
            ...
            invalid('invalid-extension-namespace', `${itemPointer}/namespace`, 'Codex source extensions require a document-specific namespace');
        }
        const jsonPointer = strictJsonPointerAt(extension.path, `${itemPointer}/path`);
        const key = `${namespace}\0${jsonPointer}`;
        if (keys.has(key))
            invalid('duplicate', itemPointer, 'duplicate extension namespace/path');
        keys.add(key);
...
function parseValidation(value, pointer, evidenceIds, artifacts) {
    ...
    for (const key of ['method', 'summary', 'confidenceRationale']) {
        if (validation[key] !== undefined)
            stringAt(validation[key], `${pointer}/${key}`);
    }
    if (validation.disposition !== undefined) {
        enumAt(validation.disposition, ['reportable', 'suppressed', 'not_applicable', 'deferred'], `${pointer}/disposition`);
    }
    if (validation.confidence !== undefined) {
        enumAt(validation.confidence, ['high', 'medium', 'low'], `${pointer}/confidence`);
    }
...
function parseAttackPath(value, pointer, evidenceIds) {
    if (value === null)
        return;
    ...
    if (attack.summary !== undefined)
        stringAt(attack.summary, `${pointer}/summary`);
    if (attack.dataflow !== undefined) {
        const dataflow = recordAt(attack.dataflow, `${pointer}/dataflow`);
        exactKeys(dataflow, [], ['summary', 'source', 'transformations', 'sink', 'outcome', 'evidenceRefs'], `${pointer}/dataflow`);
        ...
        if (dataflow.transformations !== undefined) {
            parseDocumentedJsonArray(dataflow.transformations, `${pointer}/dataflow/transformations`);
        }
        if (dataflow.evidenceRefs !== undefined) {
            const refs = uniqueStringsAt(dataflow.evidenceRefs, `${pointer}/dataflow/evidenceRefs`);
            for (const [index, ref] of refs.entries()) {
                if (!evidenceIds.has(ref)) {
                    invalid('unknown-reference', `${pointer}/dataflow/evidenceRefs/${index}`, 'must reference code evidence');
                }
            }
        }
    }
    ...
    for (const [key, levels] of [
        ...
        if (rating.why !== undefined)
            stringAt(rating.why, `${pointer}/${key}/why`);
    }
...
function parseFinding(root, findingIndex, observationId, repositoryId, targetId, producerKind, producerRunId, exactInventory, value, pointer, fileByPath, sourceArtifacts, extensionKeys) {
    ...
    if (!FINDING_ID_RE.test(findingId))
        invalid('invalid-finding-id', `${pointer}/findingId`, 'must be an Atlas finding ID');
    ...
    if (producerKind === 'codex-security' &&
        ...
        invalid('invalid-codex-slug', `${pointer}/identity`, 'Codex ruleId, anchor, and optional instance must be lowercase source slugs');
    }
    ...
    if (fingerprints.length === 0)
        invalid('missing-fingerprint', `${pointer}/fingerprints`, 'must contain the canonical Atlas fingerprint');
    let canonicalFingerprintCount = 0;
    let codexFingerprintCount = 0;
    const fingerprintKeys = new Set();
    const expectedCodexFingerprint = computeCodexFingerprint(targetId, ruleId, anchor, instance);
    for (const [index, candidate] of fingerprints.entries()) {
        ...
        const role = enumAt(fingerprint.role, ['canonical', 'producer'], `${pointer}/fingerprints/${index}/role`);
        const key = `${scheme}\0${fingerprintValue}`;
        if (fingerprintKeys.has(key))
            invalid('duplicate', `${pointer}/fingerprints/${index}`, 'duplicate fingerprint');
        ...
        if (scheme === 'codex-security/v1') {
            ...
            if (producerKind !== 'codex-security' ||
                ...
                invalid('fingerprint-mismatch', `${pointer}/fingerprints/${index}`, 'does not match the Codex Security producer fingerprint formula');
            }
        }
    }
    ...
    if ((producerKind === 'codex-security' && codexFingerprintCount !== 1) ||
        (producerKind !== 'codex-security' && codexFingerprintCount !== 0)) {
        invalid('producer-fingerprint-count', `${pointer}/fingerprints`, 'Codex observations require exactly one Codex producer fingerprint and other producers forbid it');
    }
    ...
    exactKeys(severity, ['level'], ['score', 'scoringSystem', 'vector', 'rationale', 'changeConditions'], `${pointer}/severity`);
    ...
    for (const key of ['scoringSystem', 'vector', 'rationale', 'changeConditions']) {
        if (severity[key] !== undefined)
            stringAt(severity[key], `${pointer}/severity/${key}`);
    }
    if (finding.confidence === undefined) {
        if (producerKind === 'grok-cli' || producerKind === 'codex-security') {
            invalid('missing-member', `${pointer}/confidence`, 'first-party Grok and Codex findings require confidence');
        }
    }
    else {
        const confidence = recordAt(finding.confidence, `${pointer}/confidence`);
        exactKeys(confidence, ['level'], ['rationale'], `${pointer}/confidence`);
        enumAt(confidence.level, ['low', 'medium', 'high'], `${pointer}/confidence/level`);
    ...
    if (taxonomy.cwe !== undefined)
        uniqueStringsAt(taxonomy.cwe, `${pointer}/taxonomy/cwe`);
    ...
    for (const [index, candidate] of locations.entries()) {
        ...
        if (endLine < startLine)
            invalid('invalid-line', `${pointer}/locations/${index}`, 'endLine must not precede startLine');
        if (location.role !== undefined)
            stringAt(location.role, `${pointer}/locations/${index}/role`);
        if (exactInventory) {
            const scoped = fileByPath.get(locationPath);
            if (!scoped) {
                invalid('location-receipt-mismatch', `${pointer}/locations/${index}/path`, 'exact authoritative locations must reference a scoped file receipt');
            }
            if (endLine > scoped.receipt.lines) {
                invalid('invalid-line', `${pointer}/locations/${index}`, 'location range exceeds the exact file receipt line count');
            }
    ...
    if (finding.codeEvidence !== undefined) {
        ...
        if (evidenceRows.length > 32)
            invalid('snippet-limit', `${pointer}/codeEvidence`, 'exceeds the 32 snippet limit');
        let totalBytes = 0;
        for (const [index, candidate] of evidenceRows.entries()) {
            ...
            const evidenceBasis = enumAt(evidence.evidenceBasis, ['exact-blob', 'sealed-producer-snippet'], `${evidencePointer}/evidenceBasis`);
            exactKeys(evidence, [
                'evidenceBasis',
                'id',
                'label',
            ...
            ], ['endLine', 'language', 'role'], evidencePointer);
            const id = stringAt(evidence.id, `${evidencePointer}/id`);
            if (evidenceIds.has(id))
                invalid('duplicate', `${evidencePointer}/id`, 'duplicate code-evidence ID');
            evidenceIds.add(id);
            ...
            if (endLine < startLine)
                invalid('invalid-line', evidencePointer, 'endLine must not precede startLine');
            const code = stringAt(evidence.code, `${evidencePointer}/code`, producerKind === 'codex-security');
            const snippetBytes = Buffer.byteLength(code, 'utf8');
            totalBytes += snippetBytes;
            if (snippetBytes > 16 * 1024)
                invalid('snippet-limit', `${evidencePointer}/code`, 'exceeds the 16 KiB snippet limit');
            if (totalBytes > 128 * 1024)
                invalid('snippet-limit', `${pointer}/codeEvidence`, 'exceeds the 128 KiB aggregate snippet limit');
            stringAt(evidence.explanation, `${evidencePointer}/explanation`);
            if (evidence.language !== undefined)
                stringAt(evidence.language, `${evidencePointer}/language`);
            if (evidence.role !== undefined)
                stringAt(evidence.role, `${evidencePointer}/role`);
            if (evidenceBasis === 'exact-blob') {
                ...
                exactKeys(sourceSeal, ['artifactPath', 'artifactSha256', 'jsonPointer'], [], `${evidencePointer}/sourceSeal`);
                const artifactPath = repoPathAt(sourceSeal.artifactPath, `${evidencePointer}/sourceSeal/artifactPath`);
                const artifactSha256 = rawSha256At(sourceSeal.artifactSha256, `${evidencePointer}/sourceSeal/artifactSha256`);
                const sourceJsonPointer = strictJsonPointerAt(sourceSeal.jsonPointer, `${evidencePointer}/sourceSeal/jsonPointer`);
                if (producerKind === 'codex-security' &&
                    artifactPath !== 'findings.json') {
                    invalid('source-seal-artifact-mismatch', `${evidencePointer}/sourceSeal/artifactPath`, 'Codex source seals must resolve through findings.json');
                }
                if (producerKind === 'codex-security' &&
                    sourceJsonPointer !==
                        `/findings/${findingIndex}/codeEvidence/${index}`) {
                    invalid('source-seal-pointer-mismatch', `${evidencePointer}/sourceSeal/jsonPointer`, 'Codex source seals must identify the matching findings.json code-evidence slot');
                }
                const artifact = sourceArtifacts.get(artifactPath);
                if (!artifact ||
                    artifact.integrityKind !== 'producer-manifest' ||
                    artifact.sha256 !== artifactSha256) {
                    invalid('source-seal-mismatch', `${evidencePointer}/sourceSeal`, 'must resolve to exactly one producer-manifest source artifact');
                }
            }
        }
    }
    if (finding.rootCause !== undefined) {
        if (typeof finding.rootCause === 'string') {
            ...
            if (rootCause.evidenceRefs !== undefined) {
                ...
                for (const [index, ref] of refs.entries()) {
                    if (!evidenceIds.has(ref)) {
                        invalid('unknown-reference', `${pointer}/rootCause/evidenceRefs/${index}`, 'must reference code evidence');
                    }
                }
            }
            if (rootCause.legacyCode !== undefined) {
                ...
                stringAt(legacyCode.code, `${pointer}/rootCause/legacyCode/code`, producerKind === 'codex-security');
    ...
    if (finding.validation !== undefined) {
        parseValidation(finding.validation, `${pointer}/validation`, evidenceIds, sourceArtifacts);
    }
    ...
    exactKeys(provenance, ['source'], ['producerSource', 'sourceFindingId', 'sourceOccurrenceId', 'candidateId', 'ledgerRowId', 'reportId'], `${pointer}/provenance`);
    ...
    if (producerKind === 'codex-security') {
        if (provenance.source !== 'codex-security') {
            invalid('provenance-source-mismatch', `${pointer}/provenance/source`, 'Codex observations require normalized source codex-security');
        }
        const producerSource = stringAt(provenance.producerSource, `${pointer}/provenance/producerSource`);
        void producerSource;
        const expectedSourceFindingId = `csf_${sha256Text(expectedCodexFingerprint).slice(0, 24)}`;
        const expectedSourceOccurrenceId = `occ_${sha256Text(`${producerRunId}\0${expectedCodexFingerprint}`).slice(0, 24)}`;
        if (provenance.sourceFindingId !== expectedSourceFindingId) {
            invalid('source-finding-identity-mismatch', `${pointer}/provenance/sourceFindingId`, 'does not match the Codex finding identity formula');
        }
        if (provenance.sourceOccurrenceId !== expectedSourceOccurrenceId) {
            invalid('source-occurrence-identity-mismatch', `${pointer}/provenance/sourceOccurrenceId`, 'does not match the Codex occurrence identity formula');
        }
    }
    else if (provenance.source === 'codex-security') {
        invalid('provenance-source-mismatch', `${pointer}/provenance/source`, 'non-Codex producer cannot claim Codex normalized provenance');
    }
    if (finding.artifactRefs !== undefined) {
        ...
        for (const [index, row] of artifactRefs.entries()) {
            ...
            enumAt(artifactRef.kind, ['external'], `${itemPointer}/kind`);
            const artifactPath = repoPathAt(artifactRef.sourceArtifactPath, `${itemPointer}/sourceArtifactPath`);
            const integrityKind = enumAt(artifactRef.integrityKind, ['producer-manifest', 'adapter-bundle'], `${itemPointer}/integrityKind`);
            ...
            const retainedInAtlas = booleanAt(artifactRef.retainedInAtlas, `${itemPointer}/retainedInAtlas`);
    ...
    if (finding.extensions !== undefined) {
        parseExtensions(finding.extensions, `${pointer}/extensions`, producerKind === 'codex-security', extensionKeys);
    }
    return finding;
}
function parseObservation(root, slug, value, pointer, context) {
    ...
    enumAt(observation.reviewState, ['complete'], `${pointer}/reviewState`);
    ...
    if (producer.completedAt !== undefined && observedAt !== producer.completedAt) {
        invalid('timestamp-mismatch', `${pointer}/observedAt`, 'must normalize the Codex source completedAt timestamp');
    }
    const target = parseTarget(root, producer.kind, observation.target, `${pointer}/target`);
    const scope = recordAt(observation.scope, `${pointer}/scope`);
    const identityBasis = enumAt(scope.identityBasis, ['exact-inventory', 'semantic-declaration'], `${pointer}/scope/identityBasis`);
    exactKeys(scope, [
        'mode',
        'identityDigest',
        'identityBasis',
    ...
    for (const key of ['summary', 'runtimeStatus', 'validationMode', 'context']) {
        if (scope[key] !== undefined)
            stringAt(scope[key], `${pointer}/scope/${key}`);
    }
    ...
    if (identityBasis === 'exact-inventory') {
        const mode = enumAt(scope.mode, ['repository', 'scoped_path', 'unit', 'diff', 'custom'], `${pointer}/scope/mode`);
        scopeMode = mode;
        ...
        for (const [index, row] of fileRows.entries()) {
            ...
            if (producer.rulesetId !== undefined) {
                if (parsed.receipt.status === 'reviewed' &&
                    ...
                    invalid('ruleset-receipt-mismatch', `${pointer}/scope/files/${index}/ruleset`, 'reviewed file receipt requires the producer ruleset ID');
                }
                if (parsed.receipt.ruleset !== undefined &&
                    parsed.receipt.ruleset !== producer.rulesetId) {
                    invalid('ruleset-receipt-mismatch', `${pointer}/scope/files/${index}/ruleset`, 'file receipt ruleset must equal producer.ruleset.id');
                }
            }
            if (previousPath !== undefined &&
                utf16Compare(previousPath, parsed.receipt.path) >= 0) {
                invalid('invalid-order', `${pointer}/scope/files/${index}/path`, 'files must be uniquely sorted by path');
            }
        ...
        const expectedInventory = computeAuditInventoryDigest([...fileByPath.values()].map(({ receipt }) => receipt));
        ...
        const mode = enumAt(scope.mode, ['repository', 'scoped_path', 'diff', 'commit', 'branch_diff', 'working_tree', 'deep_repository', 'unit', 'custom'], `${pointer}/scope/mode`);
        scopeMode = mode;
        const inventoryStrategy = enumAt(scope.inventoryStrategy, ['repository', 'scoped_path', 'diff', 'directory', 'custom', 'unit'], `${pointer}/scope/inventoryStrategy`);
        const explicitExclusions = parseExplicitExclusions(scope.explicitExclusions, `${pointer}/scope/explicitExclusions`);
        ...
        if (identityDigest !== expectedIdentity) {
            invalid('identity-mismatch', `${pointer}/scope/identityDigest`, 'does not match the semantic declaration identity');
        }
    }
    const exactCoverage = recordAt(observation.exactCoverage, `${pointer}/exactCoverage`);
    const exactBasis = enumAt(exactCoverage.basis, ['full-read-receipts', 'unavailable'], `${pointer}/exactCoverage/basis`);
    if (exactBasis === 'unavailable') {
        exactKeys(exactCoverage, ['completeness', 'basis', 'reason'], [], `${pointer}/exactCoverage`);
        enumAt(exactCoverage.completeness, ['unknown'], `${pointer}/exactCoverage/completeness`);
        ...
        if (producer.kind === 'codex-security' &&
            ...
            invalid('invalid-reason', `${pointer}/exactCoverage/reason`, 'semantic Codex coverage requires the stable unavailable reason');
        }
    }
    else {
        exactKeys(exactCoverage, ['completeness', 'basis', 'reviewedFileCount', 'unreviewed'], [], `${pointer}/exactCoverage`);
        if (identityBasis !== 'exact-inventory') {
            invalid('coverage-basis-mismatch', `${pointer}/exactCoverage/basis`, 'full-read receipts require an exact-inventory scope');
        }
        const completeness = enumAt(exactCoverage.completeness, ['complete', 'partial'], `${pointer}/exactCoverage/completeness`);
        const reviewedFileCount = safeIntegerAt(exactCoverage.reviewedFileCount, `${pointer}/exactCoverage/reviewedFileCount`);
        const unreviewed = arrayAt(exactCoverage.unreviewed, `${pointer}/exactCoverage/unreviewed`);
        ...
        for (const [index, row] of unreviewed.entries()) {
            ...
            if (!fileByPath.has(unreviewedPath) ||
                fileByPath.get(unreviewedPath)?.receipt.status !== 'not-reviewed') {
                invalid('invalid-reference', `${itemPointer}/path`, 'must reference a not-reviewed scoped file');
            }
        ...
        if (reviewedFileCount !== receiptReviewedCount ||
    ...
    const semanticCoverage = parseSemanticCoverage(observation.semanticCoverage, `${pointer}/semanticCoverage`);
    if (semanticCoverage.mode !== scopeMode ||
        ...
        invalid('scope-coverage-mismatch', `${pointer}/semanticCoverage`, 'must match the declared scope mode, strategy, and exclusions');
    }
    ...
    for (const [index, ref] of evidenceRefs.entries())
        repoPathAt(ref, `${pointer}/evidenceRefs/${index}`);
    ...
    if (identityBasis === 'exact-inventory') {
        for (const occurrenceId of occurrenceIds) {
            const referenced = [...fileByPath.values()].some(({ receipt }) => receipt.findingOccurrenceIds.includes(occurrenceId));
            if (!referenced) {
                invalid('unknown-reference', `${pointer}/findings`, `occurrence ${occurrenceId} is not bound to a scoped file`);
            }
        }
    }
    for (const [index, ref] of semanticCoverage.findingRefs.entries()) {
        ...
        if (!FINDING_ID_RE.test(findingId) || !findingIds.has(findingId)) {
            invalid('unknown-reference', `${pointer}/semanticCoverage/surfaces`, `unknown finding receipt ${index}`);
        }
    }
...
function parseAuditCurrentLedgerWithContext(root, repoPath, value, context) {
    try {
        ...
        if (Buffer.byteLength(canonicalInput, 'utf8') > LEDGER_BYTE_LIMIT) {
            invalid('ledger-byte-limit', '', `canonical ledger exceeds the ${LEDGER_BYTE_LIMIT}-byte limit`);
        }
        const wrapper = recordAt(JSON.parse(canonicalInput), '');
        exactKeys(wrapper, ['formatVersion', 'format', 'domain', 'slug', 'title', 'current', 'currentDigest', 'history'], ['conceptSlug'], '');
        if (wrapper.formatVersion !== 3)
            invalid('invalid-version', '/formatVersion', 'must equal 3');
        if (wrapper.format !== 'atlas-audit-v3')
            invalid('invalid-format', '/format', 'must equal atlas-audit-v3');
        if (wrapper.domain !== 'security')
            invalid('invalid-domain', '/domain', 'V3 currently supports security only');
        const slug = stringAt(wrapper.slug, '/slug');
        if (!SECURITY_SLUG_RE.test(slug))
            invalid('invalid-slug', '/slug', 'must begin with security- and use lowercase kebab-case');
        const expectedPath = `.atlas/audits/${slug}.json`;
        if (normalizedPath !== expectedPath)
            invalid('filename-mismatch', '/slug', `ledger path must be ${expectedPath}`);
...
export function parseAuditCurrentLedger(root, repoPath, value) {
    return parseAuditCurrentLedgerWithContext(root, repoPath, value, createAuditValidationContext());
}
function parseAuditObservationHistoryWithContext(root, repoPath, value, context) {
    try {
        const normalizedPath = normalizeAuditRepoPath(repoPath);
        const canonicalInput = canonicalJson(value);
        const history = recordAt(JSON.parse(canonicalInput), '');
        exactKeys(history, ['formatVersion', 'format', 'domain', 'slug', 'entries'], [], '');
        if (history.formatVersion !== 1) {
            invalid('invalid-version', '/formatVersion', 'must equal 1');
        }
        if (history.format !== 'atlas-audit-history-v1') {
            invalid('invalid-format', '/format', 'must equal atlas-audit-history-v1');
        }
        ...
        for (const [index, row] of entries.entries()) {
            ...
            if (observationIds.has(observationId)) {
                invalid('duplicate', `${pointer}/observationId`, 'history observation IDs must be unique');
            }
            observationIds.add(observationId);
            const observationDigest = sha256At(entry.observationDigest, `${pointer}/observationDigest`);
            ...
            if (observationDigest !== computeAuditCanonicalDigest(observation)) {
                invalid('history-observation-digest-mismatch', `${pointer}/observationDigest`, 'does not seal the embedded observation');
            }
            if (entry.previousEntryDigest !== previousEntryDigest) {
                invalid('history-chain-mismatch', `${pointer}/previousEntryDigest`, index === 0
            ...
            if (entryDigest !== expectedEntryDigest) {
                invalid('history-entry-digest-mismatch', `${pointer}/entryDigest`, 'does not seal the history entry');
            }
            for (const [findingIndex, finding] of observation.findings.entries()) {
                if (occurrenceIds.has(finding.occurrenceId)) {
                    invalid('duplicate', `${pointer}/observation/findings/${findingIndex}/occurrenceId`, 'history occurrence IDs must be unique');
                }
                occurrenceIds.add(finding.occurrenceId);
            }
...
export function parseAuditObservationHistory(root, repoPath, value) {
    return parseAuditObservationHistoryWithContext(root, repoPath, value, createAuditValidationContext());
}
function safeAuditDirectoryPaths(root, repoDirectory) {
    return listBoundedAuditDirectory(root, repoDirectory)
        .map((entry) => `${repoDirectory}/${entry}`);
}
function readOptionalAuditJson(root, repoPath, maxBytes) {
    return readOptionalAuditJsonDocument(root, repoPath, maxBytes)?.value ?? null;
}
function readOptionalAuditJsonDocument(root, repoPath, maxBytes) {
...
function resultError(result, subject) {
    return new Error(`${subject}: ${result.diagnostics.map((entry) => `${entry.path || '/'} ${entry.message}`).join('; ')}`);
}
function stateDiagnostic(code, path, message) {
    return { code, path, message };
}
function registerHistoryIdentityClaims(value, repoPath, context) {
    const diagnostics = [];
    ...
    if (!Array.isArray(entries))
        return diagnostics;
    const register = (id, pointer, pattern, owners, code, label) => {
        if (typeof id !== 'string' || !pattern.test(id))
            return;
        const owner = owners.get(id);
        if (owner !== undefined && owner !== repoPath) {
            diagnostics.push(stateDiagnostic(code, `${repoPath}${pointer}`, `${label} ${id} collides across history ledgers ${owner} and ${repoPath}`));
            return;
        }
    ...
    for (const [entryIndex, candidate] of entries.entries()) {
        ...
        register(entry.observationId, `/entries/${entryIndex}/observationId`, OBSERVATION_ID_RE, context.historyObservationIds, 'audit-history-observation-id-collision', 'observation ID');
        ...
        for (const [findingIndex, findingCandidate] of findings.entries()) {
            if (findingCandidate === null ||
                typeof findingCandidate !== 'object' ||
                Array.isArray(findingCandidate)) {
                continue;
            }
            register(findingCandidate.occurrenceId, `/entries/${entryIndex}/observation/findings/${findingIndex}/occurrenceId`, OCCURRENCE_ID_RE, context.historyOccurrenceIds, 'audit-history-occurrence-id-collision', 'occurrence ID');
        }
    }
    return diagnostics;
}
function currentHistoryIndex(ledger, history) {
    if (ledger.slug !== history.slug ||
    ...
    const index = history.entries.findIndex((entry) => entry.entryDigest === ledger.history.entryDigest);
    if (index < 0) {
        throw new Error('current ledger history reference is missing from the history chain');
    }
    const entry = history.entries[index];
    if (entry.observationId !== ledger.current.observationId ||
...
function loadAuditObservationHistoryWithContext(root, context) {
    const histories = [];
    const diagnostics = [];
    let paths;
    try {
        ...
        return {
            histories,
            diagnostics: [stateDiagnostic('audit-history-directory-invalid', '.atlas/audit-history', error instanceof Error ? error.message : String(error))],
        };
    }
    for (const repoPath of paths) {
        if (!repoPath.endsWith('.json')) {
            diagnostics.push(stateDiagnostic('audit-history-unexpected-entry', repoPath, 'audit history directory entries must be JSON files'));
            continue;
        }
        try {
            const raw = readBoundedAuditJson(root, repoPath);
            const identityDiagnostics = registerHistoryIdentityClaims(raw, repoPath, context);
            diagnostics.push(...identityDiagnostics);
            const parsed = parseAuditObservationHistoryWithContext(root, repoPath, raw, context);
            ...
            diagnostics.push(stateDiagnostic('audit-history-read-failed', repoPath, error instanceof Error ? error.message : String(error)));
        }
    }
    histories.sort((left, right) => utf16Compare(left.slug, right.slug));
    return { histories, diagnostics };
}
export function loadAuditObservationHistory(root) {
    return loadAuditObservationHistoryWithContext(root, createAuditValidationContext());
}
export function loadAuditObservations(root) {
    const observations = [];
    const historyAhead = [];
    ...
    const historyBySlug = new Map(historyResult.histories.map((history) => [history.slug, history]));
    ...
    try {
        ...
        return {
            observations,
            historyAhead,
            diagnostics: [...diagnostics, stateDiagnostic('audit-current-directory-invalid', '.atlas/audits', error instanceof Error ? error.message : String(error))],
        };
    }
    for (const repoPath of paths) {
        if (!repoPath.endsWith('.json')) {
            diagnostics.push(stateDiagnostic('audit-current-unexpected-entry', repoPath, 'audit current directory entries must be JSON files'));
            continue;
        }
        try {
            ...
            if (!raw ||
                ...
                Array.isArray(raw) ||
                (raw.formatVersion !== 3 &&
                    raw.format !== 'atlas-audit-v3')) {
                continue;
            }
            const parsed = parseAuditCurrentLedgerWithContext(root, repoPath, raw, context);
            if (!parsed.ok) {
                diagnostics.push(...parsed.diagnostics.map((entry) => ({
                    ...entry,
                    path: `${repoPath}${entry.path}`,
            ...
            diagnostics.push(stateDiagnostic('audit-current-read-failed', repoPath, error instanceof Error ? error.message : String(error)));
        }
    }
    for (const [slug, ledger] of [...currentBySlug.entries()]
        ...
        if (!history) {
            diagnostics.push(stateDiagnostic('audit-history-missing', ledger.history.path, 'V3 current ledger has no valid observation history'));
            continue;
        }
        try {
            ...
            diagnostics.push(stateDiagnostic('audit-current-history-mismatch', ledger.history.path, error instanceof Error ? error.message : String(error)));
            historyBySlug.delete(slug);
        }
    }
    for (const [slug, history] of [...historyBySlug.entries()]
        ...
        if (history.entries.length === 1) {
            ...
            diagnostics.push(stateDiagnostic('audit-history-without-current', `.atlas/audit-history/${slug}.json`, 'history without current may contain only one genesis history-ahead entry'));
        }
    }
...
function readHistoryForSlug(root, slug) {
    ...
    if (!parsed.ok)
        throw resultError(parsed, 'invalid existing audit history');
    return parsed.value;
}
function readCurrentForSlug(root, slug) {
    ...
    if (!parsed.ok)
        throw resultError(parsed, 'invalid existing current ledger');
    return parsed.value;
}
export function prepareAuditObservationPublication(root, observation, metadata) {
    ...
    if (existingCurrent?.current.observationId === parsedObservation.observationId &&
        (existingCurrent.title !== title ||
            existingCurrent.conceptSlug !== metadata.conceptSlug)) {
        throw new Error('same current observation conflicts with existing wrapper title or conceptSlug metadata');
    }
    let existingEntry;
    if (existingHistory) {
        if (existingCurrent) {
            ...
            if (trailing === 1) {
                ...
                if (existingEntry?.observationId !== parsedObservation.observationId ||
            ...
            if (existingHistory.entries.length !== 1) {
                throw new Error('history without current may contain only one genesis history-ahead entry');
            }
        ...
        const sameId = existingHistory.entries.find((entry) => entry.observationId === parsedObservation.observationId);
        if (sameId) {
            if (sameId.observationDigest !== observationDigest ||
    ...
    const historyEntry = existingEntry ?? {
        ...entryCore,
    ...
    const parsedLedger = parseAuditCurrentLedger(root, `.atlas/audits/${slug}.json`, ledger);
    if (!parsedLedger.ok)
        throw resultError(parsedLedger, 'prepared current ledger is invalid');
    return {
...
export function publishAuditObservation(root, ledger) {
    return withAuditLock(root, () => {
        ...
        const parsed = parseAuditCurrentLedger(root, `.atlas/audits/${candidateSnapshot.slug}.json`, candidateSnapshot);
        if (!parsed.ok)
            throw resultError(parsed, 'publication ledger is invalid');
        const prepared = prepareAuditObservationPublication(root, parsed.value.current, {
        ...
        const historyBytesMatch = existingHistory !== null &&
            Buffer.from(existingHistory.bytes).equals(Buffer.from(prepared.historyBytes, 'utf8'));
        if (!historyBytesMatch) {
            atomicWriteAuditFile(root, historyPath, prepared.historyBytes);
        }
        const existingCurrent = readOptionalAuditJsonDocument(root, currentPath, LEDGER_BYTE_LIMIT);
        ...
        const currentBytesMatch = existingCurrent !== null &&
            Buffer.from(existingCurrent.bytes).equals(Buffer.from(prepared.currentBytes, 'utf8'));
