...
function exactKeys(value, required, optional, pointer) {
    ...
    for (const key of Object.keys(value)) {
        if (!allowed.has(key))
            fail(`${pointer}/${key}`, 'unknown source field');
    }
    for (const key of required) {
        if (!Object.hasOwn(value, key))
            fail(`${pointer}/${key}`, 'missing source field');
    }
}
...
function sha1At(value, pointer) {
    const parsed = stringAt(value, pointer);
    if (!SHA1_RE.test(parsed))
        fail(pointer, 'expected a lowercase Git SHA-1');
    return parsed;
}
function nullableSha1At(value, pointer) {
    return value === null ? null : sha1At(value, pointer);
}
function dateAt(value, pointer) {
    const parsed = stringAt(value, pointer);
    if (!DATE_RE.test(parsed) ||
...
function repoPathAt(value, pointer) {
    try {
        ...
        fail(pointer, error instanceof Error ? error.message : 'invalid repository path');
    }
}
function stringsAt(value, pointer) {
    const rows = arrayAt(value, pointer).map((row, index) => stringAt(row, `${pointer}/${index}`));
    if (new Set(rows).size !== rows.length)
        fail(pointer, 'contains duplicates');
    return rows;
}
...
function parseRetirement(value, pointer) {
    const retirement = recordAt(value, pointer);
    exactKeys(retirement, ['reason', 'retiredAt', 'deletionCommit', 'successorPath', 'evidence'], [], pointer);
    ...
    if (retirement.reason === 'deleted' &&
        ...
        fail(`${pointer}/deletionCommit`, 'deleted retirement requires a full Git commit');
    }
    if (retirement.reason === 'uncommitted_snapshot_absent' &&
        deletionCommit !== null) {
        fail(`${pointer}/deletionCommit`, 'uncommitted snapshot absence forbids a deletion commit');
    }
...
function parseScan(value, index) {
    ...
    exactKeys(scan, [
        ...
        'finding_count',
        'findings_ref',
    ], ['note', 'remediated', 'retired'], pointer);
    const status = scan.status;
    if (status !== 'clean' && status !== 'findings') {
        fail(`${pointer}/status`, 'expected clean or findings');
    }
    const findingCount = integerAt(scan.finding_count, `${pointer}/finding_count`);
    ...
    if (scan.remediated !== undefined) {
        ...
        exactKeys(source, ['commit', 'at', 'fix_blob_sha1'], ['note'], `${pointer}/remediated`);
        remediated = {
            commit: stringAt(source.commit, `${pointer}/remediated/commit`),
            at: dateAt(source.at, `${pointer}/remediated/at`),
            fix_blob_sha1: sha1At(source.fix_blob_sha1, `${pointer}/remediated/fix_blob_sha1`),
    ...
    return {
        ...
        git_blob_sha1: sha1At(scan.git_blob_sha1, `${pointer}/git_blob_sha1`),
...
function parseLedger(value) {
    ...
    exactKeys(ledger, ['schema', 'ruleset', 'scanner', 'note', 'scans'], [], '/ledger.json');
    ...
    exactKeys(ruleset, ['id', 'categories'], [], '/ledger.json/ruleset');
    ...
    const categories = stringsAt(ruleset.categories, '/ledger.json/ruleset/categories');
    const scanner = recordAt(ledger.scanner, '/ledger.json/scanner');
    exactKeys(scanner, ['name', 'method', 'run'], [], '/ledger.json/scanner');
    const scans = arrayAt(ledger.scans, '/ledger.json/scans')
        .map(parseScan)
        .sort((left, right) => compareText(left.path, right.path));
    if (scans.length > 10_000)
        fail('/ledger.json/scans', 'exceeds 10000 rows');
...
function parseCandidate(value, index) {
    ...
    exactKeys(candidate, [
        ...
        'detail',
        'recommendedFix',
    ], ['id', 'duplicateOf', 'remediationCommit'], pointer);
    if (candidate.recordKind !== 'current_candidate' &&
        candidate.recordKind !== 'historical_remediation') {
        fail(`${pointer}/recordKind`, 'unsupported candidate record kind');
    }
    if (candidate.sourceKind !== 'round9' &&
    ...
    const identityKey = stringAt(candidate.identityKey, `${pointer}/identityKey`);
    ...
    return {
        ...
        sourceOrdinal: integerAt(candidate.sourceOrdinal, `${pointer}/sourceOrdinal`, 1),
        sourceLocator: stringAt(candidate.sourceLocator, `${pointer}/sourceLocator`),
        path: repoPathAt(candidate.path, `${pointer}/path`),
        symbolLocation: stringAt(candidate.symbolLocation, `${pointer}/symbolLocation`),
        category: stringAt(candidate.category, `${pointer}/category`),
        title: stringAt(candidate.title, `${pointer}/title`),
        normalizedTitle: stringAt(candidate.normalizedTitle, `${pointer}/normalizedTitle`),
        severity: severity,
        ...
        recommendedFix: stringAt(candidate.recommendedFix, `${pointer}/recommendedFix`),
        id,
        ...(duplicateOf === undefined ? {} : { duplicateOf }),
        ...(candidate.remediationCommit === undefined
            ? {}
            : {
                remediationCommit: stringAt(candidate.remediationCommit, `${pointer}/remediationCommit`),
            }),
    };
}
function countAt(counts, key) {
...
function parseCandidates(value) {
    ...
    exactKeys(candidates, ['schema', 'generatedFrom', 'counts', 'entries'], [], '/candidates.v1.json');
    if (candidates.schema !== CANDIDATE_SCHEMA) {
        fail('/candidates.v1.json/schema', `expected ${CANDIDATE_SCHEMA}`);
    }
    const generated = recordAt(candidates.generatedFrom, '/candidates.v1.json/generatedFrom');
    exactKeys(generated, ['round9ResultFiles', 'phaseZeroProvenance', 'atlasImport'], [], '/candidates.v1.json/generatedFrom');
    ...
    for (const candidate of entries) {
        ...
        if (locators.has(candidate.sourceLocator)) {
            fail('/candidates.v1.json/entries', `duplicate source locator ${candidate.sourceLocator}`);
        }
        locators.add(candidate.sourceLocator);
    }
    const canonicalIds = new Set(entries.filter((entry) => entry.duplicateOf === undefined)
        .map((entry) => entry.id));
    for (const candidate of entries) {
        if (candidate.duplicateOf !== undefined &&
            !canonicalIds.has(candidate.duplicateOf)) {
            fail(`/candidates.v1.json/entries/${candidate.sourceIndex}/duplicateOf`, 'does not resolve to a canonical candidate');
        }
    }
    const derived = {
        sourceRecords: entries.length,
        historicalRemediations: entries.filter((entry) => entry.recordKind === 'historical_remediation').length,
        currentCandidateRecords: entries.filter((entry) => entry.recordKind === 'current_candidate').length,
        duplicateRecords: entries.filter((entry) => entry.duplicateOf !== undefined).length,
        canonicalCurrentFindings: entries.filter((entry) => entry.recordKind === 'current_candidate' &&
            entry.duplicateOf === undefined).length,
        canonicalHistoricalFindings: entries.filter((entry) => entry.recordKind === 'historical_remediation' &&
            entry.duplicateOf === undefined).length,
        canonicalFindings: canonicalIds.size,
    };
    for (const [key, expected] of Object.entries(derived)) {
        if (countAt(counts, key) !== expected) {
            fail(`/candidates.v1.json/counts/${key}`, `declared count does not equal recomputed ${expected}`);
        }
    }
    return {
        schema: CANDIDATE_SCHEMA,
        generatedFrom: {
            round9ResultFiles: stringsAt(generated.round9ResultFiles, '/candidates.v1.json/generatedFrom/round9ResultFiles'),
            phaseZeroProvenance: repoPathAt(generated.phaseZeroProvenance, '/candidates.v1.json/generatedFrom/phaseZeroProvenance'),
            atlasImport: repoPathAt(generated.atlasImport, '/candidates.v1.json/generatedFrom/atlasImport'),
        },
        counts: derived,
        entries,
    };
}
function optionalArtifactBinding(value, pointer, blobKey) {
    ...
    return blobKey === 'fixBlob'
        ...
        : {
            ...common,
            reviewedBlob: sha1At(binding.reviewedBlob, `${pointer}/reviewedBlob`),
        };
}
function parseDisposition(value, index) {
    ...
    if (disposition.reviewedBlob !== undefined) {
        base.reviewedBlob = nullableSha1At(disposition.reviewedBlob, `${pointer}/reviewedBlob`);
    }
    if (disposition.postFixScan !== undefined) {
        base.postFixScan = optionalArtifactBinding(disposition.postFixScan, `${pointer}/postFixScan`, 'fixBlob');
    }
    if (disposition.currentScan !== undefined) {
        base.currentScan = disposition.currentScan === null
            ? null
            : optionalArtifactBinding(disposition.currentScan, `${pointer}/currentScan`, 'reviewedBlob');
    }
    if (disposition.regression !== undefined) {
        const regression = recordAt(disposition.regression, `${pointer}/regression`);
        exactKeys(regression, ['kind', 'name', 'command', 'result'], [], `${pointer}/regression`);
        if (regression.kind !== 'test' &&
    ...
    else if (base.status === 'accepted_risk' ||
        base.status === 'separate_design') {
        if (base.reviewedBlob === undefined ||
    ...
    else if ((base.replacementId === undefined ||
...
function parseDispositions(value) {
    const document = recordAt(value, '/dispositions.v1.json');
    exactKeys(document, ['schema', 'dispositions'], [], '/dispositions.v1.json');
    if (document.schema !== DISPOSITION_SCHEMA) {
        fail('/dispositions.v1.json/schema', `expected ${DISPOSITION_SCHEMA}`);
    }
    const dispositions = arrayAt(document.dispositions, '/dispositions.v1.json/dispositions')
        .map(parseDisposition)
        .sort((left, right) => compareText(left.id, right.id));
    const ids = new Set();
    for (const disposition of dispositions) {
        if (ids.has(disposition.id)) {
            fail('/dispositions.v1.json/dispositions', `duplicate disposition ${disposition.id}`);
        }
...
function parseProvenance(value) {
    ...
    exactKeys(document, ['schema', 'legacyRecords', 'round9SourceBlobs'], [], '/phase-zero-provenance.v1.json');
    if (document.schema !== PROVENANCE_SCHEMA) {
        fail('/phase-zero-provenance.v1.json/schema', `expected ${PROVENANCE_SCHEMA}`);
    }
    const legacyRecords = arrayAt(document.legacyRecords, '/phase-zero-provenance.v1.json/legacyRecords').map((value_, index) => recordAt(value_, `/phase-zero-provenance.v1.json/legacyRecords/${index}`));
    const sourceBlobs = recordAt(document.round9SourceBlobs, '/phase-zero-provenance.v1.json/round9SourceBlobs');
    const round9SourceBlobs = {};
    for (const [unsafePath, value_] of Object.entries(sourceBlobs)) {
        const repoPath = repoPathAt(unsafePath, '/phase-zero-provenance.v1.json/round9SourceBlobs');
        round9SourceBlobs[repoPath] = sha1At(value_, `/phase-zero-provenance.v1.json/round9SourceBlobs/${repoPath}`);
    }
    return {
...
function policyGlobsAt(value, pointer, nonempty) {
    const rows = stringsAt(value, pointer);
    if (nonempty && rows.length === 0)
        fail(pointer, 'must not be empty');
    if (rows.length > 10_000)
        fail(pointer, 'contains too many globs');
...
function parseMigrationPolicy(value, policyPath) {
    ...
    const units = arrayAt(policy.units, `/${policyPath}/units`)
        ...
        const include = policyGlobsAt(unit.include, `${pointer}/include`, true);
        ...
        historicalAssignments = (policy.historicalUnitAssignments === undefined
            ...
            : arrayAt(policy.historicalUnitAssignments, `/${policyPath}/historicalUnitAssignments`).map((value_, index) => {
                ...
                return {
                    ...
                    include: policyGlobsAt(assignment.include, `${pointer}/include`, true),
                };
            }));
    }
    else {
        const legacyTargets = new Set(RELAYOS_LEGACY_HISTORICAL_ASSIGNMENTS.map(({ unit }) => unit));
        historicalAssignments = [...legacyTargets].every((slug) => slugs.has(slug))
    ...
    historicalAssignments.sort((left, right) => compareText(left.id, right.id));
...
function validateCrossSource(source) {
    const scansByPath = new Map(source.ledger.scans.map((scan) => [scan.path, scan]));
    const canonicalCandidates = source.candidates.entries.filter((candidate) => candidate.duplicateOf === undefined);
    const canonicalIds = new Set(canonicalCandidates.map(({ id }) => id));
    const dispositions = new Map(source.dispositions.dispositions.map((disposition) => [
        disposition.id,
        disposition,
    ]));
    if (dispositions.size !== canonicalIds.size ||
        [...canonicalIds].some((id) => !dispositions.has(id)) ||
        [...dispositions].some(([id]) => !canonicalIds.has(id))) {
        fail('/dispositions.v1.json/dispositions', 'canonical candidates and dispositions are not one-to-one');
    }
    for (const candidate of source.candidates.entries) {
        if (!scansByPath.has(candidate.path)) {
            fail(`/candidates.v1.json/entries/${candidate.sourceIndex}/path`, 'candidate path has no legacy scan record');
        }
        if (candidate.duplicateOf !== undefined)
            continue;
        const disposition = dispositions.get(candidate.id);
        if (disposition.sourceBlob !== candidate.sourceBlob) {
            fail(`/dispositions.v1.json/dispositions/${disposition.sourceIndex}/sourceBlob`, `does not match candidate ${candidate.id}`);
        }
    }
    const legacyLocators = new Set();
    for (const [index, row] of source.provenance.legacyRecords.entries()) {
        const sourceKind = stringAt(row.sourceKind, `/phase-zero-provenance.v1.json/legacyRecords/${index}/sourceKind`);
        if (sourceKind !== 'legacy_ledger') {
            fail(`/phase-zero-provenance.v1.json/legacyRecords/${index}/sourceKind`, 'expected legacy_ledger');
        }
        legacyLocators.add(stringAt(row.sourceLocator, `/phase-zero-provenance.v1.json/legacyRecords/${index}/sourceLocator`));
    }
    ...
    if (legacyLocators.size !== candidateLegacyLocators.length ||
        candidateLegacyLocators.some((locator) => !legacyLocators.has(locator))) {
        fail('/phase-zero-provenance.v1.json/legacyRecords', 'legacy candidate provenance is incomplete or inconsistent');
    }
    for (const candidate of source.candidates.entries) {
        if (candidate.sourceKind === 'round9' &&
            source.provenance.round9SourceBlobs[candidate.path] !==
                candidate.sourceBlob) {
            fail(`/phase-zero-provenance.v1.json/round9SourceBlobs/${candidate.path}`, `does not match round9 candidate ${candidate.id}`);
        }
    }
}
function parseRepositoryId(root) {
    ...
    if (!/^repo_[a-z0-9](?:[a-z0-9_-]*[a-z0-9])?$/u.test(repositoryId)) {
        fail('/.atlas/config.json/repositoryId', 'expected a stable lowercase repo_ identity');
    }
    return repositoryId;
}
function semanticSourceValue(source) {
    return {
        ledger: {
            ...source.ledger,
        ...
        candidates: {
            ...source.candidates,
            entries: source.candidates.entries.map(({ sourceIndex: _index, ...candidate }) => candidate),
        },
        dispositions: {
            ...source.dispositions,
            dispositions: source.dispositions.dispositions.map(({ sourceIndex: _index, ...disposition }) => disposition),
        },
        provenance: {
            ...source.provenance,
            legacyRecords: [...source.provenance.legacyRecords].sort((left, right) => compareText(String(left.sourceLocator ?? ''), String(right.sourceLocator ?? ''))),
        },
...
function readSource(git, options) {
    ...
    if (policy.schema === POLICY_SCHEMA) {
        ...
        policy = {
            ...policy,
            digest: `sha256:${loaded.policyHash}`,
        };
    }
    return {
...
function policyMatcher(include, except = []) {
...
function partitionLegacyPaths(source, policyPath) {
    ...
    const historical = source.policy.historicalAssignments.map((assignment) => ({
        assignment,
        matches: policyMatcher(assignment.include),
    }));
    const pathUnits = new Map();
    ...
    for (const candidate of source.candidates.entries) {
        if (!pathUnits.has(candidate.path)) {
            fail(`/candidates.v1.json/entries/${candidate.sourceIndex}/path`, 'candidate path has no policy-unit assignment');
        }
    }
    return {
        pathUnits,
        unitTitles: new Map(source.policy.units.map(({ slug, title }) => [slug, title])),
    };
}
...
function currentStateDigest(repositoryRevision, currentFiles) {
...
function exactUtf8LineCount(bytes, pointer) {
    if (bytes.length === 0)
        return 0;
    let text;
    try {
...
function buildContext(root, options) {
    return withAnchoredAuditSupportSnapshot(root, () => {
        ...
        return withAnchoredAuditGitCapability(root, (git) => {
            ...
            ], AUDIT_LIMITS.jsonBytes)).toString('ascii').trim();
            const availableObjects = new Set(objectInventory.length === 0 ? [] : objectInventory.split(/\r?\n/u));
...
function producerFor(context, layer) {
    return {
        kind: 'migration',
...
function locationFor(candidate, maximumLine) {
    const matches = [...candidate.symbolLocation.matchAll(/:(\d+)(?:-(\d+))?(?=$|[^0-9])/gu)];
...
function candidateFingerprint(context, candidate) {
...
function candidateFinding(context, candidate, observationId, unit, options = {}) {
    const ruleId = `relayos-security-scan/${normalizeRulePart(candidate.category)}`;
    ...
    if (options.exactCode !== undefined) {
        ...
        if (code !== undefined) {
            codeEvidence = [{
                    ...
                    code,
                    explanation: 'Preserved only because the legacy source blob matches repository bytes.',
                }];
        }
    }
    return {
...
function scanFingerprint(context, scan, ordinal) {
    return computeAtlasFingerprint({
        repositoryId: context.repositoryId,
        domain: 'security',
...
function scanFinding(context, scan, ordinal, observationId, unit) {
    ...
    return {
        ...
        summary: `The sealed RelayOS ledger recorded finding ${ordinal} of ` +
        ...
        remediation: `Consult the sealed legacy finding reference ${sourceReference}.`,
        provenance: {
            source: SOURCE_KIND,
...
function scanLineCount(context, scan) {
    return context.blobLineCounts.get(scan.git_blob_sha1) ?? scan.lines;
}
function exactObservation(context, unit, layer, scans, findingBuilder) {
    ...
    const files = scans.map((scan) => {
        const occurrences = [...(occurrenceByPath.get(scan.path) ?? [])]
            .sort(compareText);
        if ((scan.status === 'findings' && occurrences.length !== scan.finding_count) ||
            (scan.status === 'clean' && occurrences.length !== 0)) {
            fail(`/ledger.json/scans/${scan.sourceIndex}`, `mapped occurrence count ${occurrences.length} does not equal ` +
...
function candidateObservation(context, unit, candidates) {
    ...
    const findings = candidates.map((candidate) => candidateFinding(context, candidate, observationId, unit))
        .sort((left, right) => compareText(left.findingId, right.findingId));
    return {
        observationId,
        observedAt: context.recordedAt,
        ...
        exactCoverage: {
            completeness: 'unknown',
            basis: 'unavailable',
            reason: 'Candidate reconciliation is semantic evidence; exact file receipts ' +
...
function exactCodeFor(context, candidate, scan) {
...
function buildObservations(context) {
    ...
    const dispositionById = new Map(context.source.dispositions.dispositions.map((disposition) => [
        disposition.id,
        disposition,
    ]));
    const scansByUnit = new Map();
    const scanUnit = new Map();
    ...
    const candidateUnit = new Map();
    for (const candidate of canonicalCandidates) {
        const unit = scanUnit.get(candidate.path);
        if (unit === undefined) {
            fail(`/candidates.v1.json/entries/${candidate.sourceIndex}/path`, 'candidate path has no scan policy unit');
        }
    ...
    for (const unit of [...units].sort(compareText)) {
        ...
        const unitCandidates = [...(candidatesByUnit.get(unit) ?? [])]
            .sort((left, right) => compareText(left.id, right.id));
        const baseline = exactObservation(context, unit, 'baseline', allScans, (observationId) => allScans.flatMap((scan) => Array.from({ length: scan.finding_count }, (_, index) => scanFinding(context, scan, index + 1, observationId, unit))));
        const candidates = candidateObservation(context, unit, unitCandidates);
        for (const finding of candidates.findings) {
            const alias = finding.fingerprints.find(({ scheme }) => scheme === 'relayos-security-scan/v1')?.value;
            if (alias !== undefined)
                candidateFindings.set(alias, finding);
        }
        ...
        const currentCandidates = unitCandidates.filter((candidate) => {
            ...
            return (disposition.status === 'accepted_risk' ||
                disposition.status === 'separate_design') &&
                disposition.currentScan !== undefined &&
                disposition.currentScan !== null &&
                activeScans.some((scan) => scan.path === candidate.path &&
                    scan.git_blob_sha1 === disposition.currentScan.reviewedBlob);
        });
        const current = exactObservation(context, unit, 'current', activeScans, (observationId) => currentCandidates.map((candidate) => {
            const scan = activeScans.find((row) => row.path === candidate.path);
            return candidateFinding(context, candidate, observationId, unit, {
        ...
        for (const finding of current.findings) {
            const alias = finding.fingerprints.find(({ scheme }) => scheme === 'relayos-security-scan/v1')?.value;
            if (alias !== undefined)
                currentFindings.set(alias, finding);
        }
...
function decisionBinding(candidate, blob) {
    return {
        path: candidate.path,
        blob: `git-sha1:${blob}`,
    };
}
function reviewContext(context, candidate, observationId, blob) {
    return {
        observationId,
        bindings: [decisionBinding(candidate, blob)],
        ruleset: {
            id: CANONICAL_RULESET,
            digest: context.rulesetDigest,
        },
...
function buildDispositionInput(context, observations, candidate, disposition) {
    ...
    const dispositionArtifact = sourceArtifact(context, 'dispositions.v1.json');
    const reviewObservationId = (disposition.status === 'accepted_risk' ||
        disposition.status === 'separate_design' ||
        disposition.status === 'false_positive')
        ? layer.current.observationId
        : layer.candidates.observationId;
    const reviewBlob = (disposition.status === 'accepted_risk' ||
        disposition.status === 'separate_design' ||
        disposition.status === 'false_positive')
    ...
    const common = {
        ...
        occurrenceId: observations.candidateFindings.get(candidate.id).occurrenceId,
        actor: MIGRATION_ACTOR,
        owner: disposition.owner,
        reason: disposition.rationale,
        createdAt: context.recordedAt,
        createdAtBasis: 'source-revision-upper-bound',
        reviewContext: reviewContext(context, candidate, reviewObservationId, reviewBlob),
        evidenceRefs: evidenceRefs(disposition.evidence),
        reviews: [],
    };
    if (disposition.status === 'accepted_risk' ||
        disposition.status === 'separate_design') {
    ...
    if (disposition.replacementId !== undefined &&
        disposition.replacementId !== null) {
        const replacement = observations.candidateFindings.get(disposition.replacementId);
        if (replacement === undefined) {
            fail(`/dispositions.v1.json/dispositions/${disposition.sourceIndex}/replacementId`, 'does not resolve to a canonical finding');
        }
        return {
            ...common,
            action: 'superseded',
...
function buildRetirementInput(context, observations, scan) {
    ...
    if (retirement.successorPath !== null) {
        const successorScan = context.source.ledger.scans.find((candidate) => candidate.path === retirement.successorPath);
        const successorBlob = context.currentFiles.get(retirement.successorPath)?.sha1 ?? successorScan?.git_blob_sha1;
...
function buildAliasInput(context, observations, candidate) {
    ...
    if (canonicalFinding === undefined) {
        fail(`/candidates.v1.json/entries/${candidate.sourceIndex}`, 'candidate alias has no canonical destination');
    }
    return {
...
function buildEvents(context, observations) {
    const canonicalCandidates = new Map(context.source.candidates.entries
        .filter((candidate) => candidate.duplicateOf === undefined)
    ...
    for (const candidate of context.source.candidates.entries) {
        ...
        inputs.push({
            input,
            slug: observations.candidateFindings.get(candidate.duplicateOf ?? candidate.id).decisionLedger,
        });
    }
    for (const disposition of context.source.dispositions.dispositions) {
        const candidate = canonicalCandidates.get(disposition.id);
        inputs.push({
            input: buildDispositionInput(context, observations, candidate, disposition),
            slug: observations.candidateUnit.get(candidate.id),
        });
    }
    inputs.sort((left, right) => compareText(left.slug, right.slug) ||
    ...
    for (const { input, slug } of inputs) {
        const event = prepareAuditDecisionAppend(null, 'security', slug, input).event;
        const { eventId } = event;
    ...
    const bytes = new Map([...ledgers].map(([slug, ledger]) => [
        slug,
...
function buildHistory(slug, title, observations) {
...
function output(path, bytes, family) {
    return {
        path,
        bytes,
        sha256: rawSha256(bytes),
...
function dispositionHistogram(dispositions) {
    return {
        remediated: dispositions.filter(({ status }) => status === 'remediated').length,
        separateDesign: dispositions.filter(({ status }) => status === 'separate_design').length,
        acceptedRisk: dispositions.filter(({ status }) => status === 'accepted_risk').length,
        falsePositive: dispositions.filter(({ status }) => status === 'false_positive').length,
        superseded: dispositions.filter(({ status }) => status === 'superseded').length,
    };
}
function buildMappings(context, observations, events) {
    const mappings = [];
    for (const scan of context.source.ledger.scans) {
        const unit = observations.scanUnit.get(scan.path);
        const baseline = observations.byUnit.get(unit).baseline;
        ...
        if (scan.retired !== undefined) {
            const retirement = events.retirementEvents.find((event) => event.path === scan.path && event.blob === `git-sha1:${scan.git_blob_sha1}`);
    ...
    for (const candidate of context.source.candidates.entries) {
        ...
        const alias = events.reconciliationEvents.find((event) => event.aliases.some(({ value }) => value === candidate.id));
        if (alias === undefined) {
    ...
    for (const disposition of context.source.dispositions.dispositions) {
        const event = events.decisionEvents.find((candidate) => candidate.findingId ===
    ...
    return mappings.sort((left, right) => compareText(left.sourcePath, right.sourcePath) ||
...
function buildPlan(root, options) {
    ...
    for (const [unit, layers] of [...observations.byUnit.entries()]
        ...
        if (title === undefined)
            fail(`/outputs/${unit}`, 'missing policy unit title');
    ...
    for (const [unit, bytes] of [...events.bytes.entries()]
        .sort(([left], [right]) => compareText(left, right))) {
        outputs.push(output(`.atlas/audit-decisions/${unit}.json`, bytes, 'decision'));
    }
    outputs.sort((left, right) => compareText(left.path, right.path));
    const active = context.source.ledger.scans.filter((scan) => scan.retired === undefined);
    const exactMatches = active.filter((scan) => context.currentFiles.get(scan.path)?.sha1 === scan.git_blob_sha1).length;
    const counts = {
        ...
        activeFindingOccurrences: active.reduce((sum, scan) => sum + scan.finding_count, 0),
        candidateSourceRecords: context.source.candidates.entries.length,
        canonicalFindings: context.source.candidates.entries.filter((candidate) => candidate.duplicateOf === undefined).length,
        duplicateCandidates: context.source.candidates.entries.filter((candidate) => candidate.duplicateOf !== undefined).length,
        dispositions: dispositionHistogram(context.source.dispositions.dispositions),
    };
    const receiptCore = {
        formatVersion: 1,
        ...
        parityChecks: [
            ...
            },
            {
                ...
                status: 'passed',
                details: `${counts.candidateSourceRecords} source candidates mapped to ` +
            ...
            },
            {
                ...
                status: 'passed',
                details: `${counts.dispositions.remediated}/` +
    ...
    outputs.push(output(receiptPath, `${canonicalJson(receipt)}\n`, 'receipt'));
    const orderedOutputs = [...outputs].sort((left, right) => compareText(left.path, right.path));
    const result = {
        migrationId: context.migrationId,
        receipt,
...
function applyPlan(root, options, expected) {
    ...
    withAuditLock(root, () => {
        ...
        for (const output_ of current.outputs) {
            let existing = null;
            try {
                existing = readBoundedAuditBytes(root, output_.path, Math.max(Buffer.byteLength(output_.bytes, 'utf8'), AUDIT_LIMITS.jsonBytes));
            }
            catch (error) {
            ...
            if (existing !== null &&
                ...
                fail(`/apply/${output_.path}`, 'existing migration output diverges from the deterministic plan');
            }
        }
        const applyOrder = [
            'history',
        ...
        for (const family of applyOrder) {
            for (const output_ of current.outputs.filter((candidate) => candidate.family === family)) {
                let existingMatches = false;
                try {
                    existingMatches = Buffer.from(readBoundedAuditBytes(root, output_.path, Buffer.byteLength(output_.bytes, 'utf8'))).equals(Buffer.from(output_.bytes, 'utf8'));
                }
                catch (error) {
                    if (!isMissingOutput(error))
                        throw error;
        ...
        for (const output_ of current.outputs) {
            const bytes = readBoundedAuditBytes(root, output_.path, Buffer.byteLength(output_.bytes, 'utf8'));
