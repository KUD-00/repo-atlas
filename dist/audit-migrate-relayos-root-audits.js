...
import { AUDIT_LIMITS, atomicWriteAuditFile, canonicalJson, normalizeAuditRepoPath, parseBoundedAuditJsonBytes, readBoundedAuditBytes, withAnchoredAuditGitCapability, withAnchoredAuditSupportSnapshot, withAuditLock, } from './audit-core.js';
import { computeAuditCanonicalDigest } from './audit-v3.js';
import { gitText, parseRepositoryId, readRevisionFile, RelayOSMigrationError, verifyRevisionCommit, } from './audit-migrate-relayos.js';
...
const DESIGN_PROSE_FILES = ['README.md', 'findings.md'];
...
function fail(pointer, message) {
    throw new RelayOSRootAuditsMigrationError(`RelayOS root-audits migration ${pointer}: ${message}`);
}
function translateSharedFailure(error) {
    if (error instanceof RelayOSMigrationError) {
        throw new RelayOSRootAuditsMigrationError(error.message.replace(/^RelayOS legacy migration/u, 'RelayOS root-audits migration'));
    }
    throw error;
}
...
function repoPathAt(value, pointer) {
    try {
        ...
        if (error instanceof RelayOSRootAuditsMigrationError)
            throw error;
...
function parseJsonDocument(bytes, repoPath, pointer) {
    try {
        return parseBoundedAuditJsonBytes(bytes, SOURCE_BYTES, repoPath);
    }
    catch (error) {
        fail(pointer, error instanceof Error ? error.message : 'is not valid bounded JSON');
    }
}
function parseDesignScan(value, index) {
...
function parseDesignScanLedger(value) {
    const pointer = '/design-scan/ledger.json';
    const ledger = recordAt(value, pointer);
    exactKeys(ledger, ['schema', 'note', 'ruleset', 'scanner', 'scans'], [], pointer);
    ...
    const categories = stringsAt(ruleset.categories, `${pointer}/ruleset/categories`);
    ...
    if (scans.length > 10_000)
        fail(`${pointer}/scans`, 'exceeds 10000 rows');
...
function headingPaths(heading, scannedPaths, pointer) {
    const parts = heading
        .split('·')
        .map((part) => part.trim())
    ...
    if (first === undefined || !scannedPaths.has(first)) {
        fail(pointer, `heading path is outside the audited scope: ${first ?? heading}`);
    }
    const directory = first.includes('/')
        ? first.slice(0, first.lastIndexOf('/'))
        : '.';
    return parts.map((part, index) => index === 0 ? part : directory === '.' ? part : `${directory}/${part}`);
}
function resolveLocationPath(token, candidates, pointer) {
    const hit = candidates.filter((candidate) => candidate === token || candidate.endsWith(`/${token}`));
    if (hit.length !== 1) {
        fail(pointer, `location "${token}" resolves to ${hit.length} scope paths ` +
...
function parseDesignFindingsDocument(text, scannedPaths, categories, pointer) {
    const findings = [];
    const dropped = [];
    let heading = null;
    let section = null;
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        const bullet = /^- \*\*\[(MEDIUM|LOW|HIGH)\]\[([a-z-]+)\]\[(high|medium|low)\]\*\*\s+(.+)$/u
            .exec(line);
        if (bullet === null)
            continue;
        if (heading === null) {
        ...
        const locations = [];
        for (const match of locationSegment.matchAll(/`([^`:]+):([0-9,\-–]+)`/gu)) {
    ...
    for (const finding of findings) {
        const paths = finding.locations.map((location) => location.slice(0, location.lastIndexOf(':')));
        if (!paths.some((repoPath) => scannedPaths.has(repoPath))) {
            fail(pointer, `finding lands entirely outside the audited scope: ${finding.locations.join(', ')}`);
        }
    }
    return { findings, dropped };
}
function parseDesignV2Finding(value, index, scannedPaths, pointer) {
    ...
    const confidence = stringAt(finding.confidence, `${findingPointer}/confidence`);
    ...
    const locations = stringsAt(finding.locations, `${findingPointer}/locations`).map((location, locationIndex) => {
...
function parseDesignV2Dropped(value, index, pointer) {
...
function designScopeHash(files, hashes) {
    const lines = files.map((file) => `${hashes.get(file) ?? ''}  ${file}`);
    return createHash('sha1')
        .update([...lines].sort().join('\n') + '\n', 'utf8')
        .digest('hex');
}
function validateDesignV2Ledger(value, seal, designLedgersPath, designScan, expectedEvidenceRefs) {
    ...
    if (latestScan !== undefined && scannedAt !== latestScan) {
        fail(`${pointer}/scanned_at`, 'does not match the latest legacy scan date');
    }
    const scopeHash = sha1At(ledger.scope_hash, `${pointer}/scope_hash`);
    const files = stringsAt(ledger.files, `${pointer}/files`).map((file, index) => repoPathAt(file, `${pointer}/files/${index}`));
    const sortedFiles = [...files].sort(compareText);
    if (files.length !== sortedFiles.length ||
    ...
    for (const [key, hash] of Object.entries(hashesValue)) {
        hashes.set(repoPathAt(key, `${pointer}/hashes/${key}`), sha1At(hash, `${pointer}/hashes/${key}`));
    }
    ...
    const evidenceRefs = stringsAt(ledger.evidenceRefs, `${pointer}/evidenceRefs`).map((ref, index) => repoPathAt(ref, `${pointer}/evidenceRefs/${index}`));
    if ([...evidenceRefs].sort(compareText).join('\n') !==
        [...expectedEvidenceRefs].sort(compareText).join('\n')) {
        fail(`${pointer}/evidenceRefs`, 'does not name exactly the legacy design-scan sources');
    }
    const scannedPaths = new Set(designScan.scans.map((scan) => scan.path));
    const findings = arrayAt(ledger.findings, `${pointer}/findings`).map((finding, index) => parseDesignV2Finding(finding, index, scannedPaths, pointer));
    const dropped = arrayAt(ledger.dropped, `${pointer}/dropped`).map((entry, index) => parseDesignV2Dropped(entry, index, pointer));
    return {
        seal,
        slug,
...
function verifyDesignParity(context) {
    ...
    if (ledger.files.length !== scanPaths.length ||
        ledger.files.some((file, index) => file !== scanPaths[index])) {
        fail(`${pointer}/files`, 'V2 scope does not equal the sorted legacy scan paths');
    }
    for (const scan of scans) {
        const hash = ledger.hashes.get(scan.path);
        if (hash !== scan.gitBlobSha1) {
            fail(`${pointer}/hashes/${scan.path}`, 'V2 file hash does not match the legacy scan seal');
        }
    }
    const recomputed = designScopeHash(ledger.files, ledger.hashes);
    if (recomputed !== ledger.scopeHash) {
        fail(`${pointer}/scope_hash`, 'V2 scope hash does not match its own file seals');
    }
    if (ledger.findings.length !== context.parsedFindings.length) {
        fail(`${pointer}/findings`, `V2 records ${ledger.findings.length} findings but ` +
    ...
    for (const [index, parsed] of context.parsedFindings.entries()) {
        ...
        const findingPointer = `${pointer}/findings/${index}`;
        if (candidate.severity !== parsed.severity ||
            ...
            candidate.locations.some((location, locationIndex) => location !== parsed.locations[locationIndex])) {
            fail(findingPointer, 'V2 finding does not match the exact legacy prose parse');
        }
    }
    if (ledger.dropped.length !== context.parsedDropped.length) {
        fail(`${pointer}/dropped`, 'V2 bar-held entries do not match the legacy prose parse');
    }
    for (const [index, parsed] of context.parsedDropped.entries()) {
        const candidate = ledger.dropped[index];
        if (candidate.subject !== parsed.subject ||
            candidate.reason !== parsed.reason ||
            candidate.outcome !== parsed.outcome) {
            fail(`${pointer}/dropped/${index}`, 'V2 bar-held entry does not match the exact legacy prose parse');
        }
    }
    const findingsByPath = new Map();
    for (const finding of context.parsedFindings) {
        const files = new Set(finding.locations.map((location) => location.slice(0, location.lastIndexOf(':'))));
        for (const file of files) {
    ...
    for (const scan of scans) {
        ...
        if (actual !== scan.findingCount) {
            fail(`/design-scan/ledger.json/scans/${scan.sourceIndex}/finding_count`, `declares ${scan.findingCount} findings but ${actual} parsed findings ` +
                `cite ${scan.path}`);
        }
    }
}
function listDesignLedgerSeals(git, revision, designLedgersPath) {
    ...
    for (const record of records) {
        ...
        if (name.includes('/') || !DESIGN_LEDGER_NAME_RE.test(name))
            continue;
    ...
    for (const candidate of candidates) {
        const document = readRevisionFile(git, revision, candidate.path, SOURCE_BYTES, '/validation');
        if (document === null) {
            fail(`/validation/${candidate.path}`, 'design ledger vanished from the pinned revision tree');
        }
        seals.push({
            path: candidate.path,
...
function snapshotOptions(unsafeOptions) {
    ...
    exactKeys(options, ['sourceRevision', 'validationRevision'], ['auditsRoot', 'designLedgersPath', 'historicalArtifactsPath', 'apply'], '/options');
    ...
    const historicalArtifactsPath = options.historicalArtifactsPath === undefined
        ...
        : repoPathAt(options.historicalArtifactsPath, '/options/historicalArtifactsPath');
...
function readSourceDocument(git, revision, repoPath) {
    const document = readRevisionFile(git, revision, repoPath, SOURCE_BYTES, '/source');
    if (document === null) {
        fail(`/source/${repoPath}`, 'canonical source file is missing at the source revision');
    }
    return document;
}
function sealOf(repoPath, bytes, gitBlob) {
...
function buildContext(root, options) {
    return withAnchoredAuditSupportSnapshot(root, () => {
        ...
        return withAnchoredAuditGitCapability(root, (git) => {
            ...
            const sealSource = (repoPath) => {
                const document = readSourceDocument(git, options.sourceRevision, repoPath);
            ...
            const historicalReports = HISTORICAL_REPORTS.map(({ source, artifact }) => {
                ...
                try {
                    ...
                    fail(`/source/${sourcePath}`, 'historical report is not strict UTF-8');
                }
            ...
            const egressBefore = sealOf(egressBeforePath, egressBeforeBytes, sourceFiles.get(egressBeforePath).gitBlob);
            const designScan = parseDesignScanLedger(parseJsonDocument(designBytes.get('ledger.json'), `${designScanRoot}/ledger.json`, `/source/${designScanRoot}/ledger.json`));
            let findingsText;
            try {
                findingsText = UTF8_STRICT.decode(designBytes.get('findings.md'));
            }
            catch {
                fail(`/source/${designScanRoot}/findings.md`, 'findings document is not strict UTF-8');
            }
            const scannedPaths = new Set(designScan.scans.map((scan) => scan.path));
            const { findings: parsedFindings, dropped: parsedDropped } = parseDesignFindingsDocument(findingsText, scannedPaths, new Set(designScan.categories), `/source/${designScanRoot}/findings.md`);
            ...
            const egressAfterDocument = readRevisionFile(git, options.validationRevision, EGRESS_POLICY_DESTINATION, SOURCE_BYTES, '/validation');
            if (egressAfterDocument === null) {
                fail(`/validation/${EGRESS_POLICY_DESTINATION}`, 'egress policy destination is missing at the validation revision; ' +
                    'the reviewed RelayOS product edit must land first');
            }
            const egressAfter = sealOf(EGRESS_POLICY_DESTINATION, egressAfterDocument.bytes, egressAfterDocument.gitBlob);
            if (egressAfter.gitBlob !== egressBefore.gitBlob) {
                fail(`/validation/${EGRESS_POLICY_DESTINATION}`, 'egress policy destination bytes differ from the sealed source bytes');
            }
            const designLedgers = listDesignLedgerSeals(git, options.validationRevision, options.designLedgersPath);
            ...
            for (const seal of designLedgers) {
                const document = readRevisionFile(git, options.validationRevision, seal.path, SOURCE_BYTES, '/validation');
                if (document === null) {
                    fail(`/validation/${seal.path}`, 'design ledger vanished from the pinned revision tree');
                }
                const value = parseJsonDocument(document.bytes, seal.path, `/validation/${seal.path}`);
                const record = recordAt(value, `/validation/${seal.path}`);
                if (record.format !== 'atlas-audit-v2' || record.domain !== 'design') {
                    fail(`/validation/${seal.path}`, 'design-*.json is not an atlas-audit-v2 design ledger');
                }
                const refs = arrayAt(record.evidenceRefs, `/validation/${seal.path}/evidenceRefs`).map((ref, index) => stringAt(ref, `/validation/${seal.path}/evidenceRefs/${index}`));
                if (expectedEvidenceRefs.every((expected) => refs.includes(expected))) {
                    claiming.push(validateDesignV2Ledger(value, seal, options.designLedgersPath, designScan, expectedEvidenceRefs));
                }
            }
            if (claiming.length === 0) {
                fail(`/validation/${options.designLedgersPath}`, 'no design V2 ledger claims the legacy design scan; every legacy ' +
                    'unit and finding would be unmapped');
            }
            if (claiming.length > 1) {
                fail(`/validation/${options.designLedgersPath}`, `multiple design V2 ledgers claim the legacy design scan: ${claiming
                    .map(({ seal }) => seal.path)
                    .join(', ')}`);
            }
...
function output(path, bytes, sha256, family) {
    return {
        path,
        bytes,
        sha256,
...
function buildMappings(context, receiptPath) {
    const mappings = [];
    ...
    for (const name of ['check.mjs', 'to-atlas-ledger.mjs']) {
...
function buildPlan(root, options) {
    ...
    const outputs = [];
    for (const report of context.historicalReports) {
        outputs.push(output(report.artifactPath, report.text, report.sha256, 'artifact'));
    }
    const counts = {
        ...
        parityChecks: [
            {
                ...
                status: 'passed',
                details: `${DESIGN_SCAN_FILES.length} design-scan files sealed by Git blob ` +
                    'and SHA-256 at the source revision',
            },
            {
                name: 'design V2 ledger parity exact',
                status: 'passed',
                details: `${counts.designScanRecords} scan records, ` +
            ...
            },
            {
                ...
                status: 'passed',
                details: `${counts.historicalReports} final reports projected to ` +
                    `${context.options.historicalArtifactsPath} with original Git blobs`,
            },
            {
                name: 'egress policy relocation recorded',
                status: 'passed',
                details: `${context.egressBefore.path} -> ${context.egressAfter.path} is ` +
...
function applyPlan(root, options, expected) {
    ...
    withAuditLock(root, () => {
        ...
        const applyOrder = ['artifact', 'receipt'];
...
export function buildRelayOSRootAuditsMigration(root, unsafeOptions) {
    try {
...
export function migrateRelayOSRootAudits(root, unsafeOptions) {
    try {
        const options = snapshotOptions(unsafeOptions);
