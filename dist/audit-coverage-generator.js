...
function snapshotFailureResult(prepared, error, wrote = false) {
    const snapshotError = diagnostic('coverage-snapshot-invalid', `coverage inputs changed or could not be retained as one snapshot: ${error instanceof Error ? error.message : String(error)}`);
...
function checkAuditCoverageUnlocked(root, options = {}) {
    ...
    if (!prepared.writeForbiddenObjectFormat && !current) {
        if (existing.error !== null)
            ...
            byteErrors.push(diagnostic('coverage-byte-drift', 'committed coverage bytes are not the exact canonical generated bytes', { path: COVERAGE_PATH }));
        }
    }
    const diagnostics = uniqueDiagnostics([
        ...prepared.diagnostics,
