...
function pathlike(t) {
    ...
    if (/^[@\-#/~.]/.test(t))
        return false;
    if (t.includes('@') || t.includes('://') || t.startsWith('node:'))
        return false;
    if (!/^[\w./-]+$/.test(t))
        return false;
    if (!t.includes('/'))
        return BARE_FILE_EXT.test(t);
...
export function checkRefs(root, scanResult, notes, moved = []) {
    ...
    const exists = (p) => {
        if (present(p))
            return true;
        if (/\.[cm]?js$/.test(p))
            return present(p.replace(/\.([cm]?)js$/, '.$1ts')) || present(p.replace(/\.js$/, '.tsx'));
        if (!/\.\w+$/.test(p)) {
    ...
    const addChain = (p) => {
        ...
        for (let i = 0; i < segs.length; i++) {
            ...
            if (!set)
                childNames.set(parent, (set = new Set()));
            set.add(segs[i]);
        }
    };
    for (const p of scanResult.files.keys())
        addChain(p);
    for (const p of scanResult.ignored)
        addChain(p);
    for (const [notePath, note] of notes) {
        if (note.type !== 'dir' || !note.order?.length)
            continue;
    ...
    for (const [notePath, note] of notes) {
        ...
        while ((m = INLINE_CODE.exec(prose)) !== null) {
            const raw = m[1].trim();
            if (seen.has(raw) || !pathlike(raw))
                continue;
            seen.add(raw);
            const t = normalize(raw);
            if (!t || (!pathlike(t) && !raw.endsWith('/')))
                continue;
            if (searchBases(base).some((a) => exists(a ? a + '/' + t : t)))
                continue;
            if (!t.includes('/')) {
                if (basenames.has(t))
                    continue;
