...
import { AUDIT_LIMITS, normalizeAuditRepoPath, } from './audit-core.js';
...
function dataProperty(value, key, context) {
    ...
    if (descriptor === undefined ||
        ...
        throw new Error(`review coverage inventory ${context} must be an enumerable data property`);
    }
    return descriptor.value;
}
function snapshotInventoryTuples(input) {
    if (utilTypes.isProxy(input) ||
        !Array.isArray(input) ||
        Object.getPrototypeOf(input) !== Array.prototype) {
        throw new Error('review coverage inventory must be a plain dense array');
    }
    ...
    if (Object.getOwnPropertySymbols(input).length !== 0 ||
        ...
        throw new Error('review coverage inventory must be dense and have no extra properties');
    }
    const paths = new Set();
    const tuples = [];
    ...
    for (let index = 0; index < input.length; index += 1) {
        const tuple = dataProperty(input, String(index), `tuple ${index}`);
        if (tuple === null ||
            typeof tuple !== 'object' ||
            utilTypes.isProxy(tuple) ||
            Array.isArray(tuple) ||
            Object.getPrototypeOf(tuple) !== Object.prototype) {
            throw new Error(`review coverage inventory tuple ${index} must be a plain object`);
        }
        const keys = Reflect.ownKeys(tuple);
        if (keys.length !== 2 ||
            !keys.includes('marker') ||
            !keys.includes('path')) {
            throw new Error(`review coverage inventory tuple ${index} must contain exactly marker and path`);
        }
        ...
        if (typeof marker !== 'string' ||
            ...
            throw new Error(`review coverage inventory tuple ${index} marker must be a lowercase SHA-1 or GENERATED-PROOF`);
        }
        if (typeof repoPath !== 'string' ||
            repoPath.length > AUDIT_LIMITS.textCodeUnits ||
            hasLoneSurrogate(repoPath) ||
            repoPath.normalize('NFC') !== repoPath) {
            throw new Error(`review coverage inventory tuple ${index} path exceeds its bound or is invalid Unicode`);
        }
        let normalized;
        try {
            ...
            throw new Error(`review coverage inventory tuple ${index} path must be normalized and repository-relative`);
        }
        if ((normalized === COVERAGE_PATH) !==
            (marker === GENERATED_PROOF)) {
            throw new Error(`review coverage inventory tuple ${index} must reserve GENERATED-PROOF for ${COVERAGE_PATH}`);
        }
        if (paths.has(normalized)) {
            throw new Error(`review coverage inventory tuple ${index} repeats a path`);
        }
        ...
        if (totalCodeUnits > AUDIT_LIMITS.textTotalCodeUnits ||
            totalBytes > AUDIT_LIMITS.jsonBytes) {
            throw new Error('review coverage inventory exceeds its aggregate text bound');
        }
 ...
 */
export function reviewCoverageInventoryHash(input) {
    ...
    const tuples = snapshot.map((tuple) => ({
        marker: tuple.marker,
        path: tuple.path,
    })).sort((left, right) => compareText(left.marker, right.marker) ||
