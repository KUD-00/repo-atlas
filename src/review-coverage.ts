...
import {
  ...
  withAnchoredAuditFileIdentity,
  withAnchoredAuditRootIdentity,
} from './audit-core.js'
...
function unitOwnershipErrors(
  ...
  const domainsBySlug = new Map<string, Set<PortfolioDomain>>()
  ...
  for (const entry of entries) {
    ...
    for (const domain of Object.keys(entry.classification.domains) as PortfolioDomain[]) {
      ...
      if (!registered) {
        ...
        let crossDomain: PortfolioDomain | undefined
        ...
        if (crossDomain !== undefined) {
          errors.push(diagnostic(
            'unit-ownership',
            `review domain ${domain} names cross-domain unit ${unitSlug} registered under ${crossDomain}`,
...
function parseStructure(raw: unknown): StructuralParseResult {
  ...
  if (verdict === 'invalid' && reportErrors.length === 0) {
    return {
      ...
      errors: [diagnostic(
        'invalid-verdict',
        'invalid verdict requires at least one reportErrors item',
      )],
    }
  }
...
function inventoryHashFrom(hashes: Map<string, string>): string {
  return reviewCoverageInventoryHash(
    [...hashes.entries()].map(([repoPath, blob]) => ({
...
function readTrackedInventory(root: string): TrackedInventory | CoverageDiagnostic[] {
  ...
  if (inventory.objectFormat !== 'sha1') {
    return inventory.diagnostics.length > 0
      ...
      : [diagnostic(
          'git-error',
          'failed to read a supported tracked Git inventory',
        )]
  }
  const fatalDiagnostics = inventory.diagnostics.filter((entry) =>
    entry.code !== 'tracked-deletion')
  if (fatalDiagnostics.length > 0) {
    return fatalDiagnostics.map((entry) =>
      entry.code === 'unsafe-worktree-file'
  ...
  for (const file of inventory.files) {
    ...
    if (file.currentBlob === null || file.currentBlob.length !== 40) {
      return [diagnostic(
        'malformed-inventory',
        `tracked path has no readable SHA-1 worktree blob: ${file.path}`,
        { path: file.path },
      )]
    }
...
function policyClassificationErrors(
  report: ReviewCoverageReport,
  inventory: AuditTrackedInventory,
  policy: NonNullable<ReturnType<typeof loadAuditReviewPolicy>['policy']>,
): CoverageDiagnostic[] {
  ...
  const expectedByPath = new Map(classified.files.map((file) =>
  ...
  for (const entry of report.entries) {
    ...
    if (
      ...
      errors.push(diagnostic(
        ...
        `coverage classification does not match the current strict policy for ` +
  ...
  if (report.entries.length !== expectedByPath.size) {
    errors.push(diagnostic(
      'policy-classification-mismatch',
      'coverage classification path set does not match the current tracked inventory',
    ))
  }
  ...
  if (
    ...
    errors.push(diagnostic(
      'policy-classification-mismatch',
      'coverage unit identities and titles do not match the current strict policy',
    ))
  }
  return errors
}
...
function freshEvidenceErrors(
  ...
  acceptedRulesets: readonly string[] | null,
): CoverageDiagnostic[] {
  type ExactUnit = AuditExactEvidenceLoad['units'][number]
  interface IndexedExactUnit {
    unit: ExactUnit
    receiptsByPath: Map<string, ExactUnit['receipts'][number]>
  }
  const unitsByKey = new Map<string, IndexedExactUnit[]>()
  const domainsBySlug = new Map<string, Set<PortfolioDomain>>()
  for (const unit of exactEvidence.units) {
    const key = `${unit.domain}:${unit.slug}`
    const indexed: IndexedExactUnit = {
      unit,
      receiptsByPath: new Map(unit.receipts.map((receipt) =>
        [receipt.path, receipt] as const)),
    }
  ...
  for (const entry of report.entries) {
    ...
    for (const domain of Object.keys(entry.evidence) as PortfolioDomain[]) {
      ...
      if (
        ...
        errors.push(diagnostic(
          'wrong-unit-ledger',
          `fresh ${domain} evidence must name only the policy-assigned unit`,
          { path: entry.path, slug: assignedSlug },
        ))
        continue
      }
      ...
      for (const slug of claim.ledgers) {
        ...
        if (units.length !== 1) {
          errors.push(diagnostic(
            'ambiguous-ledger',
            `fresh ${domain} evidence names more than one ledger for ${slug}`,
            { path: entry.path, slug },
          ))
          continue
        }
      ...
      if (!matched) {
        errors.push(diagnostic(
          'fresh-evidence',
          `fresh ${domain} claim has no accepted current same-domain exact ` +
            'full-read receipt for the assigned unit and blob',
          { path: entry.path },
        ))
      }
    }
...
function revalidateAgainstRepository(
  ...
  if (report.policy.format !== 'atlas-review-policy-v1') {
    return invalidPortfolio([diagnostic(
      ...
      `coverage policy format ${JSON.stringify(report.policy.format)} is unsupported`,
    )])
  }
  ...
  if (policy.policyHash !== report.policy.hash) {
    return invalidPortfolio([diagnostic(
      'policy-drift',
      'coverage report policy hash does not match the current parsed review policy',
    )])
  }
  const classificationErrors = policyClassificationErrors(
    report,
    inventory.inventory,
    policy.policy,
  )
  ...
  if (exactEvidence.invalidClaimedPaths.length > 0) {
    return invalidPortfolio(exactEvidence.invalidClaimedPaths.map((claim) =>
      diagnostic(
        'invalid-evidence-path',
        `invalid audit ledger claimed path ${JSON.stringify(claim.path)}`,
        { slug: claim.slug ?? undefined },
      )))
  }
  for (const unit of exactEvidence.units) {
    if (unit.invalidClaimedPaths.length > 0) {
      return invalidPortfolio(unit.invalidClaimedPaths.map((repoPath) =>
        diagnostic(
          'invalid-evidence-path',
          `audit ledger contains invalid claimed path ${JSON.stringify(repoPath)}`,
          { slug: unit.slug },
        )))
    }
  }

  const evidenceErrors = freshEvidenceErrors(
    report,
    inventory.hashes,
    exactEvidence,
    policy.policy.securityDecisions.acceptedRulesets,
  )
...
function loadReviewCoverageAnchored(
  root: string,
  portfolios: AuditPortfolios,
): ReviewCoveragePortfolio {
  try {
    return withAnchoredAuditFileIdentity(
      root,
      COVERAGE_REL,
      () => {
        let raw: unknown
        try {
          raw = readBoundedAuditJsonDocument(
            root,
            COVERAGE_REL,
            MAX_REPORT_BYTES,
          ).value
          ...
          const code = /exceeds the \d+-byte limit/iu.test(message)
            ? 'report-too-large'
            : /missing|safe regular|symlink|outside|changed while/iu.test(
                message,
              )
              ? 'unsafe-path'
              : 'malformed-json'
          return invalidPortfolio([diagnostic(
            code,
            `coverage report could not be parsed: ${message}`,
            { path: COVERAGE_REL },
          )])
        }
        ...
        // retained.
        return revalidateAgainstRepository(
          root,
          parsed.report,
          portfolios,
        )
    ...
    return invalidPortfolio([diagnostic(
      code,
      `coverage report identity could not be retained: ${message}`,
      { path: COVERAGE_REL },
    )])
  }
}
...
export function loadReviewCoverage(
  ...
  try {
    return withAnchoredAuditRootIdentity(
      root,
      (anchoredRoot) => loadReviewCoverageAnchored(
        anchoredRoot,
        portfolios,
      ),
    )
    ...
    return invalidPortfolio([diagnostic(
      'root-identity',
      `coverage reader could not retain one repository root identity: ${
