...
const escapeReg = (s: string): string => s.replace(/[.*+?^${}()|[\]\\]/gu, '\\$&')
 ...
 */
export function fillGlossaryRefs(
  glossary: GlossaryEntry[],
  notes: Iterable<{ path: string; body?: string | null }>,
): void {
  if (!glossary.length) return
  const names: [string, GlossaryEntry][] = []
  for (const g of glossary) for (const t of [g.term, ...g.aliases]) if (t) names.push([t, g])
  ...
  const isWord = (ch: string | undefined) => !!ch && /[A-Za-z0-9_]/u.test(ch)

  const refs = new Map<GlossaryEntry, Set<string>>()
