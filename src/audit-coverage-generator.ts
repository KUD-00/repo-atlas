...
import type {
  ...
  AuditExactEvidenceReceipt,
  AuditExactEvidenceUnit,
  AuditInventoryClassification,
  AuditReviewPolicyV1,
  AuditTrackedFile,
  AuditTrackedInventory,
  CoverageDiagnostic,
  CoverageEntry,
...
const DOMAINS: readonly PortfolioDomain[] = ['security', 'test']
type Picomatch = (
  glob: string,
  options?: { dot?: boolean },
...
function createHistoricalMatcher(
  pattern: string,
): (candidate: string) => boolean {
...
function sortDiagnostics(
  values: readonly CoverageDiagnostic[],
): CoverageDiagnostic[] {
...
function inventoryHash(files: readonly AuditTrackedFile[]): string {
...
function emptySummary(): ReviewCoverageSummary {
  return {
    tracked: 0,
    securityRequired: 0,
    securityFresh: 0,
...
function summarize(
  ...
  invalidLedgers: readonly CoverageDiagnostic[],
): ReviewCoverageSummary {
  ...
  for (const entry of entries) {
    ...
    if (security && test) summary.dualRequired += 1
    for (const domain of DOMAINS) {
      if (entry.classification.domains[domain] === undefined) continue
      const status = entry.evidence[domain]?.status
      if (domain === 'security') {
        summary.securityRequired += 1
        if (status === 'fresh') summary.securityFresh += 1
        else if (status === 'stale') summary.securityStale += 1
        else if (status === 'invalid') summary.securityInvalid += 1
        else summary.securityMissing += 1
      } else {
        summary.testRequired += 1
        if (status === 'fresh') summary.testFresh += 1
        else if (status === 'stale') summary.testStale += 1
        else if (status === 'invalid') summary.testInvalid += 1
        else summary.testMissing += 1
      }
    }
  }
  return summary
...
function hasCoverageGap(summary: ReviewCoverageSummary): boolean {
  return (
...
function evidenceKey(domain: PortfolioDomain, slug: string): string {
  return `${domain}:${slug}`
}

function validateEvidenceUnit(
  unit: AuditExactEvidenceUnit,
): CoverageDiagnostic[] {
  ...
  for (const receipt of unit.receipts) {
    try {
      ...
      errors.push(diagnostic(
        ...
        `exact evidence ${unit.domain}:${unit.slug} claims an invalid path`,
        { slug: unit.slug },
      ))
      continue
    }
    if (paths.has(receipt.path)) {
      errors.push(diagnostic(
        ...
        `exact evidence ${unit.domain}:${unit.slug} claims ${receipt.path} more than once`,
        { path: receipt.path, slug: unit.slug },
      ))
    }
    ...
    if (
      ...
      errors.push(diagnostic(
        'invalid-evidence-blob',
        `exact evidence has an invalid Git blob for ${receipt.path}`,
        { path: receipt.path, slug: unit.slug },
      ))
    }
    if (receipt.fullRead && (!receipt.reviewed || receipt.blob === null)) {
      errors.push(diagnostic(
        'inconsistent-full-read',
        `full-read evidence requires a reviewed exact blob for ${receipt.path}`,
        { path: receipt.path, slug: unit.slug },
      ))
    }
    if (unit.version === 1 && receipt.fullRead) {
      errors.push(diagnostic(
        'forged-v1-full-read',
        `V1 ledger ${unit.slug} cannot claim schema-owned full-read evidence`,
        { path: receipt.path, slug: unit.slug },
      ))
    }
  }
  for (const claimedPath of unit.invalidClaimedPaths) {
    errors.push(diagnostic(
      'invalid-evidence-path',
      `ledger ${unit.slug} contains invalid claimed path ${JSON.stringify(claimedPath)}`,
      { slug: unit.slug },
    ))
  }
  return errors
...
interface IndexedEvidenceUnit {
  unit: AuditExactEvidenceUnit
  receiptsByPath: ReadonlyMap<string, AuditExactEvidenceReceipt>
}

function joinEvidence(
  file: AuditTrackedFile,
  domain: PortfolioDomain,
  unitSlug: string,
  unitsByKey: ReadonlyMap<string, IndexedEvidenceUnit[]>,
  policy: AuditReviewPolicyV1,
  fatalErrors: CoverageDiagnostic[],
): { status: CoverageEvidenceStatus; ledgers: string[] } {
  const units = unitsByKey.get(evidenceKey(domain, unitSlug)) ?? []
  if (units.length > 1) {
    fatalErrors.push(diagnostic(
      'ambiguous-evidence-unit',
      `multiple exact-evidence ledgers claim assigned unit ${domain}:${unitSlug}`,
      { path: file.path, slug: unitSlug },
    ))
...
function normalizedUnits(policy: AuditReviewPolicyV1): CoverageUnitRef[] {
...
export interface AuditCoverageInput {
  policy: AuditReviewPolicyV1
  policyHash: string
  inventory: AuditTrackedInventory
  classification: AuditInventoryClassification
  /** `exactEvidence` is the canonical name; `evidence` is retained for callers
   * that constructed the Task 4 input before the seam was named. */
  exactEvidence?: AuditExactEvidenceLoad
  evidence?: AuditExactEvidenceLoad
}

function snapshotCoveragePublicData(
  value: unknown,
  pointer = '/input',
  depth = 0,
  budget: { items: number; text: number } = { items: 0, text: 0 },
): unknown {
  if (depth > 128) {
  ...
  if (Array.isArray(value)) {
    ...
    for (let index = 0; index < lengthDescriptor.value; index += 1) {
      ...
      output.push(snapshotCoveragePublicData(
        descriptor.value,
        `${pointer}/${index}`,
        depth + 1,
        budget,
  ...
  for (const key of ownKeys as string[]) {
    ...
    if (
      key.length > AUDIT_LIMITS.textCodeUnits ||
      /[\uD800-\uDFFF]/u.test(key)
    ) {
    ...
    output[key] = snapshotCoveragePublicData(
      descriptor.value,
      `${pointer}/${key}`,
      depth + 1,
      budget,
...
function invalidCoverageInput(pointer: string, message: string): never {
...
function coverageArray(value: unknown, pointer: string): unknown[] {
  if (!Array.isArray(value)) {
...
function coverageText(
  value: unknown,
  pointer: string,
  options: { routeSlug?: boolean; ruleset?: boolean } = {},
): string {
  if (
    typeof value !== 'string' ||
    value.length === 0 ||
...
function coverageRepoPath(value: unknown, pointer: string): string {
  if (typeof value !== 'string') {
...
function parseCoverageDiagnostics(
  value: unknown,
  pointer: string,
): CoverageDiagnostic[] {
  return coverageArray(value, pointer).map((item, index) =>
    parseCoverageDiagnostic(item, `${pointer}/${index}`))
}

function parseExactEvidenceReceipt(
  value: unknown,
  pointer: string,
  version: 1 | 2 | 3,
): AuditExactEvidenceUnit['receipts'][number] {
  const row = coverageRecord(value, pointer)
  exactCoverageKeys(
    row,
    ['path', 'blob', 'reviewed', 'fullRead'],
    [],
    pointer,
  )
  ...
  if (row.blob === null) {
    ...
    invalidCoverageInput(
      `${pointer}/blob`,
      `has an invalid V${version} exact Git object shape`,
    )
  }
  if (
    version === 1 &&
...
function parseExactEvidenceUnit(
  value: unknown,
  pointer: string,
): AuditExactEvidenceUnit {
  const row = coverageRecord(value, pointer)
  ...
  const slug = coverageText(
    row.slug,
    `${pointer}/slug`,
    { routeSlug: true },
  )
  let ruleset: string | null
  if (row.ruleset === null) ruleset = null
  else {
    ruleset = coverageText(
      row.ruleset,
      `${pointer}/ruleset`,
      { ruleset: true },
    )
  }
  let rulesetDigest: string | null
  if (row.rulesetDigest === null) rulesetDigest = null
  else if (
    typeof row.rulesetDigest === 'string' &&
    SHA256_RE.test(row.rulesetDigest)
  ) {
    rulesetDigest = row.rulesetDigest
  } else {
    invalidCoverageInput(
      `${pointer}/rulesetDigest`,
      'must be null or a sha256 digest',
    )
  }
  if (
    (ruleset === null) !== (rulesetDigest === null && row.version === 3) &&
    row.version === 3
  ) {
    invalidCoverageInput(
      pointer,
      'V3 ruleset and rulesetDigest must both be null or both be present',
    )
  }
  ...
  if (
    row.semanticStatus !== 'covered' &&
    row.semanticStatus !== 'unknown' &&
    row.semanticStatus !== 'gap'
  ) {
    invalidCoverageInput(
      `${pointer}/semanticStatus`,
      'must be covered, unknown, or gap',
    )
  }
  ...
  const receipts = coverageArray(row.receipts, `${pointer}/receipts`)
    .map((receipt, index) => parseExactEvidenceReceipt(
      receipt,
      `${pointer}/receipts/${index}`,
      version,
    ))
  ...
  const invalidClaimedPaths = coverageArray(
    row.invalidClaimedPaths,
    `${pointer}/invalidClaimedPaths`,
  ).map((candidate, index) =>
    coverageText(
      candidate,
      `${pointer}/invalidClaimedPaths/${index}`,
    ))
...
function parseExactEvidenceLoad(
  value: unknown,
  pointer: string,
): AuditExactEvidenceLoad {
  const row = coverageRecord(value, pointer)
  exactCoverageKeys(
    row,
    ['units', 'invalidLedgers', 'invalidClaimedPaths'],
    [],
    pointer,
  )
  const units = coverageArray(row.units, `${pointer}/units`)
    .map((unit, index) =>
      parseExactEvidenceUnit(unit, `${pointer}/units/${index}`))
  ...
  const invalidLedgers = parseCoverageDiagnostics(
    row.invalidLedgers,
    `${pointer}/invalidLedgers`,
  )
  const invalidClaimedPaths = coverageArray(
    row.invalidClaimedPaths,
    `${pointer}/invalidClaimedPaths`,
    ...
    const domain = claim.domain as PortfolioDomain | null
    let slug: string | null
    if (claim.slug === null) slug = null
    else {
      slug = coverageText(
        claim.slug,
        `${claimPointer}/slug`,
...
interface NormalizedCoverageInputResult {
  input: AuditCoverageInput | null
  safeInventoryFiles: AuditTrackedFile[]
  error: CoverageDiagnostic | null
}

function normalizedCoverageInput(
  unsafeInput: AuditCoverageInput,
): NormalizedCoverageInputResult {
  ...
  if (
    ...
    return {
      ...
      error: diagnostic(
        'invalid-coverage-input',
        'coverage input must be a plain object',
      ),
    }
  }
  const row = snapshot as Record<string, unknown>
  let safeInventoryFiles: AuditTrackedFile[] = []
  try {
    ...
    if (
      ...
      invalidCoverageInput(
        '/input',
        'must contain exactly one of exactEvidence or evidence',
      )
    }
    exactCoverageKeys(
      row,
      ['policy', 'policyHash', 'inventory', 'classification', evidenceField],
      [],
      '/input',
    )
    ...
    exactCoverageKeys(
      inventoryRow,
      ['objectFormat', 'files', 'diagnostics'],
      [],
      '/input/inventory',
    )
    if (
      inventoryRow.objectFormat !== 'sha1' &&
      inventoryRow.objectFormat !== 'sha256' &&
      inventoryRow.objectFormat !== null
    ) {
      invalidCoverageInput(
        '/input/inventory/objectFormat',
        'must be sha1, sha256, or null',
      )
    }
    const inventoryFiles = coverageArray(
      inventoryRow.files,
      '/input/inventory/files',
    )
    const inventoryProbe = classifyAuditInventory(
      inventoryFiles as AuditTrackedFile[],
      null,
    )
    const invalidInventoryDiagnostics = inventoryProbe.diagnostics.filter(
      (entry) =>
        entry.code !== 'missing-review-policy' &&
        entry.code !== 'tracked-deletion',
    )
    if (invalidInventoryDiagnostics.length > 0) {
      invalidCoverageInput(
        '/input/inventory/files',
        invalidInventoryDiagnostics[0].message,
      )
    }
    ...
    if (
      ...
      safeInventoryFiles.some((file) =>
        file.indexBlob.length !== expectedBlobLength ||
        (
          file.currentBlob !== null &&
      ...
      invalidCoverageInput(
        '/input/inventory/files',
        'Git object IDs do not match inventory objectFormat',
      )
    }
    const inventoryDiagnostics = parseCoverageDiagnostics(
      inventoryRow.diagnostics,
      '/input/inventory/diagnostics',
    )
    const classificationRow = coverageRecord(
      row.classification,
    ...
    const classificationDiagnostics = parseCoverageDiagnostics(
      classificationRow.diagnostics,
      '/input/classification/diagnostics',
    )
    const exactEvidence = parseExactEvidenceLoad(
      row[evidenceField],
      `/input/${evidenceField}`,
    )
    const normalized: AuditCoverageInput = {
      policy: row.policy as AuditReviewPolicyV1,
      ...
      classification: {
        files: classificationRow.files as AuditInventoryClassification['files'],
...
export function buildAuditCoverageReport(
  unsafeInput: AuditCoverageInput,
): ReviewCoverageReport {
  const normalizedInput = normalizedCoverageInput(unsafeInput)
  if (normalizedInput.input === null) {
    return placeholderInvalidReport(
      normalizedInput.safeInventoryFiles,
      [normalizedInput.error ?? diagnostic(
        'invalid-coverage-input',
  ...
  if (input.inventory.objectFormat !== 'sha1') {
    const objectFormatError = input.inventory.objectFormat === 'sha256'
      ? diagnostic(
          ...
          'atlas-review-coverage-v1 supports only SHA-1 Git repositories',
        )
      : diagnostic(
          'invalid-object-format',
          'coverage inventory is missing a supported Git object format',
        )
    return placeholderInvalidReport(
      [],
      [...input.inventory.diagnostics, objectFormatError],
    )
  }
  ...
  try {
    ...
    return placeholderInvalidReport(input.inventory.files, [diagnostic(
      'invalid-public-policy',
      `coverage policy input is invalid: ${message}`,
    )])
  }
  const policy = normalizedPolicy.policy
  const trustedClassification = classifyAuditInventory(
    input.inventory.files,
    policy,
  )
  ...
  const fatalErrors: CoverageDiagnostic[] = [
    ...input.inventory.diagnostics,
    ...trustedClassification.diagnostics,
  ]
  if (
    !/^[0-9a-f]{64}$/.test(input.policyHash) ||
    input.policyHash !== normalizedPolicy.policyHash
  ) {
    fatalErrors.push(diagnostic(
      'invalid-policy-hash',
      'review policy hash must equal the canonical parsed policy SHA-256',
    ))
  }
  try {
    if (
      canonicalJson(input.classification.files) !==
        canonicalJson(trustedClassification.files)
    ) {
      fatalErrors.push(diagnostic(
        'forged-classification',
        'supplied classification differs from strict policy recomputation',
      ))
    }
  } catch {
    fatalErrors.push(diagnostic(
      'forged-classification',
      'supplied classification is not plain canonical data',
    ))
  }

  const unitsByKey = new Map<string, IndexedEvidenceUnit[]>()
  for (const unit of exactEvidence.units) {
    fatalErrors.push(...validateEvidenceUnit(unit))
    const key = evidenceKey(unit.domain, unit.slug)
    const indexed: IndexedEvidenceUnit = {
      unit,
      receiptsByPath: new Map(
        unit.receipts.map((receipt) => [receipt.path, receipt]),
      ),
    }
    const existing = unitsByKey.get(key)
    if (existing === undefined) unitsByKey.set(key, [indexed])
    else existing.push(indexed)
  }
  for (const claim of exactEvidence.invalidClaimedPaths ?? []) {
    fatalErrors.push(diagnostic(
      'invalid-evidence-path',
      `invalid audit ledger ${claim.sourcePath} claimed path ` +
  ...
  const historicalGlobCount = policy.historicalUnitAssignments.reduce(
    (count, assignment) => count + assignment.include.length,
    0,
  )
  const receiptCount = exactEvidence.units.reduce(
    (count, unit) => count + unit.receipts.length,
    0,
  )
  const historicalReceiptMatchOperations =
    BigInt(historicalGlobCount) * BigInt(receiptCount)
  const historicalReceiptResourceLimited =
    historicalReceiptMatchOperations > AUDIT_MATCH_OPERATION_LIMIT
  if (historicalReceiptResourceLimited) {
    fatalErrors.push(diagnostic(
      'historical-receipt-resource-limit',
      `historical receipt overlap requires ` +
        `${historicalReceiptMatchOperations.toString()} worst-case match ` +
        `operations; limit is ${AUDIT_MATCH_OPERATION_LIMIT.toString()}`,
    ))
  } else {
    const compiledHistoricalAssignments =
      policy.historicalUnitAssignments.map((assignment) => ({
        assignment,
        // Patterns were normalized by the strict policy parser.
        matches: assignment.include.map((pattern) =>
          createHistoricalMatcher(pattern)),
      }))
    for (const candidate of compiledHistoricalAssignments) {
      for (const unit of exactEvidence.units) {
        for (const receipt of unit.receipts) {
          ...
          fatalErrors.push(diagnostic(
            'historical-active-receipt-overlap',
            `historical assignment ${candidate.assignment.id} matches active exact ` +
              `receipt ${JSON.stringify(receipt.path)}`,
            { path: receipt.path, slug: unit.slug },
          ))
        }
      }
  ...
  const classifiedByPath = new Map(
    trustedClassification.files.map((file) => [file.path, file]),
  )
  ...
  for (const file of [...trustedClassification.files]
    ...
    if (file.deleted || file.currentBlob === null) {
      fatalErrors.push(diagnostic(
        'tracked-deletion',
        `tracked path is deleted from the worktree: ${JSON.stringify(file.path)}`,
        { path: file.path },
      ))
    }
    if (file.path === COVERAGE_PATH) {
      ...
      if (
        ...
        fatalErrors.push(diagnostic(
          'invalid-generated-proof',
          'tracked .atlas/review-coverage.json requires the exact reserved ' +
            'generated-proof policy classification',
          { path: COVERAGE_PATH },
        ))
        entries.push({
          path: file.path,
    ...
    if (classified === undefined) {
      fatalErrors.push(diagnostic(
        ...
        `tracked path has no classification result: ${JSON.stringify(file.path)}`,
        { path: file.path },
      ))
      entries.push({
        path: file.path,
    ...
    const evidence: CoverageEntry['evidence'] = {}
  ...
  for (const entry of entries) {
    if (
      entry.classification.kind === 'unclassified' ||
      entry.classification.kind === 'conflict'
    ) {
      fatalErrors.push(diagnostic(
        'invalid-classification',
        `tracked path is ${entry.classification.kind}: ${JSON.stringify(entry.path)}`,
        { path: entry.path },
      ))
    }
    if (
      entry.classification.kind !== 'review' &&
      Object.keys(entry.evidence).length > 0
    ) {
      fatalErrors.push(diagnostic(
        'inconsistent-evidence-join',
        `non-review path carries domain evidence: ${JSON.stringify(entry.path)}`,
        { path: entry.path },
      ))
    }
  }

  const invalidLedgerDetails = uniqueDiagnostics(
    exactEvidence.invalidLedgers,
  )
  if (invalidLedgerDetails.length > 0) {
    fatalErrors.push(diagnostic(
      'invalid-audit-ledgers',
      `${invalidLedgerDetails.length} invalid audit ledger diagnostic(s) ` +
...
export interface AuditCoverageRulesetAssurance {
  domain: PortfolioDomain
  slug: string
  ruleset: string | null
...
export interface AuditCoverageSemanticAssurance {
  domain: PortfolioDomain
  slug: string
  status: 'covered' | 'unknown' | 'gap'
}

export interface AuditCoverageLifecycleFinding {
  findingId: string
  disposition: string
  blocking: boolean
  lifecycle: 'new' | 'persisting' | 'resolved' | 'reopened' | 'unknown'
  expiryState: 'not-applicable' | 'active' | 'warning' | 'expired'
  /** Why the state is what it is — `carry-invalidated` means a decision exists but no longer applies. */
  derivation:
    | 'explicit-event'
    | 'carried'
    | 'carry-invalidated'
...
export interface AuditCoverageLifecycleAssurance {
  findings: AuditCoverageLifecycleFinding[]
  diagnostics: CoverageDiagnostic[]
}

export interface AuditCoverageRuntimeAssurance {
  semantic: AuditCoverageSemanticAssurance[]
  rulesets: AuditCoverageRulesetAssurance[]
  lifecycle: AuditCoverageLifecycleAssurance
}
...
export interface AuditCoverageResult {
  ...
  diagnostics: CoverageDiagnostic[]
  runtimeAssurance: AuditCoverageRuntimeAssurance
}
 ...
 */
function blockingFindingDiagnostics(
  assurance: AuditCoverageRuntimeAssurance,
): CoverageDiagnostic[] {
  const byDerivation = new Map<string, number>()
  ...
  return [...byDerivation.entries()]
    ...
    .map(([derivation, count]) =>
      diagnostic(
        'blocking-findings',
        `${String(count)} finding(s) block coverage with derivation "${derivation}"` +
...
function lifecycleAssurance(
  root: string,
  policy: AuditReviewPolicyV1,
): AuditCoverageLifecycleAssurance {
  ...
  try {
    ...
    return {
      ...
      diagnostics: [diagnostic(
        'audit-lifecycle-invalid',
        `audit lifecycle assurance could not be reduced: ${
          error instanceof Error ? error.message : String(error)
        }`,
      )],
    }
...
function runtimeAssurance(
  root: string,
  policy: AuditReviewPolicyV1,
  evidence: AuditExactEvidenceLoad,
): AuditCoverageRuntimeAssurance {
  const units = [...evidence.units].sort((left, right) =>
    compareText(left.domain, right.domain) ||
    compareText(left.slug, right.slug) ||
...
function gapDiagnostics(report: ReviewCoverageReport): CoverageDiagnostic[] {
  const errors: CoverageDiagnostic[] = []
  for (const entry of report.entries) {
    ...
    for (const domain of DOMAINS) {
      ...
      if (status !== undefined && status !== 'fresh') {
        errors.push(diagnostic(
          'coverage-gap',
          `${domain} exact evidence is ${status} for ${JSON.stringify(entry.path)}`,
          { path: entry.path },
        ))
      }
    }
...
function canonicalCoverageBytes(report: ReviewCoverageReport): string {
  return `${canonicalJson(report)}\n`
}

function placeholderInvalidReport(
  files: readonly AuditTrackedFile[],
  errors: readonly CoverageDiagnostic[],
): ReviewCoverageReport {
  const representableFiles = files.filter((file) =>
    file.path === COVERAGE_PATH ||
    /^[0-9a-f]{40}$/u.test(file.currentBlob ?? file.indexBlob))
  const entries: CoverageEntry[] = [...representableFiles]
    .sort((left, right) => compareText(left.path, right.path))
...
interface PreparedCoverage {
  report: ReviewCoverageReport
  bytes: string
  assurance: AuditCoverageRuntimeAssurance
  diagnostics: CoverageDiagnostic[]
  writeForbiddenObjectFormat: boolean
}

interface PreparedCoverageSnapshot {
...
type PreparedCoverageSnapshotResult =
  | { ok: true; value: PreparedCoverageSnapshot }
  | {
      ok: false
      prepared: PreparedCoverage | null
      error: unknown
    }

function prepareCoverage(root: string): PreparedCoverage {
  ...
  if (inventory.objectFormat !== 'sha1') {
    ...
    const report = placeholderInvalidReport(
      [],
      [...inventory.diagnostics, objectFormatError],
    )
    return {
      report,
  ...
  if (policyLoad.policy === null || policyLoad.policyHash === null) {
    const report = placeholderInvalidReport(
      inventory.files,
      [...policyLoad.diagnostics, ...inventory.diagnostics],
    )
    return {
      report,
      bytes: canonicalCoverageBytes(report),
...
function prepareCoverageSnapshot(
  root: string,
): PreparedCoverageSnapshotResult {
  let prepared: PreparedCoverage | null = null
  try {
...
function snapshotFailureResult(
  ...
  const snapshotError = diagnostic(
    'coverage-snapshot-invalid',
    `coverage inputs changed or could not be retained as one snapshot: ${
      error instanceof Error ? error.message : String(error)
    } — a build or generator writing during the read is the usual cause; re-run ` +
...
function readCommittedCoverageBytes(root: string): {
  bytes: string | null
  error: CoverageDiagnostic | null
} {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
      return {
        ...
        error: diagnostic(
          ...
          'committed coverage report is missing',
          { path: COVERAGE_PATH },
        ),
      }
    }
    return {
      ...
      error: diagnostic(
        ...
        'committed coverage report is unreadable',
        { path: COVERAGE_PATH },
      ),
    }
  }
  if (!stat.isFile() || stat.isSymbolicLink()) {
    return {
      bytes: null,
      error: diagnostic(
        'coverage-report-unsafe',
        'committed coverage report is not a safe regular file',
        { path: COVERAGE_PATH },
      ),
    }
  }
  try {
    return {
      bytes: Buffer.from(
        readBoundedAuditBytes(root, COVERAGE_PATH),
      ).toString('utf8'),
      error: null,
    }
  } catch (error) {
    return {
      ...
      error: diagnostic(
        'coverage-report-unreadable',
        `committed coverage report could not be read safely: ${
          error instanceof Error ? error.message : String(error)
        }`,
        { path: COVERAGE_PATH },
      ),
...
function runtimeAssuranceAllows(
  assurance: AuditCoverageRuntimeAssurance,
): boolean {
  return (
    assurance.lifecycle.diagnostics.length === 0 &&
    !assurance.lifecycle.findings.some((finding) => finding.blocking)
  )
}

function updateAuditCoverageUnlocked(
  root: string,
  ...
  while (true) {
    ...
    if (writeCount >= MAX_COVERAGE_UPDATE_WRITES) {
      const convergenceError = diagnostic(
        'coverage-update-did-not-converge',
        `coverage inputs changed across ${MAX_COVERAGE_UPDATE_WRITES} consecutive atomic writes`,
        { path: COVERAGE_PATH },
      )
      return {
        ok: false,
    ...
    try {
      ...
      const writeError = diagnostic(
        'coverage-write-failed',
        `coverage report could not be replaced atomically: ${
          error instanceof Error ? error.message : String(error)
        }`,
        { path: COVERAGE_PATH },
      )
...
export function updateAuditCoverage(
  root: string,
  options: { allowIncomplete?: boolean } = {},
): AuditCoverageResult {
  return withAuditLock(
    root,
    () => updateAuditCoverageUnlocked(
      anchoredAuditRootPath(root),
      options,
    ),
  )
}

function checkAuditCoverageUnlocked(
  root: string,
  options: { allowIncomplete?: boolean } = {},
): AuditCoverageResult {
  const snapshot = prepareCoverageSnapshot(root)
  ...
  const { prepared, existing } = snapshot.value
  const current =
    !prepared.writeForbiddenObjectFormat &&
    existing.bytes === prepared.bytes
  const byteErrors: CoverageDiagnostic[] = []
  if (!prepared.writeForbiddenObjectFormat && !current) {
    if (existing.error !== null) byteErrors.push(existing.error)
    else {
      byteErrors.push(diagnostic(
        'coverage-byte-drift',
        'committed coverage bytes are not the exact canonical generated bytes — ' +
          'run `repo-atlas audit coverage update` and commit the result',
        { path: COVERAGE_PATH },
      ))
    }
  }
