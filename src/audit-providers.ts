...
export interface AuditProviderInvocationRequest {
  ...
  /**
   * Carry a file's receipt forward from the unit's published observation when
   * the file's blob is unchanged and every verdict-shaping input still matches
   * (see planCarriedReceipts). Off by default, like `--resume`: reuse must be
   * asked for, so "review this unit again" always means exactly that.
   */
  reuseUnchangedReceipts?: boolean
}
...
export interface AuditProviderContext {
  ...
  /**
   * Published observations this run may carry receipts from. Present only when
   * the invocation asked for reuse; absent means every file is reviewed again.
   */
  priorObservations?: readonly AuditProviderPriorObservation[]
}
...
export interface AuditProviderResult {
  ...
  /**
   * Files whose receipt was carried forward from a previously published
   * observation instead of being reviewed again. Empty unless the invocation
   * asked for reuse. Publication needs this to attribute the receipt honestly:
   * a carried receipt was never proven by THIS run's transcript.
   */
  carriedReceipts: AuditProviderCarriedReceipt[]
  /**
   * Which review chunk actually reviewed each freshly reviewed path. Batch
   * membership is chosen by the placement heuristic, not by position in the
   * inventory, and carrying receipts forward removes files from batching
   * entirely — so a positional guess names a batch that never held the file.
   */
  reviewUnitByPath: Record<string, string>
  /** Which verification chunk dispositioned each fresh candidate. */
  verificationUnitByFingerprint: Record<string, string>
}
...
export interface AuditProviderSynthesisInput {
  ...
  candidates: AuditProviderCandidateFinding[]
  /**
   * Receipts proven by an earlier observation and carried into this one. These
   * files were NOT reviewed this run: synthesis must emit their outcome exactly
   * as carried, and their findings exactly as carried, or the observation would
   * report a file as clean that nothing re-examined.
   */
  carried: AuditProviderCarryPlan
}
...
function validateInvocationRequest(
  ...
  if (
    request.reuseUnchangedReceipts !== undefined &&
...
/** A finding as a published observation recorded it, on one file. */
export interface AuditProviderPriorFinding {
  /** The producer-side candidate id, republished as `identity.anchor`. */
  fingerprint: string
  occurrenceId: string
  ruleId: string
  title: string
...
/** A file receipt as a published observation recorded it. */
export interface AuditProviderPriorFileReceipt {
  path: string
  blob: AuditGitBlob
  lines: number
  outcome: 'clean' | 'findings'
...
export interface AuditProviderPriorObservation {
  slug: string
  observationId: string
  /** Everything that could change a verdict for identical bytes. */
  rulesetDigest: AuditSha256
  effectiveConfigDigest: AuditSha256
  environmentPolicyDigest: AuditSha256
  producerVersion: string
  receipts: ReadonlyMap<string, AuditProviderPriorFileReceipt>
  findingsByPath: ReadonlyMap<string, readonly AuditProviderPriorFinding[]>
}

/** A receipt this run publishes without having re-read the file. */
export interface AuditProviderCarriedReceipt {
  path: string
  blob: AuditGitBlob
  lines: number
  outcome: 'clean' | 'findings'
  /** The observation that proved this receipt, and the unit that holds it. */
  observationId: string
  slug: string
  reviewedAt?: string
  reviewedAtPrecision?: 'timestamp' | 'date'
...
export interface AuditProviderCarryPlan {
  receipts: ReadonlyMap<string, AuditProviderCarriedReceipt>
  findings: readonly AuditProviderFinding[]
}

const EMPTY_CARRY_PLAN: AuditProviderCarryPlan = {
  receipts: new Map(),
  findings: [],
}

interface AuditProviderCarryKey {
  rulesetDigest: AuditSha256
  effectiveConfigDigest: AuditSha256
  environmentPolicyDigest: AuditSha256
  producerVersion: string
...
function readPriorFinding(
  value: unknown,
): { path: string; finding: AuditProviderPriorFinding } | null {
  if (!isPlainObject(value)) return null
  ...
  if (locations.length !== 1 || !isPlainObject(locations[0])) return null
  const location = locations[0]
  const anchor =
    isPlainObject(identity) ? optionalPlainString(identity.anchor) : undefined
  ...
  if (
    ...
    !SEVERITIES.includes(severity as AuditSeverity) ||
    !CONFIDENCES.includes(confidence as AuditConfidence) ||
    !DISPOSITIONS.includes(validation.disposition as AuditProviderDisposition)
  ) {
    return null
  }
  ...
  return {
    ...
    finding: {
      ...
      detail,
      fix,
      disposition: validation.disposition as AuditProviderDisposition,
      dispositionRationale: rationale,
    },
  }
}

function readPriorFileReceipt(value: unknown): AuditProviderPriorFileReceipt | null {
  if (!isPlainObject(value)) return null
  ...
  if (
    (reviewedAt === undefined) !== (precision === undefined) ||
  ...
  return {
    path: repoPath,
    blob: blob as AuditGitBlob,
    lines: value.lines,
    outcome: value.outcome,
    ...(reviewedAt !== undefined
      ? { reviewedAt, reviewedAtPrecision: precision as 'timestamp' | 'date' }
      : {}),
    ...
    ...(optionalPlainString(value.ruleset) !== undefined
      ? { ruleset: value.ruleset as string }
      : {}),
    receiptRefs: [...(value.receiptRefs as string[])],
    findingOccurrenceIds: [...(value.findingOccurrenceIds as string[])],
  }
}

function readPriorObservation(
  slug: string,
  ledger: unknown,
): AuditProviderPriorObservation | null {
  ...
  const receipts = new Map<string, AuditProviderPriorFileReceipt>()
  for (const raw of scope.files) {
    ...
    if (receipt === null || receipts.has(receipt.path)) return null
    receipts.set(receipt.path, receipt)
  }
  const findingsByPath = new Map<string, AuditProviderPriorFinding[]>()
  ...
  return {
    slug,
    observationId,
    rulesetDigest: rulesetDigest as AuditSha256,
    effectiveConfigDigest: effectiveConfigDigest as AuditSha256,
    environmentPolicyDigest: environmentPolicyDigest as AuditSha256,
    producerVersion,
    receipts,
    findingsByPath,
  }
 ...
 * again".
 */
export function readAuditProviderPriorObservations(
  repoRoot: string,
): AuditProviderPriorObservation[] {
  let names: string[]
  try {
    names = fs
  ...
  const observations: AuditProviderPriorObservation[] = []
  for (const name of names) {
    ...
    try {
      document = readBoundedAuditJsonDocument(
        repoRoot,
        `${AUDIT_LEDGER_DIR}/${name}`,
        AUDIT_LEDGER_BYTE_LIMIT,
      )
    } catch {
      continue
    }
 ...
 */
function planCarriedReceipts(
  context: AuditProviderContext,
  key: AuditProviderCarryKey,
): AuditProviderCarryPlan {
  const priors = context.priorObservations
  if (priors === undefined || priors.length === 0) return EMPTY_CARRY_PLAN
  const usable = priors.filter(
    (prior) =>
      prior.rulesetDigest === key.rulesetDigest &&
  ...
  if (usable.length === 0) return EMPTY_CARRY_PLAN
  const receipts = new Map<string, AuditProviderCarriedReceipt>()
  const findings: AuditProviderFinding[] = []
  for (const target of context.targets) {
    ...
    if (receipt.blob !== target.blob || receipt.lines !== target.lines) continue
    ...
    if (receipt.findingOccurrenceIds.length !== priorFindings.length) continue
    const occurrenceIds = new Set(receipt.findingOccurrenceIds)
    if (priorFindings.some((finding) => !occurrenceIds.has(finding.occurrenceId))) continue
    if ((receipt.outcome === 'findings') !== (priorFindings.length > 0)) continue
    ...
    const carried: AuditProviderFinding[] = []
    ...
    for (const finding of priorFindings) {
      ...
      const recomputed = candidateFingerprint(
...
export async function runAuditProviderPhases(
  ...
  const reviewFiles = context.targets.filter(
    (target) => target.role === 'review' && !carryPlan.receipts.has(target.path),
  )
  ...
  const reviewUnitByPath: Record<string, string> = {}
  batches.forEach((files, index) => {
    for (const file of files) reviewUnitByPath[file.path] = `review:${index}`
  })
  ...
  const verificationUnitByFingerprint: Record<string, string> = {}
  ...
  return {
    ...
    reusedChunks,
    executedChunks,
    carriedReceipts: [...carryPlan.receipts.values()].sort((left, right) =>
      left.path.localeCompare(right.path),
    ),
    reviewUnitByPath,
...
function validateSynthesisOutput(
  ...
  const emittedFindings = new Map(
    synthesis.findings.map((finding) => [finding.fingerprint, finding]),
  )
  for (const receipt of carried.receipts.values()) {
    ...
    if (file.outcome !== receipt.outcome) {
      fail(
        'output-invalid',
        `synthesis restated the carried outcome for ${receipt.path} as ${file.outcome} instead of ${receipt.outcome}`,
        'synthesis',
      )
    }
    if (
      canonicalJson([...file.findingFingerprints].sort()) !==
      canonicalJson([...receipt.findingFingerprints].sort())
    ) {
      fail(
        'output-invalid',
        `synthesis restated the carried findings for ${receipt.path}`,
        'synthesis',
      )
    }
  }
  for (const finding of carried.findings) {
    ...
    if (emitted === undefined) {
      fail(
        'output-invalid',
        `synthesis dropped carried finding ${finding.fingerprint} on ${finding.path}`,
        'synthesis',
      )
    }
    if (canonicalJson(emitted) !== canonicalJson(finding)) {
      fail(
        'output-invalid',
        `synthesis altered carried finding ${finding.fingerprint} on ${finding.path}`,
        'synthesis',
      )
    }
  }
