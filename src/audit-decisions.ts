...
import type {
  ...
  AuditDecisionIndexV3,
  AuditDecisionLedgerEntryV1,
  AuditDecisionLedgerPortfolioResult,
  AuditDecisionLedgerV1,
  AuditDecisionPolicyInputV1,
  AuditDecisionPolicyV1,
  AuditDecisionProofV3,
  AuditDecisionRegressionV3,
  AuditDecisionReviewContextV3,
  AuditDecisionReviewV3,
  AuditDecisionSourceArtifactV3,
  ...
  AuditIdentityAliasV3,
  AuditIdentityRecord,
  AuditIndexedObservationV3,
  AuditIndexedOccurrenceV3,
  AuditMigrationSourceProofV3,
  AuditObservationHistoryV3,
  AuditProducerKind,
  AuditReconciliationSourceV3,
  AuditRetirementHistoryProofV3,
  AuditRulesetReceiptV3,
  AuditScopeRetirementEventInputV3,
  AuditScopeRetirementEventV3,
  AuditSeverity,
  AuditSha256,
  AuditStagedDeletionAbsenceProofV3,
  AuditVerifiedTreeStateV3,
  AuditDeletionCommitProofV3,
...
const SECURITY_SLUG_RE =
  /^security-[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/u
...
function sha256Canonical(value: unknown): AuditSha256 {
  return `sha256:${createHash('sha256')
    .update(canonicalJson(value), 'utf8')
    .digest('hex')}`
}

function dataRecordAt(
  value: unknown,
  pointer: string,
  ...
  for (const [key, descriptor] of Object.entries(descriptors)) {
    if (
      ...
      invalid(
        ...
        'members must be safe enumerable data properties',
      )
    }
...
function dataArrayAt(
  ...
  if (
    ...
    invalid(
      'invalid-array',
      pointer,
      `has an invalid length or exceeds the ${limit}-item limit`,
    )
  }
...
function boundedTextAt(value: unknown, pointer: string): string {
  if (
    typeof value !== 'string' ||
...
function textAt(value: unknown, pointer: string): string {
...
function patternAt(
  ...
  if (!pattern.test(text)) invalid('invalid-identity', pointer, description)
  return text
}
...
function blobAt(value: unknown, pointer: string): AuditBlobBindingV3['blob'] {
  return patternAt(
    value,
    GIT_BLOB_RE,
    pointer,
    'must be a canonical Git blob digest',
  ) as AuditBlobBindingV3['blob']
}

function revisionAt(value: unknown, pointer: string): string {
  return patternAt(
    value,
    FULL_REVISION_RE,
    pointer,
    'must be a full lowercase repository revision',
  )
}

function timestampAt(value: unknown, pointer: string): string {
  const text = patternAt(
    value,
    TIMESTAMP_RE,
    pointer,
    'must be a canonical RFC 3339 timestamp',
  )
...
function slugAt(value: unknown, pointer: string): string {
  return patternAt(
    value,
    SECURITY_SLUG_RE,
    pointer,
    'must be a security- prefixed lowercase kebab-case slug',
  )
}

function identityAt(value: unknown, pointer: string): string {
  const identity = patternAt(
    value,
    IDENTITY_RE,
    pointer,
    'must be a canonical lowercase stable identity',
  )
...
function strictJsonPointerAt(value: unknown, pointer: string): string {
  return patternAt(
    ...
    JSON_POINTER_RE,
    pointer,
    'must be a strict non-root JSON Pointer',
  )
}

function sortedUniqueStringsAt<T extends string>(
  value: unknown,
  pointer: string,
  parse: (candidate: unknown, pointer: string) => T = textAt as (
    candidate: unknown,
    pointer: string,
  ) => T,
  nonempty = false,
): T[] {
  const rows = dataArrayAt(value, pointer).map((candidate, index) =>
    parse(candidate, `${pointer}/${index}`)
  )
...
function compareBindings(
  left: AuditBlobBindingV3,
  right: AuditBlobBindingV3,
): number {
...
function parseBindings(
  ...
  const bindings = dataArrayAt(value, pointer).map((candidate, index) =>
    parseBlobBinding(candidate, `${pointer}/${index}`)
  )
  if (nonempty && bindings.length === 0) {
    invalid('empty-array', pointer, 'must contain at least one binding')
  }
  const keys = bindings.map(({ path: bindingPath, blob }) =>
    `${bindingPath}\0${blob}`
  )
  ...
  if (bindings.some((binding, index) =>
    compareBindings(binding, sorted[index]) !== 0
  )) {
...
function parseSourceArtifact(
  ...
  exactKeys(
    artifact,
    ['path', 'repositoryRevision', 'gitBlob', 'sha256'],
    [],
    pointer,
  )
  return {
    path: repoPathAt(artifact.path, `${pointer}/path`),
    repositoryRevision: revisionAt(
      artifact.repositoryRevision,
      `${pointer}/repositoryRevision`,
    ),
...
function parseCreatedAtBasis(
  value: unknown,
  pointer: string,
): 'recorded' | 'source' | 'source-revision-upper-bound' {
  return enumAt(
    value,
    ['recorded', 'source', 'source-revision-upper-bound'] as const,
    pointer,
  )
}

function parseReconciliationSource(
  value: unknown,
  pointer: string,
): AuditReconciliationSourceV3 {
  ...
  if (kind === 'manual') {
    ...
    return {
      ...
      ...(source.sourceArtifact === undefined
        ? {}
        : {
            sourceArtifact: parseSourceArtifact(
              source.sourceArtifact,
              `${pointer}/sourceArtifact`,
            ),
          }),
    }
  }
  ...
  return {
    ...
    name: patternAt(
      ...
      SOURCE_NAME_RE,
      `${pointer}/name`,
      'must be a canonical lowercase source name',
    ),
...
function parseAlias(
  ...
  return {
    scheme: patternAt(
      ...
      ALIAS_SCHEME_RE,
      `${pointer}/scheme`,
      'must be a canonical lowercase alias scheme',
    ),
...
function parseAliasEvent(
  ...
  const aliases = dataArrayAt(value.aliases, `${pointer}/aliases`).map(
    (candidate, index) => parseAlias(candidate, `${pointer}/aliases/${index}`),
  )
  if (aliases.length === 0) {
    invalid('empty-array', `${pointer}/aliases`, 'must contain at least one alias')
  }
  const aliasKeys = aliases.map(({ scheme, value: aliasValue }) =>
    `${scheme}\0${aliasValue}`
  )
  ...
  const sortedAliases = [...aliases].sort((left, right) =>
    utf16Compare(left.scheme, right.scheme) ||
    utf16Compare(left.value, right.value)
  )
  if (aliases.some((alias, index) =>
    alias.scheme !== sortedAliases[index].scheme ||
    alias.value !== sortedAliases[index].value
  )) {
    invalid(
      'invalid-order',
      `${pointer}/aliases`,
      'aliases must be sorted by scheme and value',
    )
  }
  return {
    type: 'identity-alias-reconciliation',
    decisionLedger: slugAt(
    ...
    findingId: patternAt(
      ...
      FINDING_ID_RE,
      `${pointer}/findingId`,
      'must be an Atlas finding ID',
    ) as AtlasFindingId,
    occurrenceIds: sortedUniqueStringsAt(
      value.occurrenceIds,
      `${pointer}/occurrenceIds`,
      (candidate, candidatePointer) => patternAt(
        candidate,
        OCCURRENCE_ID_RE,
        candidatePointer,
        'must be an Atlas occurrence ID',
      ) as AtlasOccurrenceId,
      true,
    ) as [AtlasOccurrenceId, ...AtlasOccurrenceId[]],
    relationship: enumAt(
      value.relationship,
      ['canonical', 'duplicate-of'] as const,
      `${pointer}/relationship`,
    ),
    source: parseReconciliationSource(value.source, `${pointer}/source`),
    createdAt: timestampAt(value.createdAt, `${pointer}/createdAt`),
    createdAtBasis: parseCreatedAtBasis(
      value.createdAtBasis,
      `${pointer}/createdAtBasis`,
    ),
    evidenceRefs: sortedUniqueStringsAt(
      value.evidenceRefs,
...
function parseFindingReconciliationEvent(
  value: Record<string, unknown>,
  pointer: string,
): AuditFindingReconciliationEventInputV3 {
  exactKeys(
  ...
  const parseOccurrenceId = (
    candidate: unknown,
    candidatePointer: string,
  ): AtlasOccurrenceId => patternAt(
    candidate,
    OCCURRENCE_ID_RE,
    candidatePointer,
    'must be an Atlas occurrence ID',
  ) as AtlasOccurrenceId
  const beforeOccurrenceIds = sortedUniqueStringsAt(
    value.beforeOccurrenceIds,
    `${pointer}/beforeOccurrenceIds`,
    parseOccurrenceId,
    true,
  ) as [AtlasOccurrenceId, ...AtlasOccurrenceId[]]
  const afterOccurrenceIds = sortedUniqueStringsAt(
    value.afterOccurrenceIds,
    `${pointer}/afterOccurrenceIds`,
    parseOccurrenceId,
    true,
  ) as [AtlasOccurrenceId, ...AtlasOccurrenceId[]]
  if (
    beforeOccurrenceIds.length > 1 &&
    afterOccurrenceIds.length > 1
  ) {
    invalid(
      'invalid-reconciliation-shape',
      pointer,
      'many-to-many reconciliation groups are forbidden',
    )
  }
  if (beforeOccurrenceIds.some((occurrenceId) =>
    afterOccurrenceIds.includes(occurrenceId)
  )) {
    invalid(
      'invalid-reconciliation-shape',
      pointer,
      'before and after occurrence sets must be disjoint',
    )
  }
  return {
    type: 'finding-reconciliation',
    comparisonId: patternAt(
      value.comparisonId,
      COMPARISON_ID_RE,
      `${pointer}/comparisonId`,
      'must be an Atlas comparison ID',
    ),
    decisionLedger: slugAt(
      value.decisionLedger,
      `${pointer}/decisionLedger`,
    ...
    outcome: enumAt(
      value.outcome,
      ['equivalent', 'distinct', 'uncertain'] as const,
      `${pointer}/outcome`,
    ),
    confidence: enumAt(
      value.confidence,
    ...
    ...(value.supersedesEventId === undefined
      ? {}
      : {
          supersedesEventId: patternAt(
            value.supersedesEventId,
            EVENT_ID_RE,
            `${pointer}/supersedesEventId`,
            'must be an Atlas decision event ID',
          ),
        }),
  }
}
...
function parseReviewContext(
  ...
  exactKeys(
    context,
    ['observationId', 'bindings', 'ruleset', 'policyDigest'],
    [],
    pointer,
  )
...
function optionalSourceArtifact(
  value: Record<string, unknown>,
  pointer: string,
...
function parseDecisionProof(
  ...
  if (kind === 'current-review') {
    exactKeys(
      proof,
      ['kind', 'observationId', 'reviewedBindings', 'outcome', 'summary'],
      ['sourceArtifact'],
      pointer,
    )
    if (proof.outcome !== 'finding-present') {
      invalid(
        ...
        `${pointer}/outcome`,
        'current-review outcome must equal finding-present',
      )
    }
    return {
      kind,
      ...
      reviewedBindings: parseBindings(
        proof.reviewedBindings,
        `${pointer}/reviewedBindings`,
      ) as AuditDecisionReviewContextV3['bindings'],
  ...
  if (kind === 'post-fix') {
    ...
    if (proof.outcome !== 'finding-absent-after-fix') {
      invalid(
        'invalid-proof-outcome',
        `${pointer}/outcome`,
        'post-fix outcome must equal finding-absent-after-fix',
      )
    }
    return {
      kind,
      beforeObservationId: patternAt(
        proof.beforeObservationId,
        OBSERVATION_ID_RE,
        `${pointer}/beforeObservationId`,
        'must be an Atlas observation ID',
      ) as AuditDecisionReviewContextV3['observationId'],
      afterObservationId: patternAt(
        proof.afterObservationId,
        OBSERVATION_ID_RE,
        `${pointer}/afterObservationId`,
        'must be an Atlas observation ID',
      ) as AuditDecisionReviewContextV3['observationId'],
      beforeBindings: parseBindings(
        proof.beforeBindings,
        `${pointer}/beforeBindings`,
      ) as AuditDecisionReviewContextV3['bindings'],
      afterBindings: parseBindings(
        proof.afterBindings,
        `${pointer}/afterBindings`,
      ) as AuditDecisionReviewContextV3['bindings'],
  ...
  if (kind === 'source-evidence') {
    ...
    if (proof.outcome !== 'not-reportable') {
      invalid(
        'invalid-proof-outcome',
        `${pointer}/outcome`,
        'source-evidence outcome must equal not-reportable',
      )
    }
    return {
      kind,
  ...
  if (kind === 'replacement') {
    ...
    if (proof.outcome !== 'replacement-tracks-root-cause') {
      invalid(
        'invalid-proof-outcome',
        `${pointer}/outcome`,
        'replacement outcome must equal replacement-tracks-root-cause',
      )
    }
    return {
      kind,
      ...
      replacementFindingId: patternAt(
        proof.replacementFindingId,
        FINDING_ID_RE,
        `${pointer}/replacementFindingId`,
        'must be an Atlas finding ID',
      ) as AtlasFindingId,
      replacementOccurrenceId: patternAt(
        proof.replacementOccurrenceId,
        OCCURRENCE_ID_RE,
        `${pointer}/replacementOccurrenceId`,
        'must be an Atlas occurrence ID',
      ) as AtlasOccurrenceId,
      replacementBindings: parseBindings(
        proof.replacementBindings,
        `${pointer}/replacementBindings`,
      ) as AuditDecisionReviewContextV3['bindings'],
  ...
  if (kind === 'deletion') {
    ...
    if (proof.outcome !== 'exact-source-deleted') {
      invalid(
        'invalid-proof-outcome',
        `${pointer}/outcome`,
        'deletion outcome must equal exact-source-deleted',
      )
    }
    return {
      kind,
      deletionCommit: revisionAt(
        proof.deletionCommit,
        `${pointer}/deletionCommit`,
      ),
      parentRevision: revisionAt(
        proof.parentRevision,
        `${pointer}/parentRevision`,
      ),
      deletedBindings: parseBindings(
        proof.deletedBindings,
        `${pointer}/deletedBindings`,
      ) as AuditDecisionReviewContextV3['bindings'],
  ...
  if (proof.outcome !== 'no-reportable-replacement') {
    invalid(
      'invalid-proof-outcome',
      `${pointer}/outcome`,
      'no-replacement outcome must equal no-reportable-replacement',
    )
  }
  return {
    kind,
    ...
    searchRevision: revisionAt(
      proof.searchRevision,
      `${pointer}/searchRevision`,
    ),
    reviewedBindings: parseBindings(
      proof.reviewedBindings,
      `${pointer}/reviewedBindings`,
...
function parseActionEvidence(
  ...
  const kind = enumAt(
    evidence.kind,
    ['source-evidence', 'remediation', 'replacement', 'deletion'] as const,
    `${pointer}/kind`,
  )
  if (kind === 'source-evidence') {
    exactKeys(
      evidence,
      ['kind', 'reviewedBindings', 'conclusion', 'rationale'],
      [],
      pointer,
    )
    if (evidence.conclusion !== 'not-reportable') {
      invalid(
        ...
        `${pointer}/conclusion`,
        'must equal not-reportable',
      )
    }
    return {
      kind,
      reviewedBindings: parseBindings(
        evidence.reviewedBindings,
        `${pointer}/reviewedBindings`,
      ) as AuditDecisionReviewContextV3['bindings'],
  ...
  if (kind === 'remediation') {
    exactKeys(
      evidence,
      ['kind', 'beforeBindings', 'afterBindings', 'fixRevision'],
      [],
      pointer,
    )
    return {
      kind,
      beforeBindings: parseBindings(
        evidence.beforeBindings,
        `${pointer}/beforeBindings`,
      ) as AuditDecisionReviewContextV3['bindings'],
      afterBindings: parseBindings(
        evidence.afterBindings,
        `${pointer}/afterBindings`,
      ) as AuditDecisionReviewContextV3['bindings'],
      fixRevision: revisionAt(
        evidence.fixRevision,
        `${pointer}/fixRevision`,
      ),
    }
  }
  if (kind === 'replacement') {
    exactKeys(
      evidence,
      ['kind', 'replacementFindingId', 'replacementOccurrenceId'],
      [],
      pointer,
    )
    return {
      kind,
      replacementFindingId: patternAt(
        evidence.replacementFindingId,
        FINDING_ID_RE,
        `${pointer}/replacementFindingId`,
        'must be an Atlas finding ID',
      ) as AtlasFindingId,
      replacementOccurrenceId: patternAt(
        evidence.replacementOccurrenceId,
        OCCURRENCE_ID_RE,
        `${pointer}/replacementOccurrenceId`,
        'must be an Atlas occurrence ID',
      ) as AtlasOccurrenceId,
  ...
  const noReplacement = dataRecordAt(
    evidence.noReplacementEvidence,
  ...
  return {
    kind,
    deletionCommit: revisionAt(
      evidence.deletionCommit,
      `${pointer}/deletionCommit`,
    ),
    deletedBindings: parseBindings(
      evidence.deletedBindings,
      `${pointer}/deletedBindings`,
    ) as AuditDecisionReviewContextV3['bindings'],
    noReplacementEvidence: {
      observationId: patternAt(
        noReplacement.observationId,
        OBSERVATION_ID_RE,
        `${pointer}/noReplacementEvidence/observationId`,
        'must be an Atlas observation ID',
      ) as AuditDecisionReviewContextV3['observationId'],
      searchRevision: revisionAt(
        noReplacement.searchRevision,
        `${pointer}/noReplacementEvidence/searchRevision`,
      ),
      reviewedBindings: parseBindings(
        noReplacement.reviewedBindings,
        `${pointer}/noReplacementEvidence/reviewedBindings`,
      ) as AuditDecisionReviewContextV3['bindings'],
      summary: textAt(
        noReplacement.summary,
        `${pointer}/noReplacementEvidence/summary`,
      ),
    },
  }
}
...
function parseRevisionBinding(
  value: unknown,
  pointer: string,
): AuditDecisionRegressionV3['binding'] {
  const binding = dataRecordAt(value, pointer)
  exactKeys(
    binding,
    ['repositoryRevision', 'files'],
    ['observationId'],
    pointer,
  )
  return {
...
function parseRegression(
  ...
  const kind = enumAt(
    regression.kind,
    ['test', 'guardrail', 'check', 'manual'] as const,
    `${pointer}/kind`,
  )
  if (kind === 'manual') {
    exactKeys(
      regression,
      ['kind', 'name', 'result', 'binding'],
      ['observedAt'],
      pointer,
    )
    return {
      ...
      ...(regression.observedAt === undefined
        ? {}
        : {
            observedAt: timestampAt(
              regression.observedAt,
              `${pointer}/observedAt`,
            ),
          }),
    }
  ...
  exactKeys(
    regression,
    ['kind', 'name', 'command', 'result', 'binding'],
    ['observedAt'],
    pointer,
  )
  return {
...
function sameBindings(
  left: readonly AuditBlobBindingV3[],
  right: readonly AuditBlobBindingV3[],
): boolean {
  return left.length === right.length &&
    left.every((binding, index) =>
      binding.path === right[index].path &&
      binding.blob === right[index].blob
    )
}

function sameRuleset(
  left: AuditDecisionReviewContextV3['ruleset'] | null,
  right: AuditDecisionReviewContextV3['ruleset'] | null,
): boolean {
  return left === null
    ? right === null
    : right !== null &&
...
const AUDIT_PRODUCER_KINDS: readonly AuditProducerKind[] = [
  'grok-cli',
  'codex-security',
  'migration',
  'manual',
...
function producerKindOf(value: unknown): AuditProducerKind | null {
  return typeof value === 'string' &&
    (AUDIT_PRODUCER_KINDS as readonly string[]).includes(value)
    ? value as AuditProducerKind
    : null
}

function bindingPathsWithinLocations(
  bindings: readonly AuditBlobBindingV3[],
...
function migrationContextObservation(
  observations: AuditDecisionIndexV3['observations'],
  observationId: string,
  ruleset: AuditRulesetReceiptV3,
  slug: string | null,
): AuditIndexedObservationV3 | null {
  const observation = observations.get(
    observationId as AuditDecisionReviewContextV3['observationId'],
  )
  if (
...
function dispositionContextKind(
  event: Extract<AuditDecisionEventV3, { type: 'finding-disposition' }>,
  occurrence: AuditIndexedOccurrenceV3,
  observations: AuditDecisionIndexV3['observations'],
): 'exact' | 'migration' | 'invalid' {
  if (
  ...
  if (
    ...
    event.reviewContext.ruleset.id === occurrence.ruleset.id &&
    event.reviewContext.ruleset.digest === occurrence.ruleset.digest &&
    migrationContextObservation(
      observations,
      occurrence.observationId,
      occurrence.ruleset,
      occurrence.decisionLedger,
    ) !== null &&
    migrationContextObservation(
      observations,
    ...
    sameLocationPaths(
      event.reviewContext.bindings,
      occurrence.locationPaths,
    )
  ) {
    return 'migration'
  }
...
function bindingMismatchReasons(
  ...
  if (sameBindings(bindings, recorded)) return []
  ...
  for (const [index, binding] of bindings.entries()) {
    ...
    if (binding.path !== match.path) {
      reasons.push(
        `reviewContext.bindings[${index}] path ${binding.path} does not match` +
          ` the path observation ${occurrence.observationId} recorded at that` +
          ` position (${match.path})`,
      )
    } else if (binding.blob !== match.blob) {
      reasons.push(
        `reviewContext.bindings[${index}] path ${binding.path} blob` +
  ...
  if (bindings.length !== recorded.length) {
    reasons.push(
      `reviewContext.bindings lists ${String(bindings.length)} binding(s) but` +
 ...
 * reports separately.
 */
function exactContextMismatchReasons(
  event: Extract<AuditDecisionEventV3, { type: 'finding-disposition' }>,
  occurrence: AuditIndexedOccurrenceV3,
  observations: AuditDecisionIndexV3['observations'],
): string[] {
  const ruleset = occurrence.ruleset
  if (!occurrence.closureEligible || ruleset === null) return []
  ...
  if (context.ruleset.id !== ruleset.id) {
    reasons.push(
      `reviewContext.ruleset.id ${context.ruleset.id} does not match ruleset` +
    ...
    reasons.push(
      `reviewContext.ruleset.digest ${shortDigest(context.ruleset.digest)}` +
...
function requireStableIdentityReconciliation(
  event: AuditFindingReconciliationEventV3,
  occurrences: AuditDecisionIndexV3['occurrences'],
): void {
  const beforeFindingIds = new Set(event.beforeOccurrenceIds.map(
    (occurrenceId) => {
      const occurrence = occurrences.get(occurrenceId)
      if (!occurrence) {
        throw new Error(
          `reconciliation ${event.eventId} references unknown occurrence ${occurrenceId}`,
        )
      }
  ...
  if (
    ...
    throw new Error(
      `reconciliation ${event.eventId} compares occurrences of the same finding and must be high-confidence equivalent`,
    )
  }
}

function parseFindingDispositionEvent(
  value: Record<string, unknown>,
  pointer: string,
): AuditDecisionEventInputV3 {
  const action = enumAt(
  ...
  let required: string[] = [...commonRequired]
  let optional: string[] = []
  if (action === 'open') optional = ['supersedesEventId']
  if (action === 'reopened') required.push('supersedesEventId')
  ...
  if (action === 'superseded') required.push('actionEvidence')
  exactKeys(value, required, optional, pointer)

  const createdAtBasis = parseCreatedAtBasis(
    value.createdAtBasis,
    `${pointer}/createdAtBasis`,
  )
  const context = parseReviewContext(
    value.reviewContext,
    `${pointer}/reviewContext`,
  )
  const proofs = dataArrayAt(value.proofs, `${pointer}/proofs`).map(
    (candidate, index) =>
      parseDecisionProof(candidate, `${pointer}/proofs/${index}`),
  )
  const reviews = dataArrayAt(value.reviews, `${pointer}/reviews`).map(
    (candidate, index) =>
      parseDecisionReview(candidate, `${pointer}/reviews/${index}`),
  )
  if (
    ...
    invalid(
      'missing-migrated-source-artifact',
      `${pointer}/proofs`,
      'migrated proofs require a sealed sourceArtifact',
    )
  }
  const common = {
  ...
  const requireProofKinds = (
    allowed: readonly AuditDecisionProofV3['kind'][],
    minimum: number,
  ): void => {
    if (
      proofs.length < minimum ||
      proofs.some((proof) => !allowed.includes(proof.kind))
    ) {
      invalid(
        'invalid-action-proof',
        `${pointer}/proofs`,
        `${action} requires ${allowed.join(' or ')} proofs`,
      )
    }
  }
  const requireReviewProofMatches = (): void => {
    for (const [index, proof] of proofs.entries()) {
      if (
        (proof.kind === 'current-review' ||
        ...
        invalid(
          'review-proof-mismatch',
          `${pointer}/proofs/${index}`,
          'review proof must equal the event review context',
        )
      }
    }
  }
  ...
  const actionEvidence = parseActionEvidence(
    value.actionEvidence,
    `${pointer}/actionEvidence`,
  )
  if (action === 'false-positive') {
    if (value.expiresAt !== null) {
      invalid(
        'invalid-expiry',
        `${pointer}/expiresAt`,
        'false-positive expiresAt must be null',
      )
    }
    ...
    if (
      ...
      invalid(
        'invalid-action-evidence',
        `${pointer}/actionEvidence`,
        'false-positive requires source-evidence matching review bindings',
      )
    }
    return {
      ...common,
  ...
  if (action === 'remediated') {
    ...
    const regression = parseRegression(
      value.regression,
      `${pointer}/regression`,
    )
    if (regression.kind === 'manual' || regression.result !== 'passed') {
      invalid(
        'invalid-regression',
        `${pointer}/regression`,
        'remediation regression must be a passing test, guardrail, or check',
      )
    }
    if (actionEvidence.kind !== 'remediation') {
      invalid(
        'invalid-action-evidence',
        `${pointer}/actionEvidence`,
        'remediation requires remediation evidence',
      )
    }
    for (const [index, proof] of proofs.entries()) {
      if (
        ...
        invalid(
          'remediation-proof-mismatch',
          `${pointer}/proofs/${index}`,
          'post-fix proof, action evidence, regression, and context must agree',
        )
      }
    }
    return {
      ...
      regression: regression as AuditDecisionRegressionV3 & {
        result: 'passed'
      },
      actionEvidence,
    } as AuditDecisionEventInputV3
  }
  if (actionEvidence.kind === 'replacement') {
    requireProofKinds(['replacement'], 1)
    for (const [index, proof] of proofs.entries()) {
      if (
        proof.kind !== 'replacement' ||
        proof.replacementFindingId !==
        ...
        invalid(
          'replacement-proof-mismatch',
          `${pointer}/proofs/${index}`,
          'replacement proof and action evidence must agree',
        )
      }
    }
    return {
  ...
  if (actionEvidence.kind !== 'deletion') {
    invalid(
      'invalid-action-evidence',
      `${pointer}/actionEvidence`,
      'superseded requires replacement or deletion evidence',
    )
  }
  requireProofKinds(['deletion', 'no-replacement'], 2)
  const deletionProofs = proofs.filter((proof) => proof.kind === 'deletion')
  const noReplacementProofs = proofs.filter((proof) =>
    proof.kind === 'no-replacement'
  )
  if (deletionProofs.length === 0 || noReplacementProofs.length === 0) {
    invalid(
      'invalid-action-proof',
      `${pointer}/proofs`,
      'deletion supersession requires deletion and no-replacement proofs',
    )
  }
  if (
    deletionProofs.some((proof) =>
      proof.deletionCommit !== actionEvidence.deletionCommit ||
      !sameBindings(proof.deletedBindings, actionEvidence.deletedBindings)
    ) ||
    noReplacementProofs.some((proof) =>
      proof.observationId !==
      ...
      !sameBindings(
        proof.reviewedBindings,
        actionEvidence.noReplacementEvidence.reviewedBindings,
      )
    )
  ) {
    invalid(
      'deletion-proof-mismatch',
      `${pointer}/proofs`,
      'deletion proofs and action evidence must agree',
    )
  }
  return {
    ...common,
...
function parseRetirementHistoryProof(
  value: unknown,
  pointer: string,
): AuditRetirementHistoryProofV3 {
  const proof = dataRecordAt(value, pointer)
...
function parseStagedAbsenceProof(
  value: unknown,
  pointer: string,
): AuditStagedDeletionAbsenceProofV3 {
  const proof = dataRecordAt(value, pointer)
  ...
  if (
    proof.kind !== 'worktree-index-absence' ||
    ...
    invalid(
      ...
      pointer,
      'must be a worktree-index-absence proof with absent states',
    )
  }
  return {
    kind: 'worktree-index-absence',
    headRevision: revisionAt(
      proof.headRevision,
      `${pointer}/headRevision`,
    ),
    headBinding: parseBlobBinding(
      proof.headBinding,
      `${pointer}/headBinding`,
    ),
    indexState: 'absent',
...
function parseDeletionCommitProof(
  value: unknown,
  pointer: string,
): AuditDeletionCommitProofV3 {
  const proof = dataRecordAt(value, pointer)
  ...
  if (proof.kind !== 'git-deletion') {
    invalid(
      'invalid-deletion-proof',
      `${pointer}/kind`,
      'must equal git-deletion',
    )
  }
  return {
    kind: 'git-deletion',
    ...
    parentBindings: parseBindings(
      proof.parentBindings,
      `${pointer}/parentBindings`,
    ) as AuditDeletionCommitProofV3['parentBindings'],
    absentPaths: sortedUniqueStringsAt(
      proof.absentPaths,
      `${pointer}/absentPaths`,
      repoPathAt,
      true,
    ) as AuditDeletionCommitProofV3['absentPaths'],
  }
}
...
function parseTreeState(
  ...
  exactKeys(
    proof,
    ['kind', 'repositoryRevision', 'presentBindings', 'absentPaths'],
    [],
    pointer,
  )
  if (proof.kind !== 'git-tree-state') {
    invalid(
      'invalid-tree-state',
      `${pointer}/kind`,
      'must equal git-tree-state',
    )
  }
  const presentBindings = parseBindings(
    proof.presentBindings,
    `${pointer}/presentBindings`,
    false,
  )
  const absentPaths = sortedUniqueStringsAt(
    proof.absentPaths,
    `${pointer}/absentPaths`,
    repoPathAt,
  )
  if (presentBindings.some((binding) => absentPaths.includes(binding.path))) {
    invalid(
      'invalid-tree-state',
      pointer,
      'present and absent path sets must be disjoint',
    )
  }
  return {
    kind: 'git-tree-state',
    repositoryRevision: revisionAt(
      proof.repositoryRevision,
      `${pointer}/repositoryRevision`,
    ),
    presentBindings,
    absentPaths,
...
function parseMigrationSourceProof(
  value: unknown,
  pointer: string,
): AuditMigrationSourceProofV3 {
  const proof = dataRecordAt(value, pointer)
  ...
  if (
    proof.kind !== 'sealed-migration-source' ||
    proof.sourceReason !== 'uncommitted_snapshot_absent'
  ) {
    invalid(
      'invalid-migration-proof',
      pointer,
      'must be a sealed uncommitted_snapshot_absent source proof',
    )
  }
  return {
    kind: 'sealed-migration-source',
    sourceArtifact: parseSourceArtifact(
      proof.sourceArtifact,
      `${pointer}/sourceArtifact`,
    ),
    jsonPointer: strictJsonPointerAt(
      proof.jsonPointer,
      `${pointer}/jsonPointer`,
    ),
...
function parseScopeRetirementEvent(
  value: Record<string, unknown>,
  pointer: string,
): AuditScopeRetirementEventInputV3 {
  const reason = enumAt(
  ...
  if (reason === 'staged-deletion') required.push('absenceProof')
  if (reason === 'deleted') {
    required.push('deletionCommit', 'deletionProof')
    optional.push('supersedesEventId')
  }
  if (reason === 'moved') required.push('successor', 'revisionProof')
  ...
  const precision = enumAt(
    value.retiredAtPrecision,
    ['timestamp', 'date'] as const,
    `${pointer}/retiredAtPrecision`,
  )
  if (precision === 'date') required.push('originalRetiredDate')
  exactKeys(value, required, optional, pointer)

  const decisionLedger = slugAt(
    value.decisionLedger,
    `${pointer}/decisionLedger`,
  )
  ...
  const historyProof = parseRetirementHistoryProof(
    value.historyProof,
    `${pointer}/historyProof`,
  )
  if (
    historyProof.slug !== decisionLedger ||
    historyProof.path !== retiredPath ||
    historyProof.blob !== retiredBlob
  ) {
    invalid(
      'retirement-history-proof-mismatch',
      `${pointer}/historyProof`,
      'must bind the containing ledger and exact retired path/blob',
    )
  }
  const originalRetiredDate = precision === 'date'
    ? patternAt(
        value.originalRetiredDate,
        DATE_RE,
        `${pointer}/originalRetiredDate`,
  ...
  const common = {
    ...
    ...(precision === 'timestamp'
      ? { retiredAtPrecision: 'timestamp' as const }
      : {
  ...
  if (
    ...
    invalid(
      'retirement-date-precision-mismatch',
      `${pointer}/retiredAt`,
      'date precision must normalize the original date to midnight UTC',
    )
  }
  if (reason === 'staged-deletion') {
    const absenceProof = parseStagedAbsenceProof(
      value.absenceProof,
      `${pointer}/absenceProof`,
    )
    if (
      absenceProof.headBinding.path !== retiredPath ||
      absenceProof.headBinding.blob !== retiredBlob
    ) {
      invalid(
        'retirement-absence-proof-mismatch',
        `${pointer}/absenceProof/headBinding`,
        'must equal the retired path/blob',
      )
    }
    return { ...common, reason, absenceProof }
  }
  if (reason === 'deleted') {
    const deletionCommit = revisionAt(
      value.deletionCommit,
      `${pointer}/deletionCommit`,
    )
    const deletionProof = parseDeletionCommitProof(
      value.deletionProof,
      `${pointer}/deletionProof`,
    )
    if (
      !deletionProof.parentBindings.some((binding) =>
        binding.path === retiredPath && binding.blob === retiredBlob
      ) ||
      !deletionProof.absentPaths.includes(retiredPath)
    ) {
      invalid(
        'retirement-deletion-proof-mismatch',
        `${pointer}/deletionProof`,
        'must prove the retired binding in the parent and path absent after deletion',
      )
    }
    return {
      ...common,
  ...
  if (reason === 'moved' || reason === 'superseded') {
    const revisionProof = parseTreeState(
      value.revisionProof,
      `${pointer}/revisionProof`,
    )
    if (!revisionProof.absentPaths.includes(retiredPath)) {
      invalid(
        'retirement-revision-proof-mismatch',
        `${pointer}/revisionProof/absentPaths`,
        'must prove the retired path absent',
      )
    }
    if (value.successor !== undefined) {
      if (reason === 'superseded' && value.noReplacementProof !== undefined) {
        invalid(
          'invalid-retirement-branch',
          pointer,
          'superseded must choose exactly one successor or no-replacement branch',
        )
      }
      const successor = parseBlobBinding(
        value.successor,
        `${pointer}/successor`,
      )
      if (
        !revisionProof.presentBindings.some((binding) =>
          binding.path === successor.path && binding.blob === successor.blob
        ) ||
        (reason === 'moved' && successor.blob !== retiredBlob)
      ) {
        invalid(
          'retirement-successor-mismatch',
          `${pointer}/successor`,
          reason === 'moved'
    ...
    if (reason === 'moved' || value.noReplacementProof === undefined) {
      invalid(
        'missing-retirement-branch',
        pointer,
        `${reason} requires its reason-specific successor or no-replacement branch`,
      )
    }
    const noReplacementProof = parseDecisionProof(
      value.noReplacementProof,
      `${pointer}/noReplacementProof`,
    )
    if (noReplacementProof.kind !== 'no-replacement') {
      invalid(
        'invalid-retirement-proof',
        `${pointer}/noReplacementProof/kind`,
        'must equal no-replacement',
      )
    }
    if (
      common.createdAtBasis !== 'recorded' &&
      noReplacementProof.sourceArtifact === undefined
    ) {
      invalid(
        'missing-migrated-source-artifact',
        `${pointer}/noReplacementProof/sourceArtifact`,
        'migrated no-replacement proofs require a sealed sourceArtifact',
      )
    }
    return {
      ...common,
      ...
      noReplacementProof,
      revisionProof,
    } as AuditScopeRetirementEventInputV3
  }
  return {
    ...common,
    reason,
    migrationSourceProof: parseMigrationSourceProof(
      value.migrationSourceProof,
      `${pointer}/migrationSourceProof`,
    ),
  }
}

function parseDecisionEventInput(
  value: unknown,
  pointer: string,
): AuditDecisionEventInputV3 {
...
function parseStoredDecisionEvent(
  value: unknown,
  pointer: string,
): AuditDecisionEventV3 {
  const event = dataRecordAt(value, pointer)
  ...
  if (eventId !== expectedEventId) {
    invalid(
      'decision-event-id-mismatch',
      `${pointer}/eventId`,
      'does not match the canonical event identity',
    )
  }
  return { ...parsedInput, eventId } as AuditDecisionEventV3
}

export function computeAuditDecisionEventId(
  event: AuditDecisionEventInputV3,
): string {
  const parsed = parseDecisionEventInput(event, '/event')
  return stableAuditId(
    'adev',
    'atlas-decision-event/v1',
    [canonicalJson(parsed)],
  )
}

export function computeAuditDecisionEntryDigest(
  entry: Omit<AuditDecisionLedgerEntryV1, 'entryDigest'>,
): AuditSha256 {
...
export function computeAuditFindingComparisonId(boundary: {
  ...
  exactKeys(
    value,
    ['beforeObservationIds', 'afterObservationIds'],
    [],
    '/comparison',
  )
  const parseObservationId = (
    candidate: unknown,
    pointer: string,
  ): string => patternAt(
    candidate,
  ...
  const before = dataArrayAt(
    value.beforeObservationIds,
    '/comparison/beforeObservationIds',
  ).map((candidate, index) =>
    parseObservationId(
      candidate,
      `/comparison/beforeObservationIds/${index}`,
    )
  )
  const after = dataArrayAt(
    value.afterObservationIds,
    '/comparison/afterObservationIds',
  ).map((candidate, index) =>
    parseObservationId(
      candidate,
      `/comparison/afterObservationIds/${index}`,
    )
  )
  if (before.length === 0 || after.length === 0) {
    invalid(
      'invalid-comparison-boundary',
      '/comparison',
      'before and after observation sets must be nonempty',
    )
  }
  if (
    ...
    invalid(
      'invalid-comparison-boundary',
      '/comparison',
      'observation sets must contain unique IDs',
    )
  }
  if (before.some((observationId) => after.includes(observationId))) {
    invalid(
      'invalid-comparison-boundary',
      '/comparison',
      'before and after observation sets must be disjoint',
    )
  }
  const normalized = {
  ...
  return stableAuditId(
    'acmp',
    'atlas-finding-comparison/v1',
    [canonicalJson(normalized)],
  )
}

export function validateUniqueAuditIdentityRecords(
  records: readonly AuditIdentityRecord[],
): AuditDiagnostic[] {
  const diagnostics: AuditDiagnostic[] = []
  let candidates: unknown[]
  try {
    candidates = dataArrayAt(
      records,
      '/records',
  ...
  const byId = new Map<string, AuditIdentityRecord>()
  const byDigest = new Map<string, AuditIdentityRecord>()
  for (const [index, candidate] of candidates.entries()) {
    let parsed: AuditIdentityRecord
    try {
      const pointer = `/records/${index}`
      const record = dataRecordAt(candidate, pointer)
      exactKeys(
        record,
        ['namespace', 'id', 'digest', 'location'],
        [],
        pointer,
      )
      const namespace = enumAt(
        record.namespace,
        ['decision-event', 'decision-entry', 'comparison'] as const,
        `${pointer}/namespace`,
      )
...
function parseDecisionLedger(
  ...
  if (ledger.format !== 'atlas-audit-decisions-v1') {
    invalid(
      'invalid-format',
      `${pointer}/format`,
      'must equal atlas-audit-decisions-v1',
    )
  }
  ...
  if (candidates.length === 0 || candidates.length > LEDGER_ENTRY_LIMIT) {
    invalid(
      'invalid-chain',
      `${pointer}/entries`,
      `must contain between 1 and ${LEDGER_ENTRY_LIMIT} entries`,
    )
  }
  const entries: AuditDecisionLedgerEntryV1[] = []
  const identityRecords: AuditIdentityRecord[] = []
  ...
  for (const [index, candidate] of candidates.entries()) {
    ...
    exactKeys(
      entry,
      ['eventId', 'previousEntryDigest', 'event', 'entryDigest'],
      [],
      entryPointer,
    )
    const eventId = patternAt(
    ...
    const storedPrevious = entry.previousEntryDigest === null
      ...
      : sha256At(
          ...
          `${entryPointer}/previousEntryDigest`,
        )
    if (storedPrevious !== previousEntryDigest) {
      invalid(
        'decision-chain-mismatch',
        `${entryPointer}/previousEntryDigest`,
        index === 0
    ...
    if (event.eventId !== eventId) {
      invalid(
        'decision-entry-event-mismatch',
        `${entryPointer}/eventId`,
        'must equal event.eventId',
      )
    }
    if (
      'decisionLedger' in event &&
      event.decisionLedger !== slug
    ) {
      invalid(
        'decision-ledger-home-mismatch',
        `${entryPointer}/event/decisionLedger`,
        'must equal the containing ledger slug',
      )
    }
    const entryDigest = sha256At(
      entry.entryDigest,
      `${entryPointer}/entryDigest`,
    )
    const core = {
      eventId,
    ...
    if (entryDigest !== computeAuditDecisionEntryDigest(core)) {
      invalid(
        'decision-entry-digest-mismatch',
        `${entryPointer}/entryDigest`,
        'does not seal the canonical entry',
      )
    }
    const parsedEntry: AuditDecisionLedgerEntryV1 = {
      ...core,
      entryDigest,
    }
  ...
  const identityDiagnostics = validateUniqueAuditIdentityRecords(
    identityRecords,
  )
...
export function prepareAuditDecisionAppend(
  ...
  const storedEvent = { ...parsedEvent, eventId } as AuditDecisionEventV3
  ...
  const existingEntry = parsedLedger?.entries.find((entry) =>
  ...
  const previousEntryDigest =
    parsedLedger?.entries.at(-1)?.entryDigest ?? null
  const entryCore = {
    eventId,
    previousEntryDigest,
    event: storedEvent,
  }
  const entry: AuditDecisionLedgerEntryV1 = {
    ...entryCore,
    entryDigest: computeAuditDecisionEntryDigest(entryCore),
  }
  const nextLedger: AuditDecisionLedgerV1 = {
    formatVersion: 1,
    format: 'atlas-audit-decisions-v1',
    domain,
    slug: parsedSlug,
...
export function loadAuditDecisionLedgers(
  ...
  const ledgers: AuditDecisionLedgerV1[] = []
  const diagnostics: AuditDiagnostic[] = []
  let entries: string[]
  try {
    entries = listBoundedAuditDirectory(
      root,
      '.atlas/audit-decisions',
      DECISION_LEDGER_COUNT_LIMIT + 1,
    )
  } catch (error) {
    if (
      error instanceof Error &&
      error.message.startsWith(
        `audit directory exceeds the ${DECISION_LEDGER_COUNT_LIMIT + 1}-entry limit:`,
      )
    ) {
      return {
        ledgers,
        diagnostics: [{
          ...
          message:
            `decision portfolio exceeds the ${DECISION_LEDGER_COUNT_LIMIT}-ledger limit`,
        }],
      }
    }
    return {
  ...
  for (const name of entries) {
    ...
    if (remainingBytes <= 0) {
      return {
        ...
        diagnostics: [{
          ...
          path: '.atlas/audit-decisions',
          message:
            `decision portfolio exceeds the ${DECISION_PORTFOLIO_BYTE_LIMIT}-byte cumulative raw-byte limit`,
        }],
      }
    }
    ...
    try {
      ...
      const ledger = parseDecisionLedger(
        parseBoundedAuditJsonBytes(bytes, readLimit, repoPath),
      )
      ...
      if (
        remainingBytes < DECISION_LEDGER_BYTE_LIMIT &&
        error instanceof Error &&
        error.message.startsWith(
          `audit file exceeds the ${remainingBytes}-byte limit:`,
        )
      ) {
        return {
          ledgers: [],
...
export function appendAuditDecision(
  ...
  return withAuditLock(root, () => {
    ...
    if (portfolio.diagnostics.length > 0) {
      throw new Error(
        `decision portfolio is invalid: ${portfolio.diagnostics.map((row) =>
    ...
    let names: string[]
    try {
      names = listBoundedAuditDirectory(
        root,
        '.atlas/audit-decisions',
        DECISION_LEDGER_COUNT_LIMIT + 1,
      )
    ...
    for (const name of names) {
      const remainingBytes =
        DECISION_PORTFOLIO_BYTE_LIMIT - rawPortfolioBytes
      if (remainingBytes <= 0) {
        throw new Error(
          `decision portfolio exceeds the ${DECISION_PORTFOLIO_BYTE_LIMIT}-byte cumulative raw-byte limit`,
        )
      ...
      const bytes = readBoundedAuditBytes(
        root,
        `.atlas/audit-decisions/${name}`,
        Math.min(DECISION_LEDGER_BYTE_LIMIT, remainingBytes),
      )
      rawPortfolioBytes += bytes.byteLength
      if (name === targetName) existingBytes = bytes
    }
    ...
    const plan = prepareAuditDecisionAppend(
      existingLedger,
      'security',
      normalizedSlug,
      event,
    ...
    if (planByteLength > DECISION_LEDGER_BYTE_LIMIT) {
      throw new Error(
        `decision ledger exceeds the ${DECISION_LEDGER_BYTE_LIMIT}-byte limit`,
      )
    }
    const nextLedgerCount =
      names.length + (existingBytes === null ? 1 : 0)
    if (nextLedgerCount > DECISION_LEDGER_COUNT_LIMIT) {
      throw new Error(
        `decision portfolio exceeds the ${DECISION_LEDGER_COUNT_LIMIT}-ledger limit`,
      )
    }
    const nextPortfolioBytes =
      rawPortfolioBytes -
    ...
    if (plan.status === 'append') {
      ...
      for (const ledger of portfolio.ledgers) {
        const ledgerPath =
          `.atlas/audit-decisions/${ledger.slug}.json`
        for (const [index, entry] of ledger.entries.entries()) {
          identityRecords.push(
            {
              namespace: 'decision-event',
      ...
      identityRecords.push(
        {
          ...
          location: `${repoPath}/entries/${
            plan.ledger.entries.length - 1
          }/event`,
        },
        {
          namespace: 'decision-entry',
      ...
      const identityDiagnostics =
        validateUniqueAuditIdentityRecords(identityRecords)
      if (identityDiagnostics.length > 0) {
        throw new Error(
          `decision portfolio global identity validation failed: ${
            identityDiagnostics.map((row) =>
              `${row.code} ${row.path} ${row.message}`
            ).join('; ')
          }`,
        )
    ...
    const rawMatches = existingBytes !== null &&
      Buffer.from(existingBytes).equals(
        Buffer.from(plan.bytes, 'utf8'),
      )
...
export function parseAuditDecisionPolicy(
  value: unknown,
  fullPolicyDigest: unknown,
): AuditDecisionPolicyV1 {
  ...
  const parseEnumSet = <T extends string>(
    candidate: unknown,
    pointer: string,
    allowed: readonly T[],
    nonempty = false,
  ): T[] => {
    const rows = dataArrayAt(candidate, pointer).map((row, index) =>
      enumAt(row, allowed, `${pointer}/${index}`)
    )
    if (nonempty && rows.length === 0) {
      invalid('empty-array', pointer, 'must be nonempty')
    }
  ...
  exactKeys(
    drift,
    ['findingBearing', 'clean', 'unknown'],
    [],
    '/securityDecisions/drift',
  )
  const driftValue = (candidate: unknown, pointer: string) =>
    enumAt(candidate, ['blocking', 'advisory'] as const, pointer)
  ...
  const severityRows = dataArrayAt(
    expiry.severityOverrides,
    '/securityDecisions/expiry/severityOverrides',
  )
  const seenSeverities = new Set<string>()
  const severityOverrides = severityRows.map((candidate, index) => {
    const pointer =
      `/securityDecisions/expiry/severityOverrides/${index}`
    ...
    for (const severity of severities) {
      if (seenSeverities.has(severity)) {
        invalid(
          'duplicate-policy-severity',
          `${pointer}/severities`,
          'severity overrides must not overlap',
        )
      }
      seenSeverities.add(severity)
    }
    return {
      severities: severities as [
        typeof severities[number],
        ...typeof severities[number][],
      ],
      maximumDays: safeIntegerAt(
        override.maximumDays,
        `${pointer}/maximumDays`,
        1,
      ),
      minimumIndependentReviews: safeIntegerAt(
        override.minimumIndependentReviews,
        `${pointer}/minimumIndependentReviews`,
      ),
      reviewEvidenceRequired: booleanAt(
        override.reviewEvidenceRequired,
        `${pointer}/reviewEvidenceRequired`,
      ),
    }
  })

  const remediation = dataRecordAt(
    policy.remediation,
  ...
  const falsePositive = dataRecordAt(
    policy.falsePositive,
    '/securityDecisions/falsePositive',
  )
  exactKeys(
    falsePositive,
    ['reviewedBlobRequired', 'sourceEvidenceRequired'],
    [],
    '/securityDecisions/falsePositive',
  )
  const superseded = dataRecordAt(
    policy.superseded,
  ...
  const retirement = dataRecordAt(
    policy.retirement,
    '/securityDecisions/retirement',
  )
  exactKeys(
    retirement,
    ['historyProofRequired', 'allowedReasons'],
    [],
    '/securityDecisions/retirement',
  )
  const parsed: AuditDecisionPolicyInputV1 = {
    requireDisposition: booleanAt(
      policy.requireDisposition,
      '/securityDecisions/requireDisposition',
    ),
    blockingActions: parseEnumSet(
      policy.blockingActions,
      '/securityDecisions/blockingActions',
      [
        'open',
        'remediated',
        'accepted-risk',
    ...
    drift: {
      findingBearing: driftValue(
        drift.findingBearing,
        '/securityDecisions/drift/findingBearing',
      ),
      clean: driftValue(drift.clean, '/securityDecisions/drift/clean'),
      unknown: driftValue(
        drift.unknown,
        '/securityDecisions/drift/unknown',
      ),
    },
    expiry: {
      warningDays: safeIntegerAt(
        expiry.warningDays,
        '/securityDecisions/expiry/warningDays',
      ),
      requiredFor: parseEnumSet(
        expiry.requiredFor,
        '/securityDecisions/expiry/requiredFor',
        ['accepted-risk', 'separate-design'] as const,
      ),
      acceptedRiskMaximumDays: safeIntegerAt(
        expiry.acceptedRiskMaximumDays,
        '/securityDecisions/expiry/acceptedRiskMaximumDays',
        1,
      ),
      separateDesignMaximumDays: safeIntegerAt(
        expiry.separateDesignMaximumDays,
        '/securityDecisions/expiry/separateDesignMaximumDays',
        1,
      ),
      falsePositiveMustBeNull: booleanAt(
        expiry.falsePositiveMustBeNull,
        '/securityDecisions/expiry/falsePositiveMustBeNull',
      ),
      severityOverrides,
    },
    remediation: {
      fixBlobRequired: booleanAt(
        remediation.fixBlobRequired,
        '/securityDecisions/remediation/fixBlobRequired',
      ),
      postFixProofRequired: booleanAt(
        remediation.postFixProofRequired,
        '/securityDecisions/remediation/postFixProofRequired',
      ),
      passingRegressionRequired: booleanAt(
        remediation.passingRegressionRequired,
        '/securityDecisions/remediation/passingRegressionRequired',
      ),
      allowedRegressionKinds: parseEnumSet(
        remediation.allowedRegressionKinds,
        '/securityDecisions/remediation/allowedRegressionKinds',
        ['test', 'guardrail', 'check'] as const,
      ),
    },
    falsePositive: {
      reviewedBlobRequired: booleanAt(
        falsePositive.reviewedBlobRequired,
        '/securityDecisions/falsePositive/reviewedBlobRequired',
      ),
      sourceEvidenceRequired: booleanAt(
        falsePositive.sourceEvidenceRequired,
        '/securityDecisions/falsePositive/sourceEvidenceRequired',
      ),
    },
    superseded: {
      replacementOrDeletionProofRequired: booleanAt(
        superseded.replacementOrDeletionProofRequired,
        '/securityDecisions/superseded/replacementOrDeletionProofRequired',
      ),
      existingPathRequiresCurrentReview: booleanAt(
        superseded.existingPathRequiresCurrentReview,
        '/securityDecisions/superseded/existingPathRequiresCurrentReview',
      ),
    },
    retirement: {
      historyProofRequired: booleanAt(
        retirement.historyProofRequired,
        '/securityDecisions/retirement/historyProofRequired',
      ),
      allowedReasons: parseEnumSet(
        retirement.allowedReasons,
        '/securityDecisions/retirement/allowedReasons',
        [
          'deleted',
          'moved',
          'superseded',
    ...
    acceptedRulesets: sortedUniqueStringsAt(
      policy.acceptedRulesets,
      '/securityDecisions/acceptedRulesets',
      (candidate, pointer) => patternAt(
        candidate,
        SOURCE_NAME_RE,
        pointer,
        'must be a canonical ruleset ID',
      ),
    ),
  }
...
interface CanonicalRowSnapshotOptions<T> {
  perRowByteLimit?: number
  cumulativeByteLimit?: number
  countTrailingNewline?: boolean
  onSnapshot?: (
    row: T,
    index: number,
...
function snapshotCanonicalRows<T>(
  value: unknown,
  subject: string,
  pointer: string,
  limit: number,
  options: CanonicalRowSnapshotOptions<T> = {},
): T[] {
  let candidates: unknown[]
  try {
    ...
    throw new Error(
      `${subject} must be a dense bounded data-only array: ${
  ...
  for (const [index, candidate] of candidates.entries()) {
    ...
    try {
      ...
      throw new Error(
        `${subject} row ${index} must be bounded data-only canonical JSON: ${
          error instanceof Error ? error.message : String(error)
        }`,
      )
    }
    ...
    if (
      options.perRowByteLimit !== undefined &&
      byteLength > options.perRowByteLimit
    ) {
      throw new Error(
        `${subject} row ${index} exceeds the ${options.perRowByteLimit}-byte limit`,
      )
    }
    if (
      options.cumulativeByteLimit !== undefined &&
      byteLength > options.cumulativeByteLimit - cumulativeBytes
    ) {
      throw new Error(
        `${subject} exceed the ${options.cumulativeByteLimit}-byte cumulative canonical-byte limit`,
      )
    }
    cumulativeBytes += byteLength
    const row = JSON.parse(canonical) as T
    rows.push(row)
...
interface DecisionLedgerResourceCandidate {
  value: unknown
  pointer: string
  canonicalByteLength?: number
}

export function validateAuditDecisionLedgerCanonicalByteBudget(
  cumulativeCanonicalBytes: number,
  ledgerCanonicalBytes: number,
): number {
  if (
    !Number.isSafeInteger(cumulativeCanonicalBytes) ||
    cumulativeCanonicalBytes < 0 ||
    !Number.isSafeInteger(ledgerCanonicalBytes) ||
    ledgerCanonicalBytes <= 0
  ) {
    throw new Error(
      'decision ledger canonical byte counts must be positive safe integers, except the cumulative count may be zero',
    )
  }
  if (ledgerCanonicalBytes > DECISION_LEDGER_BYTE_LIMIT) {
    throw new Error(
      `decision ledger canonical bytes including its trailing newline exceed the ${DECISION_LEDGER_BYTE_LIMIT}-byte per-ledger limit`,
    )
  }
  if (
    cumulativeCanonicalBytes > DECISION_PORTFOLIO_BYTE_LIMIT ||
    ledgerCanonicalBytes >
      DECISION_PORTFOLIO_BYTE_LIMIT - cumulativeCanonicalBytes
  ) {
    throw new Error(
      `decision ledger portfolio canonical bytes exceed the ${DECISION_PORTFOLIO_BYTE_LIMIT}-byte cumulative limit`,
    )
  }
  return cumulativeCanonicalBytes + ledgerCanonicalBytes
}

function preflightDecisionLedgerResources(
  candidates: readonly DecisionLedgerResourceCandidate[],
): {
  entryCount: number
  reconciliationEventCount: number
  canonicalByteCount: number
  canonicalValues: Map<string, unknown>
} {
  ...
  for (const candidate of candidates) {
    ...
    if (canonicalByteLength === undefined) {
      ...
      try {
        ...
        invalid(
          'decision-index-resource-limit',
          candidate.pointer,
          `decision ledger must be bounded data-only canonical JSON: ${
            error instanceof Error ? error.message : String(error)
          }`,
        )
      }
    ...
    if (entries.length > DECISION_INDEX_ITEM_LIMIT - entryCount) {
      invalid(
        'decision-index-resource-limit',
        entriesPointer,
        `aggregate decision ledger entries exceed the ${DECISION_INDEX_ITEM_LIMIT}-item limit`,
      )
    }
  ...
  for (const group of entryGroups) {
    for (const [entryIndex, candidate] of group.entries.entries()) {
      ...
      if (event.type !== 'finding-reconciliation') continue
      if (reconciliationEventCount >= RECONCILIATION_EVENT_LIMIT) {
        invalid(
          'decision-index-resource-limit',
          `${entryPointer}/event/type`,
          `reconciliation events exceed the ${RECONCILIATION_EVENT_LIMIT}-event limit`,
        )
      }
...
function confirmedReconciliationEdgeCount(
  event: {
    outcome: string
    confidence: string
    beforeOccurrenceIds: readonly string[]
...
export function validateAuditDecisionReconciliationEdgeBudget(
  activeConfirmedEdgeCount: number,
  beforeOccurrenceCount: number,
  afterOccurrenceCount: number,
): number {
  if (
    !Number.isSafeInteger(activeConfirmedEdgeCount) ||
    ...
    throw new Error(
      'active confirmed reconciliation edge counts must be positive safe integers, except the active count may be zero',
    )
  }
  const remaining =
    ACTIVE_CONFIRMED_EDGE_LIMIT - activeConfirmedEdgeCount
  if (
    remaining < 0 ||
    ...
    throw new Error(
      `active confirmed reconciliation edges exceed the ${ACTIVE_CONFIRMED_EDGE_LIMIT}-edge limit`,
    )
  }
...
function requireConfirmedEdgeCapacity(
  activeConfirmedEdgeCount: number,
  event: {
    outcome: string
    confidence: string
  ...
  return validateAuditDecisionReconciliationEdgeBudget(
    activeConfirmedEdgeCount,
    beforeCount,
    afterCount,
  ) - activeConfirmedEdgeCount
}

export function buildAuditDecisionIndex(
  currentLedgers: readonly AtlasSecurityCurrentLedgerV3[],
  histories: readonly AuditObservationHistoryV3[],
  decisionLedgers: readonly AuditDecisionLedgerV1[],
): AuditDecisionIndexV3 {
  const currentRows = snapshotCanonicalRows<AtlasSecurityCurrentLedgerV3>(
    currentLedgers,
    'current ledgers',
    '/currentLedgers',
    DECISION_INDEX_ITEM_LIMIT,
  )
  const historyRows = snapshotCanonicalRows<AuditObservationHistoryV3>(
    histories,
  ...
  const decisionLedgerResourceCandidates:
    DecisionLedgerResourceCandidate[] = []
  const decisionRows = snapshotCanonicalRows<AuditDecisionLedgerV1>(
    decisionLedgers,
    'decision ledgers',
    '/decisionLedgers',
    DECISION_LEDGER_COUNT_LIMIT,
    {
  ...
  preflightDecisionLedgerResources(
    decisionLedgerResourceCandidates,
  )

  const findings: AuditDecisionIndexV3['findings'] = new Map()
  const occurrences: AuditDecisionIndexV3['occurrences'] = new Map()
  const observations: AuditDecisionIndexV3['observations'] = new Map()
  const historyBySlug = new Map<string, AuditObservationHistoryV3>()
  const observationHistorySlug = new Map<string, string>()
  const receiptOwners = new Map<string, Set<string>>()
  const findingHistorySlugs = new Map<AtlasFindingId, Set<string>>()

  const observationDetails = (
    observation: AuditObservationHistoryV3['entries'][number]['observation'],
    slug: string,
  ): {
    ...
    ruleset: AuditDecisionReviewContextV3['ruleset'] | null
    ...
    if (observations.has(observation.observationId)) {
      throw new Error(
        `observation identity collision: ${observation.observationId}`,
      )
    }
    if (observations.size >= DECISION_INDEX_ITEM_LIMIT) {
      throw new Error(
        `decision index observations exceed the ${DECISION_INDEX_ITEM_LIMIT}-item limit`,
      )
    }
    const producer = observation.producer as unknown as Record<string, unknown>
    const producerKind = producerKindOf(producer?.kind)
    const producerRuleset = producer?.ruleset
    let ruleset: AuditDecisionReviewContextV3['ruleset'] | null = null
    if (
      producerRuleset &&
      typeof producerRuleset === 'object' &&
      !Array.isArray(producerRuleset)
    ) {
      const candidate = producerRuleset as Record<string, unknown>
      if (
        typeof candidate.id !== 'string' ||
        !SHA256_RE.test(String(candidate.digest))
      ) {
        throw new Error(
          `observation ${observation.observationId} has an invalid ruleset`,
        )
      }
      ruleset = {
        id: candidate.id,
        digest: candidate.digest as AuditSha256,
      }
    }
    const target = observation.target as unknown as Record<string, unknown>
    const revisionCandidate = [
      ...
      typeof candidate === 'string' && FULL_REVISION_RE.test(candidate)
    )
    const repositoryRevision =
      typeof revisionCandidate === 'string' ? revisionCandidate : null
    const scope = observation.scope as unknown as Record<string, unknown>
    const exactByPath = new Map<string, AuditBlobBindingV3>()
    const reviewedByPath = new Map<string, AuditBlobBindingV3>()
    const reviewedBindings: AuditBlobBindingV3[] = []
    if (Array.isArray(scope?.files)) {
      for (const candidate of scope.files) {
        if (!candidate || typeof candidate !== 'object' || Array.isArray(candidate)) {
          throw new Error(
            `observation ${observation.observationId} has an invalid scope receipt`,
          )
        }
        ...
        if (
          ...
          const parsedPath = repoPathAt(
            file.path,
            `/observations/${observation.observationId}/scope/files/path`,
          )
          if (exactByPath.has(parsedPath)) {
            throw new Error(
              `observation ${observation.observationId} duplicates exact path ${parsedPath}`,
            )
          }
          const binding = {
            path: parsedPath,
            blob: file.blob as AuditBlobBindingV3['blob'],
          }
          ...
          if (file.status === 'reviewed') {
            ...
            const owners = receiptOwners.get(receiptKey) ?? new Set<string>()
            owners.add(slug)
    ...
    const occurrenceIds: AtlasOccurrenceId[] = []
    if (!Array.isArray(observation.findings)) {
      throw new Error(
        `observation ${observation.observationId} findings must be an array`,
      )
    }
    for (const findingCandidate of observation.findings) {
      if (occurrenceIds.length >= COLLECTION_LIMIT) {
        throw new Error(
          `observation ${observation.observationId} occurrence membership exceeds the ${COLLECTION_LIMIT}-item limit`,
        )
      }
      const finding = findingCandidate as unknown as Record<string, unknown>
      if (
        !FINDING_ID_RE.test(String(finding.findingId)) ||
        !OCCURRENCE_ID_RE.test(String(finding.occurrenceId))
      ) {
        throw new Error(
          `observation ${observation.observationId} contains an invalid finding identity`,
        )
      }
      const findingId = finding.findingId as AtlasFindingId
      const occurrenceId = finding.occurrenceId as AtlasOccurrenceId
      ...
      const decisionLedger = slugAt(
        ...
        `/observations/${observation.observationId}/findings/decisionLedger`,
      )
      ...
      if (
        ...
        throw new Error(
          `finding ${findingId} has conflicting stable decision-ledger ownership`,
        )
      }
      if (occurrences.has(occurrenceId)) {
        throw new Error(
          `occurrence identity collision: ${occurrenceId}`,
        )
      }
      if (occurrences.size >= DECISION_INDEX_ITEM_LIMIT) {
        throw new Error(
          `decision index occurrences exceed the ${DECISION_INDEX_ITEM_LIMIT}-item limit`,
        )
      }
      if (
        priorFinding &&
        ...
        throw new Error(
          `finding ${findingId} occurrence membership exceeds the ${COLLECTION_LIMIT}-item limit`,
        )
      }
      ...
      for (const locationCandidate of finding.locations) {
        if (
          ...
          typeof (locationCandidate as Record<string, unknown>).path !== 'string'
        ) {
          throw new Error(`finding ${findingId} has an invalid location`)
        }
        locationPaths.push(repoPathAt(
          (locationCandidate as Record<string, unknown>).path,
          `/observations/${observation.observationId}/findings/locations/path`,
        ))
      }
      ...
      const bindings = [...new Set(locationPaths)]
        ...
        .filter((binding): binding is AuditBlobBindingV3 =>
          binding !== undefined
        )
      ...
      if (
        ...
        throw new Error(
          `finding ${findingId} locations do not resolve to exact reviewed path/blob bindings`,
        )
      }
      const severityCandidate =
        (finding.severity as Record<string, unknown> | undefined)?.level
      const severity = enumAt(
        severityCandidate,
        [
          'critical',
          'high',
      ...
      if (priorFinding) {
        ...
        if (findings.size >= DECISION_INDEX_ITEM_LIMIT) {
          throw new Error(
            `decision index findings exceed the ${DECISION_INDEX_ITEM_LIMIT}-item limit`,
          )
        }
  ...
  for (const historyCandidate of historyRows) {
    const history = historyCandidate as AuditObservationHistoryV3
    if (
    ...
    for (const [index, entry] of history.entries.entries()) {
      if (entry.previousEntryDigest !== previousEntryDigest) {
        throw new Error(
          `observation history ${slug} entry ${index} breaks the chain`,
        )
      }
      if (
        ...
        throw new Error(
          `observation history ${slug} entry ${index} has a mismatched observation`,
        )
      }
      ...
      if (entry.entryDigest !== expectedEntryDigest) {
        throw new Error(
          `observation history ${slug} entry ${index} has a mismatched digest`,
        )
      }
  ...
  for (const current of currentRows) {
    ...
    const index = history.entries.findIndex((entry) =>
      entry.entryDigest === current.history.entryDigest
    )
    if (index < 0) {
    ...
    if (
      current.history.observationId !== entry.observationId ||
      current.currentDigest !== entry.observationDigest ||
      canonicalJson(current.current) !== canonicalJson(entry.observation)
    ) {
      throw new Error(
        `current pointer for ${slug} differs from its history entry`,
      )
    }
    const trailing = history.entries.length - index - 1
    if (trailing > 1) {
      throw new Error(
        `current pointer for ${slug} has more than one trailing history-ahead entry`,
      )
    }
    ...
    for (const occurrenceId of indexedObservation.occurrenceIds) {
      ...
      if (finding.currentOccurrenceIds.length >= COLLECTION_LIMIT) {
        throw new Error(
          `finding ${finding.findingId} current occurrence membership exceeds the ${COLLECTION_LIMIT}-item limit`,
        )
      }
  ...
  for (const [slug, history] of historyBySlug) {
    if (!currentBySlug.has(slug) && history.entries.length !== 1) {
      throw new Error(
        `history ${slug} without current may contain only one genesis history-ahead entry`,
      )
    }
    if (!currentBySlug.has(slug)) {
      observations.get(
        history.entries[0].observationId,
      )!.publicationState = 'history-ahead'
    }
  }
  for (const finding of findings.values()) {
    const hasAnyPublishedOccurrence = finding.occurrenceIds.some(
      (occurrenceId) => {
        const occurrence = occurrences.get(occurrenceId)!
    ...
    if (
      !findingHistorySlugs.get(finding.findingId)?.has(
        finding.decisionLedger,
      ) ||
      (hasAnyPublishedOccurrence && !hasPublishedHome)
    ) {
      throw new Error(
        `finding ${finding.findingId} names a decision home with no published historical occurrence`,
      )
    }
  }
  for (const finding of findings.values()) {
  ...
  const parsedDecisionLedgers = new Map<string, AuditDecisionLedgerV1>()
  const events: AuditDecisionIndexV3['events'] = new Map()
  const retirementEvents: AuditDecisionIndexV3['retirementEvents'] = []
  const reconciliationEvents: AuditDecisionIndexV3['reconciliationEvents'] = []
  const aliasEvents: AuditDecisionIndexV3['aliasEvents'] = []
  const aliasOwners = new Map<string, AtlasFindingId>()
  const activeStaged = new Map<string, string>()
  const terminalRetirements = new Map<string, string>()
  const activeFindingClosures = new Map<AtlasFindingId, string>()
  const activeReconciliations = new Map<
    string,
    AuditDecisionIndexV3['reconciliationEvents'][number]
  >()
  const comparisonEndpointOwners = new Map<
    string,
    Map<AtlasOccurrenceId, string>
  >()
  const confirmedBeforeOwners = new Map<AtlasOccurrenceId, string>()
  const confirmedAfterOwners = new Map<AtlasOccurrenceId, string>()
  const confirmedAdjacency = new Map<
    AtlasOccurrenceId,
    Map<AtlasOccurrenceId, number>
  >()
  let activeConfirmedEdgeCount = 0
  const isConfirmedReconciliation = (
    event: AuditDecisionIndexV3['reconciliationEvents'][number],
  ): boolean =>
    event.outcome === 'equivalent' && event.confidence === 'high'
  const removeActiveReconciliation = (
    event: AuditDecisionIndexV3['reconciliationEvents'][number],
  ): void => {
    ...
    if (!isConfirmedReconciliation(event)) return
    ...
    for (const occurrenceId of event.beforeOccurrenceIds) {
      ...
      if (targets.size === 0) confirmedAdjacency.delete(occurrenceId)
    }
  ...
  const addActiveReconciliation = (
    event: AuditDecisionIndexV3['reconciliationEvents'][number],
  ): void => {
    const confirmedEdgeCount = requireConfirmedEdgeCapacity(
      activeConfirmedEdgeCount,
      event,
    )
    const comparisonOwners =
      comparisonEndpointOwners.get(event.comparisonId) ??
        new Map<AtlasOccurrenceId, string>()
    for (const occurrenceId of [
      ...event.beforeOccurrenceIds,
      ...event.afterOccurrenceIds,
    ]) {
    ...
    if (isConfirmedReconciliation(event)) {
      for (const occurrenceId of event.beforeOccurrenceIds) {
        ...
        const targets = confirmedAdjacency.get(occurrenceId) ??
          new Map<AtlasOccurrenceId, number>()
        for (const afterOccurrenceId of event.afterOccurrenceIds) {
          targets.set(
            afterOccurrenceId,
            (targets.get(afterOccurrenceId) ?? 0) + 1,
          )
        }
  ...
  const reviewedBindingKeysByObservation = new Map<string, Set<string>>(
    [...observations].map(([observationId, observation]) => [
      observationId,
      new Set(observation.reviewedBindings.map((binding) =>
        `${binding.path}\0${binding.blob}`
      )),
    ]),
  )
  const requireBindingsInObservation = (
    observationId: string,
    bindings: readonly AuditBlobBindingV3[],
    subject: string,
  ): void => {
    ...
    if (bindings.some((binding) => !known.has(`${binding.path}\0${binding.blob}`))) {
      throw new Error(
        `${subject} contains an unknown or mismatched path/blob binding`,
      )
    }
  }
  const requireDispositionContext = (
    event: Extract<AuditDecisionEventV3, { type: 'finding-disposition' }>,
    ledgerSlug: string,
  ): void => {
    ...
    if (!occurrence || occurrence.findingId !== event.findingId) {
      throw new Error(
        `decision ${event.eventId} references an unknown or mismatched occurrence`,
      )
    }
    if (
      occurrence.decisionLedger !== ledgerSlug ||
      findings.get(event.findingId)?.decisionLedger !== ledgerSlug
    ) {
      throw new Error(
        `decision ${event.eventId} is not stored in the finding's stable ledger`,
      )
    }
    ...
    if (contextKind === 'invalid') {
      ...
      const mismatches = exactContextMismatchReasons(
        event,
        occurrence,
        observations,
      )
      throw new Error(
        occurrence.closureEligible && occurrence.ruleset !== null
          ? mismatches.length > 0
    ...
    const ruleset = occurrence.ruleset as AuditRulesetReceiptV3
    const requireMigrationProofObservation = (
      observationId: AuditDecisionReviewContextV3['observationId'],
      slug: string | null = occurrence.decisionLedger,
    ): AuditIndexedObservationV3 => {
      const observation = migrationContextObservation(
        observations,
        observationId,
        ruleset,
        slug,
      ...
      if (observation === null) {
        throw new Error(
          `decision ${event.eventId} references an observation outside the deterministic migration validation context`,
        )
      }
      return observation
    }
    const requireMigrationProofBindings = (
      bindings: readonly AuditBlobBindingV3[],
    ): void => {
      if (!bindingPathsWithinLocations(bindings, occurrence.locationPaths)) {
        throw new Error(
          `decision ${event.eventId} references bindings outside the governed occurrence locations`,
        )
      }
    }
    for (const proof of event.proofs) {
      if (proof.kind === 'current-review' || proof.kind === 'source-evidence') {
        if (contextKind === 'migration') {
          ...
          requireBindingsInObservation(
            proof.observationId,
            proof.reviewedBindings,
            `decision proof ${event.eventId}`,
          )
        }
        ...
        if (
          afterObservation.occurrenceIds.some((occurrenceId) =>
            occurrences.get(occurrenceId)?.findingId === event.findingId
          )
        ) {
          throw new Error(
            `decision ${event.eventId} post-fix observation still contains the finding`,
          )
        }
        if (
          afterObservation.repositoryRevision === null ||
          afterObservation.repositoryRevision !== proof.fixRevision
        ) {
          throw new Error(
            `decision ${event.eventId} fixRevision does not match the post-fix target revision`,
          )
        }
        ...
        if (contextKind === 'migration') {
          if (
            ...
            throw new Error(
              `decision ${event.eventId} has mismatched replacement references`,
            )
          }
          requireMigrationProofObservation(proof.observationId)
        ...
        if (contextKind === 'migration') {
          ...
          const knownBinding = proof.deletedBindings.every((binding) =>
            receiptOwners.has(`${binding.path}\0${binding.blob}`)
          )
          if (!knownBinding) {
            throw new Error(
              `decision ${event.eventId} deletion proof references unknown path/blob history`,
            )
          }
        }
      } else {
        ...
        if (
          observation.repositoryRevision === null ||
          observation.repositoryRevision !== proof.searchRevision
        ) {
          throw new Error(
            `decision ${event.eventId} no-replacement revision is mismatched`,
          )
        }
      }
    }
  ...
  for (const ledgerCandidate of [...decisionRows].sort((left, right) =>
    utf16Compare(left.slug, right.slug)
  )) {
    ...
    for (const [chainIndex, entry] of ledger.entries.entries()) {
      ...
      if (event.type === 'finding-disposition') {
        ...
        if (event.supersedesEventId !== undefined) {
          ...
          if (
            ...
            throw new Error(
              `decision ${event.eventId} supersedes an invalid or non-earlier closure`,
            )
          }
          if (
            ...
            throw new Error(
              `decision ${event.eventId} must supersede the current active closure exactly once`,
            )
          }
      ...
      if (event.type === 'scope-retirement') {
        if (
          ...
          requireBindingsInObservation(
            proof.observationId,
            proof.reviewedBindings,
            `retirement no-replacement proof ${event.eventId}`,
          )
          const searchedObservation = requireObservation(
            proof.observationId,
            `retirement no-replacement proof ${event.eventId}`,
          )
          if (
            searchedObservation.repositoryRevision === null ||
            searchedObservation.repositoryRevision !== proof.searchRevision ||
            proof.searchRevision !== event.revisionProof.repositoryRevision
          ) {
            throw new Error(
              `retirement ${event.eventId} no-replacement search revision is mismatched`,
            )
          }
        }
        ...
        if (
          ...
          throw new Error(
            `retirement ${event.eventId} does not have unique history receipt ownership`,
          )
        }
        requireBindingsInObservation(
          event.historyProof.observationId,
          [{ path: event.path, blob: event.blob }],
          `retirement ${event.eventId}`,
        )
        if (event.reason === 'staged-deletion') {
          if (
            activeStaged.has(receiptKey) ||
            terminalRetirements.has(receiptKey)
          ) {
            throw new Error(
              `retirement ${event.eventId} duplicates an active staged or terminal retirement`,
            )
          }
          ...
          if (terminalRetirements.has(receiptKey)) {
            throw new Error(
              `retirement ${event.eventId} duplicates a terminal retirement`,
            )
          }
          ...
          if (
            ...
            throw new Error(
              `deleted retirement ${event.eventId} must supersede exactly the active staged event`,
            )
          }
          ...
          if (
            ...
            throw new Error(
              `retirement ${event.eventId} conflicts with an existing staged or terminal retirement`,
            )
          }
          terminalRetirements.set(receiptKey, event.eventId)
        }
      ...
      if (event.type === 'identity-alias-reconciliation') {
        ...
        if (!finding || finding.decisionLedger !== ledger.slug) {
          throw new Error(
            `alias event ${event.eventId} references an unknown finding home`,
          )
        }
        for (const occurrenceId of event.occurrenceIds) {
          if (occurrences.get(occurrenceId)?.findingId !== event.findingId) {
            throw new Error(
              `alias event ${event.eventId} references a mismatched occurrence`,
            )
          }
        }
      ...
      const expectedHome = endpointOccurrences
        ...
        .sort(utf16Compare)[0]
      if (
        event.decisionLedger !== expectedHome ||
        ledger.slug !== expectedHome
      ) {
        throw new Error(
          `reconciliation ${event.eventId} is not stored at its deterministic global home`,
        )
      }
      const beforeObservationIds = [...new Set(
        event.beforeOccurrenceIds.map((occurrenceId) =>
          occurrences.get(occurrenceId)!.observationId
        ),
      )].sort(utf16Compare)
      const afterObservationIds = [...new Set(
        event.afterOccurrenceIds.map((occurrenceId) =>
          occurrences.get(occurrenceId)!.observationId
        ),
      )].sort(utf16Compare)
      if (
        ...
        throw new Error(
          `reconciliation ${event.eventId} has a mismatched comparison boundary`,
        )
      }
      if (event.supersedesEventId !== undefined) {
        const prior = events.get(event.supersedesEventId)
        if (
          ...
          throw new Error(
            `reconciliation ${event.eventId} correction does not supersede an earlier event in the same comparison`,
          )
        }
        const superseded = activeReconciliations.get(event.supersedesEventId)
        if (!superseded) {
          throw new Error(
            `reconciliation ${event.eventId} correction must supersede the current active event`,
          )
        }
      ...
      const comparisonOwners = comparisonEndpointOwners.get(
        event.comparisonId,
      )
      if ([...candidateEndpoints].some((occurrenceId) =>
        comparisonOwners?.has(occurrenceId)
      )) {
        throw new Error(
          `reconciliation ${event.eventId} overlaps an active group in the same comparison`,
        )
      }
      ...
      if (event.outcome === 'equivalent' && event.confidence === 'high') {
        const bindingSet = (
          ...
          if (
            ...
            throw new Error(
              `reconciliation ${event.eventId} requires exact reviewed bindings for every endpoint`,
            )
          }
          return new Set(occurrence.bindings.map((binding) =>
            `${binding.path}\0${binding.blob}`
          ))
        }
        const sideUnion = (
          occurrenceIds: readonly AtlasOccurrenceId[],
          side: 'before' | 'after',
        ): Set<string> => {
          ...
          for (const occurrenceId of occurrenceIds) {
            ...
            for (const binding of bindings) {
              if (occurrenceIds.length > 1 && union.has(binding)) {
                throw new Error(
                  `reconciliation ${event.eventId} ${side} bindings overlap instead of forming a partition`,
                )
              }
              union.add(binding)
            }
        ...
        const beforeUnion = sideUnion(
          event.beforeOccurrenceIds,
          'before',
        )
        const afterUnion = sideUnion(event.afterOccurrenceIds, 'after')
        if (
          beforeUnion.size !== afterUnion.size ||
          [...beforeUnion].some((binding) => !afterUnion.has(binding))
        ) {
          throw new Error(
            `reconciliation ${event.eventId} binding partition/union does not match across the comparison`,
          )
        }

        if (event.beforeOccurrenceIds.some((occurrenceId) =>
          confirmedBeforeOwners.has(occurrenceId)
        ) || event.afterOccurrenceIds.some((occurrenceId) =>
          confirmedAfterOwners.has(occurrenceId)
        )) {
          throw new Error(
            `reconciliation ${event.eventId} conflicts with another active confirmed group`,
          )
        }

        const targetOccurrenceIds = new Set(event.beforeOccurrenceIds)
        const reachable = [...event.afterOccurrenceIds]
        const visited = new Set<AtlasOccurrenceId>()
        ...
        while (reachable.length > 0 && !createsCycle) {
          ...
          for (const target of
            confirmedAdjacency.get(occurrenceId)?.keys() ?? []) {
            if (!visited.has(target)) reachable.push(target)
          }
        }
        if (createsCycle) {
          throw new Error(
            `reconciliation ${event.eventId} creates a cyclic equivalence graph`,
          )
        }
      }
...
interface SnapshotDecisionMapOptions {
  mapLimit?: number
  preflightValue?: (value: unknown, pointer: string) => void
  afterPreflight?: () => void
}

function snapshotDecisionMap<K, V>(
  value: unknown,
  pointer: string,
  parseKey: (value: unknown, pointer: string) => K,
  parseValue: (value: unknown, pointer: string) => V,
  options: SnapshotDecisionMapOptions = {},
): Map<K, V> {
  ...
  if (
    ...
    invalid(
      ...
      pointer,
      'must be a plain Map without own properties',
    )
  }
  ...
  if (
    ...
    invalid(
      'invalid-decision-index',
      pointer,
      `Map exceeds the ${mapLimit}-item limit`,
    )
  }
  const entries: Array<{
    key: K
  ...
  let index = 0
  for (const entry of Map.prototype.entries.call(value) as
  ...
  if (index !== size) {
    invalid(
      'invalid-decision-index',
      pointer,
      'Map size changed while it was being snapshotted',
    )
  }
  ...
  for (const entry of entries) {
    snapshot.set(
      entry.key,
      parseValue(entry.value, entry.pointer),
    )
  }
  return snapshot
}

function parseIndexedRuleset(
  value: unknown,
  pointer: string,
): AuditDecisionReviewContextV3['ruleset'] {
...
function snapshotAuditDecisionPolicy(
  value: unknown,
): AuditDecisionPolicyV1 {
  ...
  exactKeys(
    policy,
    [...inputKeys, 'policyDigest'],
    [],
    pointer,
  )
  const input: Record<string, unknown> = {}
  for (const key of inputKeys) input[key] = policy[key]
  return parseAuditDecisionPolicy(
    input,
    sha256At(policy.policyDigest, `${pointer}/policyDigest`),
  )
}

function snapshotAuditDecisionIndex(
  value: unknown,
): AuditDecisionIndexV3 {
  ...
  const parseFindingId = (candidate: unknown, pointer: string) =>
    patternAt(
      candidate,
      FINDING_ID_RE,
      pointer,
  ...
  const parseOccurrenceId = (candidate: unknown, pointer: string) =>
    patternAt(
      candidate,
      OCCURRENCE_ID_RE,
      pointer,
  ...
  const parseObservationId = (candidate: unknown, pointer: string) =>
    patternAt(
      candidate,
      OBSERVATION_ID_RE,
      pointer,
  ...
  const parseNullableRuleset = (
    candidate: unknown,
    pointer: string,
  ) => candidate === null ? null : parseIndexedRuleset(candidate, pointer)
  const parseNullableRevision = (
    candidate: unknown,
    pointer: string,
  ) => candidate === null ? null : revisionAt(candidate, pointer)

  const findings = snapshotDecisionMap(
    index.findings,
    '/index/findings',
    parseFindingId,
    (candidate, pointer) => {
      ...
      return {
        ...
        currentOccurrenceIds: sortedUniqueStringsAt(
          finding.currentOccurrenceIds,
          `${pointer}/currentOccurrenceIds`,
          parseOccurrenceId,
        ),
      }
    },
  )
  const occurrences = snapshotDecisionMap(
    index.occurrences,
    '/index/occurrences',
    parseOccurrenceId,
    (candidate, pointer) => {
      ...
      return {
        occurrenceId: parseOccurrenceId(
        ...
        observationId: parseObservationId(
          occurrence.observationId,
          `${pointer}/observationId`,
        ),
        decisionLedger: slugAt(
        ...
        reviewedBindings: parseBindings(
          occurrence.reviewedBindings,
          `${pointer}/reviewedBindings`,
          false,
        ),
        locationPaths: sortedUniqueStringsAt(
          occurrence.locationPaths,
          `${pointer}/locationPaths`,
          repoPathAt,
          true,
        ),
        closureEligible: booleanAt(
          occurrence.closureEligible,
          `${pointer}/closureEligible`,
        ),
        ruleset: parseNullableRuleset(
          occurrence.ruleset,
          `${pointer}/ruleset`,
        ),
        repositoryRevision: parseNullableRevision(
          occurrence.repositoryRevision,
          `${pointer}/repositoryRevision`,
        ),
        severity: enumAt(
          occurrence.severity,
  ...
  const observations = snapshotDecisionMap(
    index.observations,
    '/index/observations',
    parseObservationId,
    (candidate, pointer) => {
      ...
      return {
        ...
        historyIndex: safeIntegerAt(
          observation.historyIndex,
          `${pointer}/historyIndex`,
        ),
        occurrenceIds: sortedUniqueStringsAt(
          observation.occurrenceIds,
          `${pointer}/occurrenceIds`,
          parseOccurrenceId,
        ),
        inventoryBindings: parseBindings(
          observation.inventoryBindings,
          `${pointer}/inventoryBindings`,
          false,
        ),
        reviewedBindings: parseBindings(
        ...
        producerKind: observation.producerKind === null
          ...
          : enumAt(
              ...
              AUDIT_PRODUCER_KINDS,
              `${pointer}/producerKind`,
            ),
        ruleset: parseNullableRuleset(
          observation.ruleset,
          `${pointer}/ruleset`,
        ...
        repositoryRevision: parseNullableRevision(
          observation.repositoryRevision,
          `${pointer}/repositoryRevision`,
        ),
        authoritative: booleanAt(
        ...
        publicationState: enumAt(
          observation.publicationState,
          ['historical', 'current', 'history-ahead'] as const,
  ...
  const decisionLedgerResourceCandidates:
    DecisionLedgerResourceCandidate[] = []
  let canonicalDecisionLedgerValues = new Map<string, unknown>()
  const decisionLedgers = snapshotDecisionMap(
    index.decisionLedgers,
    '/index/decisionLedgers',
    (candidate, pointer) => slugAt(candidate, pointer),
    (_candidate, pointer) => parseDecisionLedger(
      canonicalDecisionLedgerValues.get(pointer),
      pointer,
    ),
    {
  ...
  const events = snapshotDecisionMap(
    index.events,
    '/index/events',
    (candidate, pointer) => patternAt(
      candidate,
    ...
    (candidate, pointer) => {
      ...
      exactKeys(
        row,
        ['decisionLedger', 'chainIndex', 'eventDigest', 'event'],
        [],
        pointer,
      )
      return {
        ...
        chainIndex: safeIntegerAt(
          row.chainIndex,
          `${pointer}/chainIndex`,
        ),
        eventDigest: sha256At(
          row.eventDigest,
          `${pointer}/eventDigest`,
        ),
  ...
  const parseEventArray = <
    T extends AuditDecisionEventV3['type'],
  >(
    candidate: unknown,
    pointer: string,
    type: T,
    limit = DECISION_INDEX_ITEM_LIMIT,
  ): Array<Extract<AuditDecisionEventV3, { type: T }>> => {
    let rows: unknown[]
    try {
      ...
      if (type === 'finding-reconciliation') {
        invalid(
          'decision-index-resource-limit',
          pointer,
          `reconciliation events exceed the ${limit}-event limit or are not a valid bounded array`,
        )
      }
      throw error
    }
    return rows.map(
      (event, eventIndex) => {
        const parsed = parseStoredDecisionEvent(
          event,
          `${pointer}/${eventIndex}`,
        )
        if (parsed.type !== type) {
          invalid(
            'invalid-decision-index',
            `${pointer}/${eventIndex}/type`,
            `must equal ${type}`,
          )
        }
        return parsed as Extract<AuditDecisionEventV3, { type: T }>
      },
    )
  }
  const retirementEvents = parseEventArray(
    index.retirementEvents,
    '/index/retirementEvents',
    'scope-retirement',
  )
  const reconciliationEvents = parseEventArray(
    index.reconciliationEvents,
    '/index/reconciliationEvents',
    'finding-reconciliation',
    RECONCILIATION_EVENT_LIMIT,
  )
  const aliasEvents = parseEventArray(
    index.aliasEvents,
    '/index/aliasEvents',
    'identity-alias-reconciliation',
  )
  ...
  const bindingKey = (binding: AuditBlobBindingV3) =>
    `${binding.path}\0${binding.blob}`
  for (const [findingId, finding] of findings) {
    if (finding.findingId !== findingId) {
      invalid(
        'decision-index-identity-mismatch',
        '/index/findings',
        'finding Map keys must equal findingId values',
      )
    }
    if (finding.occurrenceIds.length === 0) {
      invalid(
        'decision-index-reference-mismatch',
        `/index/findings/${findingId}/occurrenceIds`,
        'finding occurrence membership must be nonempty',
      )
    }
  }
  const observationsBySlug = new Map<
    string,
    Array<typeof observations extends Map<unknown, infer V> ? V : never>
  >()
  const inventoryBindingKeysByObservation =
    new Map<string, Set<string>>()
  const reviewedBindingKeysByObservation =
    new Map<string, Set<string>>()
  for (const [observationId, observation] of observations) {
    if (observation.observationId !== observationId) {
      invalid(
        'decision-index-identity-mismatch',
        '/index/observations',
        'observation Map keys must equal observationId values',
      )
    }
    if (
      observation.authoritative !==
      ...
      invalid(
        'decision-index-reference-mismatch',
        `/index/observations/${observationId}`,
        'observation authority must match current publication state',
      )
    }
    ...
    if (
      inventoryPaths.size !== observation.inventoryBindings.length ||
      reviewedPaths.size !== observation.reviewedBindings.length ||
      observation.reviewedBindings.some(
        (binding) => !inventoryKeys.has(bindingKey(binding)),
      )
    ) {
      invalid(
        'decision-index-reference-mismatch',
        `/index/observations/${observationId}`,
        'observation inventory and reviewed bindings require unique paths and an exact reviewed subset',
      )
    }
  ...
  for (const [slug, slugRows] of observationsBySlug) {
    slugRows.sort((left, right) =>
      left.historyIndex - right.historyIndex
    )
    if (slugRows.some((row, index) => row.historyIndex !== index)) {
      invalid(
        'decision-index-reference-mismatch',
        `/index/observations/${slug}`,
        'observation history indices must be unique and contiguous from zero',
      )
    }
    const currentRows = slugRows.filter(
      (row) => row.publicationState === 'current',
    )
    if (currentRows.length === 0) {
      if (
        ...
        invalid(
          'decision-index-reference-mismatch',
          `/index/observations/${slug}`,
          'a slug without current must contain exactly one index-zero history-ahead observation',
        )
      }
      continue
    }
    if (currentRows.length !== 1) {
      invalid(
        'decision-index-reference-mismatch',
        `/index/observations/${slug}`,
        'a slug must contain exactly one current observation',
      )
    }
    ...
    if (
      ...
      invalid(
        'decision-index-reference-mismatch',
        `/index/observations/${slug}`,
        'observation publication topology must be historical, one current, and at most one immediate trailing history-ahead row',
      )
    }
  }
  const occurrenceIdsByFinding = new Map<AtlasFindingId, AtlasOccurrenceId[]>()
  const currentOccurrenceIdsByFinding =
    new Map<AtlasFindingId, AtlasOccurrenceId[]>()
  const occurrenceIdsByObservation =
    new Map<AuditDecisionReviewContextV3['observationId'], AtlasOccurrenceId[]>()
  for (const [occurrenceId, occurrence] of occurrences) {
    if (occurrence.occurrenceId !== occurrenceId) {
      invalid(
        'decision-index-identity-mismatch',
        '/index/occurrences',
        'occurrence Map keys must equal occurrenceId values',
      )
    }
    const finding = findings.get(occurrence.findingId)
    const observation = observations.get(occurrence.observationId)
    if (
      ...
      invalid(
        'decision-index-reference-mismatch',
        `/index/occurrences/${occurrenceId}`,
        'occurrence must reference matching indexed finding and observation facts',
      )
    }
    if (
      occurrence.authoritative !== observation.authoritative ||
      ...
      invalid(
        'decision-index-reference-mismatch',
        `/index/occurrences/${occurrenceId}`,
        'occurrence authority, revision, and ruleset must match its observation',
      )
    }
    const inventory =
      inventoryBindingKeysByObservation.get(occurrence.observationId)!
    const reviewed =
      reviewedBindingKeysByObservation.get(occurrence.observationId)!
    const occurrenceBindings = new Set(
      occurrence.bindings.map(bindingKey),
    )
    const occurrenceLocationPaths = new Set(occurrence.locationPaths)
    if (
      occurrence.bindings.some((binding) =>
        !inventory.has(bindingKey(binding)) ||
        !occurrenceLocationPaths.has(binding.path)
      ) ||
      occurrence.reviewedBindings.some(
        (binding) =>
          !reviewed.has(bindingKey(binding)) ||
      ...
      invalid(
        'decision-index-reference-mismatch',
        `/index/occurrences/${occurrenceId}`,
        'occurrence bindings and closure eligibility must match observation facts',
      )
    }
    const findingOccurrences =
      occurrenceIdsByFinding.get(occurrence.findingId) ?? []
    ...
    if (occurrence.authoritative) {
      const current =
        currentOccurrenceIdsByFinding.get(occurrence.findingId) ?? []
      current.push(occurrenceId)
      currentOccurrenceIdsByFinding.set(occurrence.findingId, current)
    }
    const observationOccurrences =
      occurrenceIdsByObservation.get(occurrence.observationId) ?? []
    observationOccurrences.push(occurrenceId)
    occurrenceIdsByObservation.set(
      occurrence.observationId,
  ...
  for (const [findingId, finding] of findings) {
    if (
      !sameIds(
        finding.occurrenceIds,
        occurrenceIdsByFinding.get(findingId) ?? [],
      ) ||
      !sameIds(
        finding.currentOccurrenceIds,
        currentOccurrenceIdsByFinding.get(findingId) ?? [],
      )
    ) {
      invalid(
        'decision-index-reference-mismatch',
        `/index/findings/${findingId}`,
        'finding occurrence memberships must match the occurrence index',
      )
    }
    const findingObservations = finding.occurrenceIds.map((occurrenceId) =>
      observations.get(occurrences.get(occurrenceId)!.observationId)!
    )
    const hasHomeOccurrence = findingObservations.some(
      (observation) => observation.slug === finding.decisionLedger,
    )
    const hasPublishedOccurrence = findingObservations.some(
      (observation) => observation.publicationState !== 'history-ahead',
    )
    const hasPublishedHomeOccurrence = findingObservations.some(
      (observation) =>
        observation.slug === finding.decisionLedger &&
        observation.publicationState !== 'history-ahead',
    )
    if (
      !hasHomeOccurrence ||
      ...
      invalid(
        'decision-index-reference-mismatch',
        `/index/findings/${findingId}/decisionLedger`,
        'finding decision home must exist among its occurrences and contain a published occurrence whenever the finding is published',
      )
    }
  }
  for (const [observationId, observation] of observations) {
    if (!sameIds(
      observation.occurrenceIds,
      occurrenceIdsByObservation.get(observationId) ?? [],
    )) {
      invalid(
        'decision-index-reference-mismatch',
        `/index/observations/${observationId}`,
        'observation occurrence memberships must match the occurrence index',
      )
    }
  }

  const sortedDecisionLedgers = [...decisionLedgers.values()].sort(
    (left, right) => utf16Compare(left.slug, right.slug),
  )
  const eventIdentityRecords: AuditIdentityRecord[] = []
  const entryIdentityRecords: AuditIdentityRecord[] = []
  for (const ledger of sortedDecisionLedgers) {
    if (decisionLedgers.get(ledger.slug) !== ledger) {
      invalid(
        'decision-index-identity-mismatch',
        '/index/decisionLedgers',
        'decision ledger Map keys must equal ledger slugs',
      )
    }
    for (const [chainIndex, entry] of ledger.entries.entries()) {
      const entryPointer =
        `/index/decisionLedgers/${ledger.slug}/${chainIndex}`
  ...
  const expectedEvents = new Map<string, {
    decisionLedger: string
    chainIndex: number
    eventDigest: AuditSha256
    event: AuditDecisionEventV3
  }>()
  const expectedRetirements: AuditDecisionIndexV3['retirementEvents'] = []
  const expectedReconciliations:
    AuditDecisionIndexV3['reconciliationEvents'] = []
  const expectedAliases: AuditDecisionIndexV3['aliasEvents'] = []
  for (const ledger of sortedDecisionLedgers) {
    for (const [chainIndex, entry] of ledger.entries.entries()) {
      ...
      if (entry.event.type === 'scope-retirement') {
        ...
        invalid(
          'invalid-decision-index',
          `/index/decisionLedgers/${ledger.slug}/${chainIndex}/event/type`,
          'decision event type is not supported',
        )
      }
    }
  }
  if (events.size !== expectedEvents.size) {
    invalid(
      'decision-index-event-view-mismatch',
      '/index/events',
      'event Map must exactly match parsed decision ledgers',
    )
  }
  for (const [eventId, expected] of expectedEvents) {
    ...
    if (
      ...
      invalid(
        ...
        'event Map row must exactly match its parsed ledger entry',
      )
    }
  }
  const requireMatchingEventArray = (
    actual: readonly AuditDecisionEventV3[],
    expected: readonly AuditDecisionEventV3[],
    pointer: string,
  ): void => {
    if (
      actual.length !== expected.length ||
      actual.some((event, eventIndex) =>
        canonicalJson(event) !== canonicalJson(expected[eventIndex])
      )
    ) {
      invalid(
        'decision-index-event-view-mismatch',
        pointer,
        'redundant event array must exactly match parsed decision ledgers',
      )
    }
  }
  requireMatchingEventArray(
    retirementEvents,
    expectedRetirements,
    '/index/retirementEvents',
  )
  ...
  for (const observation of observations.values()) {
    for (const binding of observation.reviewedBindings) {
      ...
      const owners = receiptOwners.get(key) ?? new Set<string>()
      owners.add(observation.slug)
      receiptOwners.set(key, owners)
    }
  }
  const requireObservationBindings = (
    observationId: AuditDecisionReviewContextV3['observationId'],
    bindings: readonly AuditBlobBindingV3[],
    subject: string,
  ) => {
  ...
  const activeFindingClosures = new Map<AtlasFindingId, string>()
  const activeStagedRetirements = new Map<string, string>()
  const terminalRetirements = new Map<string, string>()
  const aliasOwners = new Map<string, AtlasFindingId>()
  const activeReconciliations = new Map<
    string,
  ...
  for (const [eventId, row] of [...events.entries()].sort(
    ([, left], [, right]) =>
      utf16Compare(left.decisionLedger, right.decisionLedger) ||
    ...
    if (row.event.eventId !== eventId) {
      invalid(
        'decision-index-identity-mismatch',
        `/index/events/${eventId}`,
        'event Map keys must equal eventId values',
      )
    }
    ...
    if (event.type === 'finding-disposition') {
      ...
      if (
        ...
        invalid(
          'decision-index-reference-mismatch',
          `/index/events/${eventId}`,
          'finding decision references a foreign occurrence or ledger',
        )
      }
      const contextKind = dispositionContextKind(event, occurrence, observations)
      if (contextKind === 'invalid') {
        throw new Error(
          `decision ${eventId} has foreign review context or cannot govern a semantic-only occurrence`,
        )
      }
      const migrationRuleset = occurrence.ruleset as AuditRulesetReceiptV3
      const requireMigrationObservation = (
        observationId: AuditDecisionReviewContextV3['observationId'],
        slug: string | null = occurrence.decisionLedger,
      ): AuditIndexedObservationV3 => {
        const observation = migrationContextObservation(
        ...
        if (observation === null) {
          throw new Error(
            `decision ${eventId} references an observation outside the deterministic migration validation context`,
          )
        }
        return observation
      }
      const requireMigrationBindings = (
        bindings: readonly AuditBlobBindingV3[],
      ): void => {
        if (!bindingPathsWithinLocations(bindings, occurrence.locationPaths)) {
          throw new Error(
            `decision ${eventId} references bindings outside the governed occurrence locations`,
          )
        }
      }
      for (const proof of event.proofs) {
        switch (proof.kind) {
          ...
          case 'source-evidence':
            if (contextKind === 'migration') {
              ...
              requireObservationBindings(
                proof.observationId,
                proof.reviewedBindings,
                `decision proof ${eventId}`,
              )
            }
            break
          case 'post-fix': {
            ...
            if (
              ...
              throw new Error(
                `decision ${eventId} has foreign post-fix observation facts`,
              )
            }
            break
          }
          case 'replacement': {
            const replacement = occurrences.get(
              proof.replacementOccurrenceId,
            )
            if (contextKind === 'migration') {
              if (
                ...
                throw new Error(
                  `decision ${eventId} has foreign replacement facts`,
                )
              }
              requireMigrationObservation(proof.observationId)
          ...
          case 'deletion':
            ...
            } else if (proof.deletedBindings.some(
              (binding) => !receiptOwners.has(bindingKey(binding)),
            )) {
              throw new Error(
                `decision ${eventId} has foreign deletion bindings`,
              )
            }
            break
          case 'no-replacement': {
            ...
            if (searched.repositoryRevision !== proof.searchRevision) {
              throw new Error(
                `decision ${eventId} has foreign no-replacement revision`,
              )
            }
            break
          }
          default:
            throw new Error(
              `decision ${eventId} contains an unsupported proof kind`,
            )
        }
      }
      if (event.supersedesEventId !== undefined) {
        ...
        if (
          ...
          throw new Error(
            `decision ${eventId} supersedes an invalid or inactive closure`,
          )
        }
        activeFindingClosures.delete(event.findingId)
      } else if (event.action === 'open') {
      ...
      if (endpointOccurrences.some((occurrence) => occurrence === undefined)) {
        invalid(
          'decision-index-reference-mismatch',
          `/index/events/${eventId}`,
          'reconciliation references a foreign occurrence',
        )
      }
      const expectedHome = endpointOccurrences
        .map((occurrence) => occurrence!.decisionLedger)
      ...
      if (
        expectedHome !== row.decisionLedger ||
        ...
        invalid(
          'decision-index-reference-mismatch',
          `/index/events/${eventId}`,
          'reconciliation home and comparison must match endpoint facts',
        )
      }
      if (event.supersedesEventId !== undefined) {
        const prior = events.get(event.supersedesEventId)
        const superseded = activeReconciliations.get(
          event.supersedesEventId,
        )
        if (
          !prior ||
          ...
          throw new Error(
            `reconciliation ${eventId} correction must supersede the current active event in the same comparison`,
          )
        }
        removeActiveReconciliation(superseded)
      }
      const endpointOwners = comparisonEndpointOwners.get(event.comparisonId)
      if (
        [...event.beforeOccurrenceIds, ...event.afterOccurrenceIds].some(
          (occurrenceId) => endpointOwners?.has(occurrenceId),
        )
      ) {
        throw new Error(
          `reconciliation ${eventId} overlaps an active group in the same comparison`,
        )
      }
      requireConfirmedEdgeCapacity(activeConfirmedEdgeCount, event)
      if (isConfirmed(event)) {
        const sideUnion = (
          ...
          for (const occurrenceId of occurrenceIds) {
            ...
            if (
              ...
              throw new Error(
                `reconciliation ${eventId} requires exact reviewed bindings`,
              )
            }
            for (const binding of occurrence.bindings) {
              ...
              if (occurrenceIds.length > 1 && union.has(key)) {
                throw new Error(
                  `reconciliation ${eventId} ${side} bindings overlap`,
                )
              }
              union.add(key)
            }
        ...
        if (
          ...
          throw new Error(
            `reconciliation ${eventId} binding partition/union is mismatched`,
          )
        }
        if (
          event.beforeOccurrenceIds.some((occurrenceId) =>
            confirmedBeforeOwners.has(occurrenceId)
          ) ||
          event.afterOccurrenceIds.some((occurrenceId) =>
            confirmedAfterOwners.has(occurrenceId)
          ...
          throw new Error(
            `reconciliation ${eventId} conflicts with another active confirmed owner`,
          )
        }
        const targetOccurrenceIds = new Set(event.beforeOccurrenceIds)
        const reachable = [...event.afterOccurrenceIds]
        ...
        while (reachable.length > 0) {
          ...
          if (targetOccurrenceIds.has(occurrenceId)) {
            throw new Error(
              `reconciliation ${eventId} creates a cyclic equivalence graph`,
            )
          }
          if (visited.has(occurrenceId)) continue
          visited.add(occurrenceId)
      ...
      if (
        ...
        event.occurrenceIds.some((occurrenceId) =>
          occurrences.get(occurrenceId)?.findingId !== event.findingId
        )
      ) {
        invalid(
          'decision-index-reference-mismatch',
          `/index/events/${eventId}`,
          'alias reconciliation references foreign finding facts',
        )
      }
      for (const alias of event.aliases) {
        const key = `${alias.scheme}\0${alias.value}`
      ...
      if (
        ...
        !observation.reviewedBindings.some((binding) =>
          binding.path === event.historyProof.path &&
        ...
        invalid(
          'decision-index-reference-mismatch',
          `/index/events/${eventId}`,
          'retirement references foreign history facts',
        )
      }
      if (
        event.reason === 'superseded' &&
        ...
        const searched = requireObservationBindings(
          event.noReplacementProof.observationId,
          event.noReplacementProof.reviewedBindings,
          `retirement ${eventId} no-replacement proof`,
        )
        if (
          searched.repositoryRevision !==
            event.noReplacementProof.searchRevision ||
          event.noReplacementProof.searchRevision !==
            event.revisionProof.repositoryRevision
        ) {
          throw new Error(
            `retirement ${eventId} no-replacement revision is mismatched`,
          )
        }
      }
      const receiptKey = `${event.path}\0${event.blob}`
      ...
      if (
        ...
        throw new Error(
          `retirement ${eventId} lacks unique history receipt ownership`,
        )
      }
      switch (event.reason) {
        case 'staged-deletion':
          if (
            activeStagedRetirements.has(receiptKey) ||
            terminalRetirements.has(receiptKey)
          ) {
            throw new Error(
              `retirement ${eventId} duplicates an active staged or terminal retirement`,
            )
          }
        ...
        case 'deleted': {
          if (terminalRetirements.has(receiptKey)) {
            throw new Error(
              `retirement ${eventId} duplicates a terminal retirement`,
            )
          }
          ...
          if (
            ...
            throw new Error(
              `deleted retirement ${eventId} must supersede exactly the active staged event`,
            )
          }
        ...
        case 'uncommitted-snapshot-absent':
          if (
            ...
            throw new Error(
              `retirement ${eventId} conflicts with an existing staged or terminal retirement`,
            )
          }
          terminalRetirements.set(receiptKey, eventId)
          break
        default:
          throw new Error(
            `retirement ${eventId} has an unsupported retirement reason`,
          )
      }
    } else {
...
export function reduceAuditDecisionState(
  index: AuditDecisionIndexV3,
  policy: AuditDecisionPolicyV1,
  now: string,
): AuditDecisionStateV3 {
  ...
  const dispositionIsPublished = (
    event: AuditFindingDispositionEventV3,
  ): boolean => {
    ...
    for (const proof of event.proofs) {
      switch (proof.kind) {
        ...
        default:
          throw new Error(
            `decision ${event.eventId} contains an unsupported proof kind`,
          )
      }
    }
    if (
  ...
  const dispositionRows = [...index.events.values()]
    .filter((row): row is typeof row & {
      ...
      row.event.type === 'finding-disposition' &&
  ...
  const dispositionsByFinding = new Map<
    AtlasFindingId,
    Array<typeof dispositionRows[number]>
  >()
  ...
  const currentInventoryPaths = new Set<string>()
  for (const observation of index.observations.values()) {
    if (!observation.authoritative) continue
    for (const binding of observation.inventoryBindings) {
  ...
  const policyStatus = (
    row: typeof dispositionRows[number],
  ): {
    ...
    switch (event.action) {
      ...
      default:
        throw new Error(
          `decision ${eventId} contains an unsupported finding action`,
        )
    }
    const occurrence = index.occurrences.get(event.occurrenceId)
    if (!occurrence) {
      throw new Error(
        `decision ${event.eventId} references an unknown occurrence during reduction`,
      )
    }
    const rulesetAccepted = policy.acceptedRulesets.includes(
      event.reviewContext.ruleset.id,
    )
    const contextCurrent =
      event.reviewContext.policyDigest === policy.policyDigest &&
      rulesetAccepted
    ...
    if (event.action === 'accepted-risk' ||
      ...
      const severityOverride = policy.expiry.severityOverrides.find(
        (candidate) => candidate.severities.includes(occurrence.severity),
      )
      ...
      if (
        ...
        throw new Error(
          `decision ${event.eventId} expiry exceeds the event-relative maximum of ${maximumDays} days`,
        )
      }
      if (severityOverride) {
        ...
        const eligibleReviewers = new Set<string>()
        let notApproving = 0
        let selfReviewed = 0
        ...
        if (
          ...
          throw new Error(
            `decision ${event.eventId} lacks the required independent review approvals and evidence:` +
    ...
    if (event.action === 'false-positive') {
      if (
        policy.falsePositive.reviewedBlobRequired &&
        !sameBindings(
          event.reviewContext.bindings,
          event.actionEvidence.reviewedBindings,
        )
      ) {
        throw new Error(
          `decision ${event.eventId} lacks exact reviewed false-positive blobs`,
        )
      }
      if (
        policy.falsePositive.sourceEvidenceRequired &&
        event.proofs.length === 0
      ) {
        throw new Error(
          `decision ${event.eventId} lacks required source evidence`,
        )
      }
    } else if (event.action === 'remediated') {
      if (
        policy.remediation.passingRegressionRequired &&
        event.regression.result !== 'passed'
      ) {
        throw new Error(
          `decision ${event.eventId} lacks a passing remediation regression`,
        )
      }
      if (
        !policy.remediation.allowedRegressionKinds.includes(
          event.regression.kind,
        )
      ) {
        throw new Error(
          `decision ${event.eventId} uses a regression kind not allowed by policy`,
        )
      }
      if (
        policy.remediation.fixBlobRequired &&
        event.actionEvidence.afterBindings.length === 0
      ) {
        throw new Error(
          `decision ${event.eventId} lacks a bound post-fix blob`,
        )
      }
      if (
        policy.remediation.postFixProofRequired &&
        event.proofs.length === 0
      ) {
        throw new Error(
          `decision ${event.eventId} lacks a post-fix proof`,
        )
      }
    } else if (
      ...
      throw new Error(
        `decision ${event.eventId} lacks replacement or deletion proof`,
      )
    }
    if (
      event.action === 'superseded' &&
      ...
      const currentPathStillExists = event.reviewContext.bindings.some(
        (binding) => currentInventoryPaths.has(binding.path),
      )
      if (
        currentPathStillExists &&
        index.observations.get(
          event.reviewContext.observationId,
        )?.publicationState !== 'current'
      ) {
        throw new Error(
          `superseded decision ${event.eventId} requires a current review while the existing path remains`,
        )
      }
    }
  ...
  const sortedIds = (values: readonly string[]): string[] =>
    [...new Set(values)].sort(utf16Compare)
  const explicitBlocking = (
    action: AuditFindingDispositionEventV3['action'],
  ): boolean => {
    switch (action) {
      case 'reopened':
        return true
  ...
  const stateFromDirectEvent = (
    ...
    const stale =
      !status.contextCurrent || status.expiryState === 'expired'
    if (stale) {
      return {
        disposition: 'open',
        blocking: true,
    ...
    return {
      ...
      lifecycle:
        event.action === 'remediated' && currentOccurrenceIds.length === 0
          ? 'resolved'
      ...
      basisEventIds:
        event.action === 'reopened' ? [event.supersedesEventId] : [],
      expiresAt:
        event.action === 'accepted-risk' ||
        event.action === 'separate-design'
          ? event.expiresAt
  ...
  const findings = new Map<
    ...
    AuditEffectiveFindingStateV3
  >()
  for (const [findingId, finding] of [...index.findings.entries()].sort(
    ...
    for (const row of rows) policyStatus(row)
    ...
    const directlyCurrent = currentOccurrenceIds.includes(
      latest.event.occurrenceId,
    )
    ...
    if (directlyCurrent || currentOccurrenceIds.length === 0) {
      findings.set(
        findingId,
        stateFromDirectEvent(latest, currentOccurrenceIds),
      )
      continue
    }
    ...
    if (
      latestEvent.action === 'accepted-risk' ||
      ...
      const compatible = latestStatus.contextCurrent &&
        ...
        currentOccurrenceIds.every((occurrenceId) => {
          const occurrence = index.occurrences.get(
            occurrenceId as AtlasOccurrenceId,
          )
          return occurrence !== undefined &&
            occurrence.closureEligible &&
            sameBindings(
              latestEvent.reviewContext.bindings,
              occurrence.bindings,
            ) &&
    ...
    findings.set(findingId, {
      ...
      derivation: 'carried',
      lifecycle:
        latestEvent.action === 'reopened' ? 'reopened' : 'persisting',
      currentOccurrenceIds,
      eventId: null,
      basisEventIds: [latestEvent.eventId],
      expiresAt: null,
  ...
  const carryBasisBindings = (
    occurrence: AuditIndexedOccurrenceV3,
  ): readonly AuditBlobBindingV3[] => {
    if (occurrence.closureEligible) return occurrence.bindings
    return dispositionRowsByOccurrence
      .get(occurrence.occurrenceId)
      ?.at(-1)?.event.reviewContext.bindings ?? occurrence.bindings
  }
  const explicitOccurrenceIds = new Set<AtlasOccurrenceId>()
  ...
  const activeEvents = [...activeReconciliations.values()].filter((event) =>
    [...event.beforeOccurrenceIds, ...event.afterOccurrenceIds].every(
      (occurrenceId) => {
        const occurrence = index.occurrences.get(occurrenceId)!
        return index.observations.get(
          occurrence.observationId,
        )!.publicationState !== 'history-ahead'
      },
    ),
  ...
  interface LifecycleEdge {
    kind: 'reconciliation' | 'stable-identity'
    beforeOccurrenceIds: AtlasOccurrenceId[]
    afterOccurrenceIds: AtlasOccurrenceId[]
    outcome: 'equivalent' | 'distinct' | 'uncertain'
    confidence: 'high' | 'medium' | 'low'
    bindingsCompatible: boolean
  }
  const explicitIncomingOccurrenceIds = new Set<AtlasOccurrenceId>(
    activeEvents.flatMap((event) => event.afterOccurrenceIds),
  )
  const explicitOutgoingOccurrenceIds = new Set<AtlasOccurrenceId>(
    activeEvents.flatMap((event) => event.beforeOccurrenceIds),
  )
  const stableIdentityEdges: LifecycleEdge[] = []
  const stableIdentityPredecessors = new Map<
    AtlasOccurrenceId,
    AtlasOccurrenceId
  >()
  for (const finding of index.findings.values()) {
    const publishedBySlug = new Map<string, AtlasOccurrenceId[]>()
    for (const occurrenceId of finding.occurrenceIds) {
      const occurrence = index.occurrences.get(occurrenceId)!
      const observation = index.observations.get(occurrence.observationId)!
      if (observation.publicationState === 'history-ahead') continue
    ...
    for (const rows of publishedBySlug.values()) {
      ...
      for (let position = 1; position < rows.length; position += 1) {
        ...
        if (explicitIncomingOccurrenceIds.has(afterOccurrenceId)) continue
        ...
        stableIdentityEdges.push({
          ...
          outcome: 'equivalent',
          confidence: 'high',
          bindingsCompatible: sameBindings(
            carryBasisBindings(beforeOccurrence),
            afterOccurrence.bindings,
          ),
        })
        stableIdentityPredecessors.set(
          afterOccurrenceId,
          beforeOccurrenceId,
        )
      }
  ...
  const crossHistoryEdgeKeys = new Set<string>()
  for (const occurrenceId of explicitOutgoingOccurrenceIds) {
    ...
    if (
      explicitOccurrenceIds.has(seedOccurrenceId) ||
    ...
    const sourceOccurrence = index.occurrences.get(
      latestDisposition.event.occurrenceId,
    )!
    const sourceObservation = index.observations.get(
      sourceOccurrence.observationId,
    )!
    const targetObservation = index.observations.get(
      seedOccurrence.observationId,
    )!
    if (
      sourceObservation.publicationState === 'history-ahead' ||
    ...
    const edgeKey =
      `${sourceOccurrence.occurrenceId}\0${seedOccurrenceId}`
    if (crossHistoryEdgeKeys.has(edgeKey)) continue
    ...
    stableIdentityEdges.push({
      ...
      confidence: 'high',
      bindingsCompatible: sameBindings(
        carryBasisBindings(sourceOccurrence),
        seedOccurrence.bindings,
      ),
    })
  }
  const lifecycleEdges: LifecycleEdge[] = [
    ...stableIdentityEdges,
  ...
  const incomingOccurrenceIds = new Set<AtlasOccurrenceId>(
    lifecycleEdges.flatMap((event) => event.afterOccurrenceIds),
  )
  const resolvedOccurrenceIds = new Set<AtlasOccurrenceId>(
    [...index.occurrences.keys()].filter((occurrenceId) =>
      explicitOccurrenceIds.has(occurrenceId) ||
  ...
  for (const occurrence of index.occurrences.values()) {
    ...
    const published = publishedOccurrenceIdsByFinding.get(
      occurrence.findingId,
    ) ?? new Set<AtlasOccurrenceId>()
    published.add(occurrence.occurrenceId)
    publishedOccurrenceIdsByFinding.set(occurrence.findingId, published)
  }
  const lifecycleSuccessorIds = new Set<AtlasOccurrenceId>(
    lifecycleEdges
  ...
  const joinLifecycleComponents = (
    leftOccurrenceId: AtlasOccurrenceId,
    rightOccurrenceId: AtlasOccurrenceId,
  ): void => {
  ...
  for (const event of lifecycleEdges) {
    if (event.outcome === 'distinct') continue
    const occurrenceIds = [
      ...event.beforeOccurrenceIds,
      ...event.afterOccurrenceIds,
    ]
  ...
  for (const event of lifecycleEdges) {
    ...
    for (const occurrenceId of event.afterOccurrenceIds) {
      const occurrence = index.occurrences.get(occurrenceId)!
      if (!occurrence.authoritative) continue
  ...
  const basisIdsForStates = (
    states: readonly AuditEffectiveFindingStateV3[],
  ): string[] => sortedIds(states.flatMap((state) =>
    state.eventId === null ? state.basisEventIds : [state.eventId]
  ))
  const dispositionRowsForBasis = (
    basisEventIds: readonly string[],
  ): Array<typeof dispositionRows[number]> =>
    basisEventIds
      .map((eventId) => index.events.get(eventId))
      .filter((row): row is typeof dispositionRows[number] =>
        row?.event.type === 'finding-disposition'
      )
  const reconciliationTemplate = (
    event: LifecycleEdge,
    priorStates: readonly AuditEffectiveFindingStateV3[],
  ): Omit<AuditEffectiveFindingStateV3, 'currentOccurrenceIds'> => {
    ...
    const retainedAction =
      basisRows.length > 0 &&
      basisRows.every((row) =>
        row.event.action === basisRows[0].event.action
      ) &&
      (
    ...
    const terminalAction =
      ...
      priorStates.every((state) =>
        state.disposition === priorStates[0].disposition
      ) &&
      (
    ...
    const allOpen = priorStates.length > 0 &&
      priorStates.every((state) =>
        state.disposition === 'open' || state.disposition === 'reopened'
      )
    if (retainedAction !== null) {
      const owners = new Set(basisRows.map((row) => row.event.owner))
      const contexts = new Set(basisRows.map((row) =>
        `${row.event.reviewContext.ruleset.id}\0` +
      ...
      const afterContextsMatch =
        referenceRuleset !== undefined &&
      ...
      const compatible =
        priorStates.every((state) => state.disposition === retainedAction) &&
        basisRows.every((row) => {
          const status = policyStatus(row)
      ...
      const expiries = basisRows
        .map((row) =>
          row.event.action === 'accepted-risk' ||
  ...
  for (const [edgeIndex, event] of lifecycleEdges.entries()) {
    for (const occurrenceId of event.beforeOccurrenceIds) {
      if (resolvedOccurrenceIds.has(occurrenceId)) continue
  ...
  while (readyEdgeIndexes.length > 0) {
    ...
    const priorStates = event.beforeOccurrenceIds.map(
      (occurrenceId) => occurrenceStates.get(occurrenceId)!,
    )
    ...
    for (const occurrenceId of event.afterOccurrenceIds) {
      ...
      for (const waitingEdgeIndex of
        waitingEdgesByOccurrence.get(occurrenceId) ?? []) {
  ...
  const frontierOccurrenceIdsByFinding = new Map<
    ...
    AtlasOccurrenceId[]
  >()
  const globallyConflictedFindingIds = new Set<AtlasFindingId>()
  for (const [findingId, publishedOccurrenceIds] of
    publishedOccurrenceIdsByFinding) {
    const frontierOccurrenceIds = sortedIds(
      [...publishedOccurrenceIds].filter(
        (occurrenceId) => !lifecycleSuccessorIds.has(occurrenceId),
      ),
    ) as AtlasOccurrenceId[]
    frontierOccurrenceIdsByFinding.set(findingId, frontierOccurrenceIds)
    if (frontierOccurrenceIds.length === 0) continue
    const frontierComponentIds = new Set(
      frontierOccurrenceIds.map(lifecycleComponent),
    )
    if (frontierComponentIds.size <= 1) continue
    const candidates = frontierOccurrenceIds.map((occurrenceId) =>
      occurrenceStates.get(occurrenceId)!
    )
    findings.set(findingId, {
      disposition: 'open',
      blocking: true,
      derivation: 'reconciliation-conflict',
      lifecycle: 'unknown',
      currentOccurrenceIds: sortedIds(
        index.findings.get(findingId)!.currentOccurrenceIds,
      ) as AtlasOccurrenceId[],
      eventId: null,
  ...
  for (const [findingId, affectedOccurrenceIds] of affectedCurrentByFinding) {
    if (globallyConflictedFindingIds.has(findingId)) continue
    const currentOccurrenceIds = sortedIds(
      index.findings.get(findingId)!.currentOccurrenceIds,
    ) as AtlasOccurrenceId[]
    const coveredOccurrenceIds = sortedIds(
      [...affectedOccurrenceIds],
    ) as AtlasOccurrenceId[]
    const candidates = currentOccurrenceIds.map((occurrenceId) =>
      occurrenceStates.get(occurrenceId)!
    )
    const first = candidates[0]
    const allDerivedCompatible = candidates.every((candidate) =>
    ...
    if (
      canonicalJson(coveredOccurrenceIds) !==
      ...
      findings.set(findingId, {
        ...
        currentOccurrenceIds,
        eventId: null,
        basisEventIds: sortedIds(candidates.flatMap((candidate) =>
          candidate.eventId === null
    ...
    findings.set(findingId, {
      ...first,
      currentOccurrenceIds,
      basisEventIds: sortedIds(candidates.flatMap(
        (candidate) => candidate.basisEventIds,
      )),
    })
  }

  for (const [findingId, frontierOccurrenceIds] of
    frontierOccurrenceIdsByFinding) {
    if (globallyConflictedFindingIds.has(findingId)) continue
    const finding = index.findings.get(findingId)!
    if (finding.currentOccurrenceIds.length > 0) continue
    if (frontierOccurrenceIds.length === 0) continue
    const candidates = frontierOccurrenceIds.map((occurrenceId) =>
      occurrenceStates.get(occurrenceId)!
    )
    if (
      frontierOccurrenceIds.length === 1 &&
  ...
  const retirements = new Map<string, AuditScopeRetirementEventV3>()
  for (const event of index.retirementEvents) {
    ...
    if (proofObservationIds.some((observationId) =>
      index.observations.get(
        observationId,
      )?.publicationState === 'history-ahead'
    )) {
    ...
    if (!policy.retirement.allowedReasons.includes(event.reason)) {
      throw new Error(
        `retirement ${event.eventId} uses a reason not allowed by policy`,
      )
    }
    if (
      policy.retirement.historyProofRequired &&
      event.historyProof === undefined
    ) {
      throw new Error(
        `retirement ${event.eventId} lacks required history proof`,
      )
    }
    retirements.set(`${event.path}\0${event.blob}`, event)
  }

  const aliases: AuditDecisionStateV3['aliases'] = new Map()
  for (const event of index.aliasEvents) {
    for (const alias of event.aliases) {
      aliases.set(`${alias.scheme}:${alias.value}`, {
        findingId: event.findingId,
        occurrenceIds: sortedIds(event.occurrenceIds) as AtlasOccurrenceId[],
