...
export function compactSidebarA11y(sideOpen: boolean): {
  inert?: true
  'aria-hidden'?: true
} {
  if (sideOpen) return {}
 ...
 */
export function shouldRestoreCompactSidebarFocus(
  compact: boolean,
  wasOpen: boolean,
  isOpen: boolean,
): boolean {
...
export function auditUnitSectionIds(
  domain: 'security' | 'test',
  slug: string,
): { toggleId: string; panelId: string } {
...
export function auditUnitToggleProps(
  open: boolean,
  panelId: string,
...
export function auditUnitPanelProps(open: boolean): { hidden: boolean } {
  return { hidden: !open }
}

/** aria-pressed for severity / impact / category / stale filter chips. */
export function auditFilterChipAriaPressed(selected: boolean): boolean {
  return selected
}
