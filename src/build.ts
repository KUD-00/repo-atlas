 ...
 */
function orderChildren(n: TreeNode, edges: Map<string, Set<string>> | undefined): TreeNode[] {
  const byName = new Map(n.children.map((c) => [c.name, c]))
  const head = (n.order ?? []).map((name) => byName.get(name)).filter((c): c is TreeNode => !!c)
 ...
 */
function siblingImports(graph: ImportGraph | null): Map<string, Map<string, Set<string>>> {
  ...
  for (const [si, di] of graph.edges) {
    ...
    if (k >= s.length || k >= d.length) continue // one path contains the other
    const dir = s.slice(0, k).join('/')
    let sib = out.get(dir)
    if (!sib) out.set(dir, (sib = new Map()))
    let tos = sib.get(s[k])
    if (!tos) sib.set(s[k], (tos = new Set()))
    tos.add(d[k])
  }
  return out
}
...
export function writeAtlas(root: string, outFile: string, html: string): string {
