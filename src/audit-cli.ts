...
import {
  ...
  checkAuditCoverage,
  updateAuditCoverage,
} from './audit-coverage-generator.js'
import { importCodexSecurityBundle } from './audit-import-codex.js'
import {
  buildRelayOSAuditMigration,
  migrateRelayOSAudit,
} from './audit-migrate-relayos.js'
import {
  buildRelayOSRootAuditsMigration,
  migrateRelayOSRootAudits,
} from './audit-migrate-relayos-root-audits.js'
import {
  AUDIT_PROVIDER_INVOCATION_COMMAND,
  AuditProviderError,
  loadAuditProviderPolicy,
  resolveAuditProviderPolicy,
  runAuditProviderInvocation,
  type AuditProviderInvocationRequest,
...
const USAGE_MIGRATE =
  'usage: repo-atlas audit migrate <relayos-security-v1|relayos-root-audits-v1> ...'
const USAGE_MIGRATE_SECURITY =
  'usage: repo-atlas audit migrate relayos-security-v1 --scan-root <path> --policy <path>' +
  ' --source-revision <commit> --validation-revision <commit>' +
  ' [--include-history | --no-include-history] [--apply]'
const USAGE_MIGRATE_ROOT_AUDITS =
  'usage: repo-atlas audit migrate relayos-root-audits-v1 --audits-root <path>' +
  ' --source-revision <commit> --validation-revision <commit> [--apply]'
const USAGE_DECISION_SET =
  'usage: repo-atlas audit decision set <finding-or-occurrence> <action> --event <event.json>'
const USAGE_RECONCILE =
  'usage: repo-atlas audit reconcile <before> <after> --event <event.json>'
const USAGE_RETIRE =
  'usage: repo-atlas audit retire <path> --event <event.json>\n' +
  '       repo-atlas audit retire --finalize-staged --event <event.json>'
const USAGE_LOCALIZATION =
  'usage: repo-atlas audit localization input --locale <en|ja|zh|ko> [--json]\n' +
...
interface AuditCliDiagnostic {
  code: string
  message: string
  path?: string
}

function usage(usageText: string, message: string): never {
...
function parseAuditArgs(
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (valueFlags.has(arg)) {
      ...
      if (values.has(arg)) usage(usageText, `${arg} given twice`)
      values.set(arg, value)
      index++
      continue
    }
    if (boolFlags.has(arg)) {
      if (flags.has(arg)) usage(usageText, `${arg} given twice`)
      flags.add(arg)
      continue
    }
    if (arg.startsWith('--')) usage(usageText, `unknown flag ${arg}`)
    positionals.push(arg)
  }
  return { positionals, values, flags }
}

function rejectPositionals(parsed: ParsedAuditArgs, usageText: string): void {
  if (parsed.positionals.length > 0) {
    usage(usageText, `unexpected positional argument ${parsed.positionals[0]}`)
  }
}

function requiredValue(parsed: ParsedAuditArgs, flag: string, usageText: string): string {
  const value = parsed.values.get(flag)
  if (value === undefined) usage(usageText, `${flag} is required`)
  return value
}
...
function printDiagnostics(diagnostics: readonly AuditCliDiagnostic[], indent = '  '): void {
  for (const diagnostic of diagnostics.slice(0, PRINT_DIAGNOSTIC_LIMIT)) {
    console.log(
      `${indent}${diagnostic.code}${diagnostic.path ? ` ${diagnostic.path}` : ''}: ${diagnostic.message}`,
    )
  }
...
// ---------------------------------------------------------------------------
// migration receipts (.atlas/migrations/<migrationId>.json)
// ---------------------------------------------------------------------------

interface AuditMigrationReceiptStatus {
  migrationId: string
  path: string
}

function loadAuditMigrationReceipts(root: string): {
  receipts: AuditMigrationReceiptStatus[]
  diagnostics: AuditCliDiagnostic[]
} {
  const receipts: AuditMigrationReceiptStatus[] = []
  const diagnostics: AuditCliDiagnostic[] = []
  let names: string[]
  try {
  ...
  receipts.sort((left, right) =>
    left.migrationId < right.migrationId ? -1 : left.migrationId > right.migrationId ? 1 : 0)
...
interface AuditCheckArea {
  area: string
  ok: boolean
  diagnostics: AuditCliDiagnostic[]
}

function legacyLedgerDiagnostics(root: string): AuditCliDiagnostic[] {
  const config = loadConfig(root) ?? {}
...
function computeAuditCheck(root: string, allowIncomplete: boolean): {
  ok: boolean
  areas: AuditCheckArea[]
  coverageVerdict: string
  coverageCurrent: boolean
  historyAhead: string[]
} {
  const area = (name: string, diagnostics: AuditCliDiagnostic[]): AuditCheckArea => ({
    area: name,
    ok: diagnostics.length === 0,
    diagnostics,
  })
  const areas: AuditCheckArea[] = []
...
function auditCheck(root: string, args: string[]): void {
  ...
  if (result.ok) {
    console.log(
      `audit check: ok — ${result.areas.length} areas valid · ` +
  ...
  for (const area of result.areas) {
    if (area.ok && area.diagnostics.length === 0) {
      ...
      console.log(
        `  ${area.area}: ${area.ok ? 'reported' : 'invalid'} — ${area.diagnostics.length} diagnostic(s)`,
      )
...
function auditCoverage(root: string, args: string[]): void {
  ...
  console.log(
    `audit coverage ${verb}: ${result.current ? 'current' : 'stale-bytes'} · verdict ${verdict}` +
...
function auditStatus(root: string, args: string[]): void {
  ...
  const historyCounts = new Map(
    histories.histories.map((history) => [history.slug, history.entries.length]),
  )
  ...
  console.log(
    `observations: ${units.length} current ledger(s) · ` +
  ...
  for (const unit of units.slice(0, PRINT_ROW_LIMIT)) {
    console.log(
      `  ${unit.slug}: ${unit.reviewState} · ${unit.findings} finding(s) · ` +
  ...
  console.log(
    `coverage: ${coverage.current ? 'current' : 'stale-bytes'} · verdict ${coverage.report.verdict}`,
  )
  const diagnostics = [
...
// ---------------------------------------------------------------------------
// audit run security (the only provider-spawning command)
// ---------------------------------------------------------------------------
...
function reviewablePaths(
  classification: AuditInventoryClassification,
): Map<string, { deleted: boolean; currentBlob: string | null }> {
  return new Map(
    classification.files.map((file) => [
      file.path,
...
function securityUnitOf(file: AuditInventoryClassification['files'][number]): string | undefined {
  return file.classification.kind === 'review'
    ? file.classification.domains.security?.unit
    : undefined
}

function selectRunTargets(root: string, selection: RunSelection): AuditProviderTarget[] {
  ...
  if (inventory.diagnostics.length > 0) {
    throw new Error(
      'audit run security requires a readable tracked inventory: ' +
  ...
  if (selection.kind === 'unit') {
    ...
    if (unit === undefined) {
      ...
      throw new Error(
        `unknown security unit: ${selection.slug}` +
  ...
  if (selection.kind === 'all') {
    const targets = classification.files
      ...
      .map((file): AuditProviderTarget => ({ path: file.path, role: 'review' }))
    if (targets.length === 0) {
  ...
  if (exactEvidence.invalidLedgers.length > 0) {
    throw new Error(
      'audit run security requires valid exact evidence ledgers: ' +
  ...
  if (report.verdict === 'invalid') {
    throw new Error(
      'audit run security refuses to select stale targets from invalid coverage inputs: ' +
...
async function auditRun(root: string, args: string[]): Promise<void> {
  ...
  const selection: RunSelection = parsed.values.has('--unit')
  ...
  try {
    ...
    if (error instanceof AuditProviderError && error.code === 'policy-invalid') {
      throw new Error(
        'audit run security requires an explicit provider policy at ' +
        `.atlas/audit-providers.json: ${error.message}`,
      )
    }
    throw error
  }
  const request: AuditProviderInvocationRequest = {
  ...
  console.log(
    `  files: ${result.files.length} reviewed (${result.files.length - findings} clean · ${findings} with findings)`,
  )
...
function auditImport(root: string, args: string[]): void {
  const [kind, ...rest] = args
  if (kind === 'codex-security') return auditImportCodexSecurity(root, rest)
  if (kind === 'legacy-v1') return auditImportLegacyCommand(root, rest)
  usage(
    USAGE_IMPORT,
    kind === undefined
...
function auditImportCodexSecurity(root: string, args: string[]): void {
  const parsed = parseAuditArgs(
    args,
    { values: ['--slug'], flags: ['--apply'] },
    USAGE_IMPORT_CODEX,
  )
  ...
  console.log(
    `audit import codex-security: ${result.observation.observationId} · ` +
  ...
  if (result.publication !== undefined) {
    ...
    console.log(
      `  appended ${result.publication.historyPath} (${result.publication.appendedObservationId})`,
    )
  } else {
...
export function auditImportLegacyCommand(root: string, args: string[]): void {
  const sources = args.filter((arg) => !arg.startsWith('--'))
  if (!sources.length) usage(USAGE_IMPORT_LEGACY, 'expected at least one <legacy-ledger.json>')
  for (const source of sources) {
    const imported = importLegacyAudit(root, source)
    console.log(
      `imported: ${imported.name} · ${imported.fileCount} files · ` +
...
function auditMigrate(root: string, args: string[]): void {
  const [kind, ...rest] = args
  if (kind === 'relayos-security-v1') return auditMigrateRelayosSecurity(root, rest)
  if (kind === 'relayos-root-audits-v1') return auditMigrateRelayosRootAudits(root, rest)
  usage(
    USAGE_MIGRATE,
    kind === undefined
...
function printMigrationWrites(
  writes: readonly { path: string; sha256: string }[],
  apply: boolean,
): void {
  ...
  if (!apply) console.log('  dry run — pass --apply to write these outputs')
}

function auditMigrateRelayosSecurity(root: string, args: string[]): void {
  const parsed = parseAuditArgs(
    args,
    {
  ...
  console.log(
    `  ${counts.scanRecords} scan record(s) · ${counts.activeFindings} active finding(s) · ` +
...
function auditMigrateRelayosRootAudits(root: string, args: string[]): void {
  const parsed = parseAuditArgs(
    args,
    {
  ...
  console.log(
    `  ${counts.sourceFiles} source file(s) · ${counts.designScanRecords} design scan record(s) · ` +
...
function readDecisionEventFile(parsed: ParsedAuditArgs, usageText: string): Record<string, unknown> {
  const file = parsed.values.get('--event')
  if (file === undefined) usage(usageText, '--event <event.json> is required')
...
function auditDecision(root: string, args: string[]): void {
  const [verb, ...rest] = args
  if (verb !== 'set') {
    usage(
      USAGE_DECISION_SET,
...
function decisionLedgerForFinding(root: string, event: {
  findingId: unknown
  ...
  if (diagnostics.length > 0) {
    throw new Error(
      'audit state is invalid; run `repo-atlas audit check` and resolve diagnostics first: ' +
  ...
  const finding = index.findings.get(findingId as never)
  if (finding === undefined) throw new Error(`unknown finding: ${findingId}`)
  const occurrence = index.occurrences.get(occurrenceId as never)
  if (occurrence === undefined) throw new Error(`unknown occurrence: ${occurrenceId}`)
...
function auditDecisionSet(root: string, args: string[]): void {
  ...
  if (raw.type !== 'finding-disposition') {
    throw new Error(
      `audit decision set accepts a finding-disposition event; got ${String(raw.type)}`,
    )
  }
  const event = raw as unknown as AuditFindingDispositionEventInputV3
  ...
  if (event.findingId !== target && event.occurrenceId !== target) {
    throw new Error(
      `positional target ${target} matches neither the event findingId nor occurrenceId`,
    )
  }
  const slug = decisionLedgerForFinding(root, event)
  if ('decisionLedger' in event && (event as { decisionLedger?: unknown }).decisionLedger !== slug) {
...
function auditReconcile(root: string, args: string[]): void {
  ...
  if (raw.type !== 'finding-reconciliation') {
    throw new Error(
      'audit reconcile appends a finding-reconciliation event ' +
      `(identity-alias reconciliations are produced by migrations); got ${String(raw.type)}`,
    )
  }
  const event = raw as unknown as AuditFindingReconciliationEventInputV3
  if (
    ...
    throw new Error(
      'positional <before> <after> must equal the event before/after occurrence sets',
    )
  }
...
function findStagedDeletion(
  root: string,
  slug: string,
  eventId: string,
): AuditScopeRetirementEventInputV3 | null {
  const portfolio = loadAuditDecisionLedgers(root)
  if (portfolio.diagnostics.length > 0) {
    throw new Error(
      'decision portfolio is invalid: ' +
...
function auditRetire(root: string, args: string[]): void {
  const parsed = parseAuditArgs(
    args,
    { values: ['--event'], flags: ['--finalize-staged'] },
    USAGE_RETIRE,
  )
  ...
  const event = raw as unknown as AuditScopeRetirementEventInputV3
  ...
  if (finalize) {
    if (event.reason === 'staged-deletion') {
      throw new Error(
        '--finalize-staged requires a final reason (deleted, moved, or superseded), ' +
        'not another staged-deletion',
      )
    }
    const supersedes = (event as { supersedesEventId?: unknown }).supersedesEventId
    ...
    if (staged === null) {
      throw new Error(
        `no staged-deletion event ${supersedes} in ${event.decisionLedger} to finalize`,
      )
    }
...
export function auditStampCommand(root: string, args: string[]): void {
  const names = args.filter((arg) => !arg.startsWith('--'))
  const { stamped, skipped, notFound } = stampAudits(
    root,
    scan(root, requireConfig(root)),
    names.length ? names : undefined,
  )
  for (const name of stamped) console.log(`stamped: ${name}`)
  for (const refusal of skipped) console.error(`refused: ${refusal}`)
...
export function auditLocalizationInputCommand(root: string, args: string[]): void {
...
export function auditLocalizationCheckCommand(root: string, args: string[]): void {
...
function auditLocalization(root: string, args: string[]): void {
  const [verb, ...rest] = args
  if (verb === 'input') return auditLocalizationInputCommand(root, rest)
  if (verb === 'check') return auditLocalizationCheckCommand(root, rest)
  usage(
    USAGE_LOCALIZATION,
    verb === undefined
 ...
 */
export async function runAuditCommand(root: string, args: string[]): Promise<void> {
  const [verb, ...rest] = args
  switch (verb) {
    case undefined:
    case 'help':
