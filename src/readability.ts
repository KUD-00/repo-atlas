...
import type { AtlasConfig, ScanResult } from './types.js'

/**
 * Mechanical code-readability features — no LLM, no absolute score.
 *
 * Research basis (docs/readability-audit.md): per-file / per-function features
 * in the spirit of Buse & Weimer (TSE 2010) and Scalabrino et al. (JSEP 2018),
 * reported as REPO-RELATIVE outliers (z-scores), because the evidence says
 * single absolute metrics do not predict human judgements — drift from the
 * repo's own norms is the actionable signal.
 *
 * Deliberate approximations (documented, not hidden):
 * - comment/string masking is a small state machine per language family, not a
 *   parser; regex literals and template-literal interpolations can confuse it;
 * - function detection is regex + brace matching (best effort, misses are fine);
 * - duplication uses 4-line shingle fingerprints (FNV-1a, collisions tolerated)
 *   and counts intra-file duplication too — both hurt readability;
 * - Halstead volume / token entropy are computed over a crude token stream.
 */
...
interface LangSpec {
  ...
  /** python-style ''' / """ */
  triple: boolean
  /** {} nesting when true, indentation nesting when false */
  braces: boolean
}

function maxOf(values: Iterable<number>, fallback = 0): number {
  let maximum = fallback
  for (const value of values) if (value > maximum) maximum = value
  return maximum
}

const CLIKE: LangSpec = {
...
const HASH: LangSpec = { ...PY, triple: false }
...
const KEYWORDS = new Set((
  'if else elif for while do switch case default try catch except finally with return break continue ' +
  ...
  'impl where match select chan range type record sealed get set void int long float double char bool'
).split(' '))
...
type MaskState = { kind: 'code' } | { kind: 'block'; closer: string } | { kind: 'str'; quote: string; span: boolean }

function maskSource(text: string, lang: LangSpec): MaskedLine[] {
  const out: MaskedLine[] = []
  let state: MaskState = { kind: 'code' }
  for (const raw of text.split('\n')) {
    ...
    while (i < n) {
      if (state.kind === 'block') {
        ...
        if (end < 0) { comment += raw.slice(i); i = n } else {
        ...
        while (j < n) {
          ...
          if (state.quote.length === 3 ? raw.startsWith(state.quote, j) : raw[j] === state.quote) break
          j++
        }
        if (j >= n) {
          i = n
          if (!state.span) state = { kind: 'code' } // unterminated EOL string: pretend closed
        } else {
        ...
        for (const lc of lang.line) {
          if (raw.startsWith(lc, i)) { comment += raw.slice(i); i = n; hit = true; break }
        }
        if (!hit) {
...
interface Dist { mean: number; sd: number; p50: number; p95: number; max: number }

function dist(xs: number[]): Dist {
  if (!xs.length) return { mean: 0, sd: 0, p50: 0, p95: 0, max: 0 }
...
type IdentStyle = 'camel' | 'snake' | 'pascal' | 'upper' | 'mixed' | 'other'

function styleOf(id: string): IdentStyle {
  const core = id.replace(/^_+|_+$/g, '')
  if (!core) return 'other'
  if (core.length === 1) return 'other'
  if (/^[A-Z0-9_]+$/.test(core)) return 'upper'
  const hasUnder = core.includes('_')
  if (hasUnder && /^[a-z0-9_]+$/.test(core)) return 'snake'
  if (hasUnder) return 'mixed'
  if (/^[a-z0-9]/.test(core)) return 'camel'
  if (/^[A-Z]/.test(core) && /[a-z]/.test(core)) return 'pascal'
  return 'other'
}
...
const STOPWORDS = new Set((
  'the and for with this that from are was were not but when then else here there which would ' +
  ...
  'any both few more most other some such nor own same too very just like make makes made need'
).split(' '))

/** split identifiers into lowercase words (camel/snake/digit boundaries) */
function splitIdentWords(id: string): string[] {
  return id
...
function braceDepths(lines: MaskedLine[]): number[] {
  const depths: number[] = []
  let d = 0
...
function indentWidths(lines: MaskedLine[]): number[] {
  return lines.map((l) => {
    const m = /^[ \t]*/.exec(l.code)![0]
    let w = 0
    for (const ch of m) w += ch === '\t' ? 4 : 1
    return w
  })
}
...
function detectFunctions(lines: MaskedLine[], lang: LangSpec): FunctionRegion[] {
  const fns: FunctionRegion[] = []
  if (lang.braces) {
    ...
    while (i < lines.length && fns.length < 500) {
      ...
      if (!name || !code.includes('{')) { i++; continue }
      ...
      for (let j = i; j < lines.length; j++) {
        ...
        if (!opened && nextDepth > startDepth) opened = true
        if (opened && nextDepth <= startDepth) { end = j; break }
        end = j
      }
    ...
    for (let i = 0; i < lines.length && fns.length < 500; i++) {
      ...
      for (let j = i + 1; j < lines.length; j++) {
        const l = lines[j]
        if (!l.code.trim()) { end = j; continue }
        if (indents[j] <= base) break
        end = j
      }
      if (end === i) continue
      const region = lines.slice(i, end + 1)
...
export interface FileFeatures {
  ...
  commentedOutRatio: number
  lineLen: Dist
  longestLines: { line: number; len: number }[]
  maxNesting: number
  branchesPer100: number
  ident: {
    count: number
    avgLen: number
    shortRatio: number
    dominantStyle: IdentStyle
    dominantShare: number
  }
  tokenEntropy: number
  halsteadPerLine: number
  /** 注释词 ∩ 标识符词 / 注释词（0..1；无注释 null）。机械版 comment-code
   *  coherence——Scalabrino textual features 的简化，只进画像不进组合分。 */
  commentCoherence: number | null
  dupRatio: number | null // filled in the repo pass; null when too few shingles to judge
  barrelRatio: number // re-export lines / code lines
  /** git blob hash at scan time — enables trend diffs between runs. */
  hash: string
...
function analyzeFile(text: string, langKey: string): FileFeatures {
  ...
  const commentedOut = masked.filter((l) => {
    ...
    if (!c) return false
    if (/^(@|#+\s|\*|todo|fixme|note|https?:|<reference\b|@ts-)/i.test(c.replace(/^\/+\s*|^--\s*/, ''))) return false
  ...
  let identLenSum = 0
  let shortCount = 0
  const identWordsByLine: Set<string>[] = []
  const styles: Record<IdentStyle, number> = { camel: 0, snake: 0, pascal: 0, upper: 0, mixed: 0, other: 0 }
  for (const l of masked) {
    const lineWords = new Set<string>()
    const ids = l.code.match(IDENT_RE)
    if (ids) {
      for (const id of ids) {
        if (KEYWORDS.has(id.toLowerCase()) || KEYWORDS.has(id)) continue
        identCount++
        identLenSum += id.length
        if (id.length <= 2) shortCount++
        styles[styleOf(id)]++
        for (const w of splitIdentWords(id)) lineWords.add(w)
      }
    }
  ...
  for (let line = 0; line < masked.length; line++) {
    ...
    for (let other = Math.max(0, line - 2); other <= Math.min(masked.length - 1, line + 2); other++) {
      for (const word of identWordsByLine[other]) nearby.add(word)
    }
    for (const word of words) {
      coherenceWords++
      if (nearby.has(word)) coherenceHits++
    }
  }
  const commentCoherence = coherenceWords ? coherenceHits / coherenceWords : null
  const styleEntries = (Object.entries(styles) as [IdentStyle, number][])
    .filter(([k]) => k !== 'other' && k !== 'mixed')
    .sort((a, b) => b[1] - a[1])
  const styleTotal = styleEntries.reduce((a, [, c]) => a + c, 0)
  const [dominantStyle, dominantCount] = styleEntries[0] ?? ['other' as IdentStyle, 0]
  ...
  for (const t of tokens) {
    if (/^[A-Za-z_$]/.test(t) || /^\d/.test(t)) {
      if (KEYWORDS.has(t)) operators.push(t)
...
export interface OutlierEntry { path: string; value: number; z: number }

export interface DirRollup {
  path: string
  files: number
  meanComposite: number
  /** files with surfaceComposite z <= -2 */
  lowFiles: number
  worst: string | null
...
export interface ReadabilityReport {
  ...
  /** population mean/sd per metric across analysed files — the repo's own norm */
  norms: Record<string, { mean: number; sd: number }>
  outliers: Record<string, OutlierEntry[]>
  dirs: DirRollup[]
  longestLines: { path: string; line: number; len: number }[]
  worstFunctions: { path: string; name: string; line: number; lines: number; maxLineLen: number; maxNesting: number; branches: number }[]
  files: Record<string, FileFeatures>
  /** Comparison captured when this report replaced an older report at --out. */
  trend?: ReadabilityTrend
}

export interface ReadabilityTrendRow { path: string; before: number; after: number }
...
export interface ReadabilityTrend {
  ...
  improved: ReadabilityTrendRow[]
  worsened: ReadabilityTrendRow[]
}
...
export interface ReadabilityStatus {
  ...
  missingFiles: string[]
  failedFiles: string[]
  latestTrend: ReadabilityTrend | null
}

function normalizeReadabilityTrend(value: unknown): ReadabilityTrend | null {
  if (!value || typeof value !== 'object') return null
  const trend = value as Partial<ReadabilityTrend>
  if (!((trend.comparedTo === null || typeof trend.comparedTo === 'string') &&
    ...
    Array.isArray(trend.improved) && Array.isArray(trend.worsened))) return null
  ...
  return { ...trend, addedFiles, removedFiles, improvedCount, worsenedCount } as ReadabilityTrend
}

export function isSupportedReadabilityReport(value: unknown): value is ReadabilityReport {
  if (!value || typeof value !== 'object') return false
  const report = value as Partial<ReadabilityReport>
...
const METRICS: Record<string, [(f: FileFeatures) => number, 'high' | 'low']> = {
...
export function surfaceCompositeOf(f: FileFeatures, norms: Record<string, { mean: number; sd: number }>): number {
...
export function diffReadabilityReports(
  prev: ReadabilityReport,
  curr: ReadabilityReport,
  topN = 10,
  minDelta = 1,
): Omit<ReadabilityTrend, 'comparedTo'> {
  const rows: { path: string; before: number; after: number }[] = []
  ...
  for (const [p, f] of Object.entries(curr.files)) {
    ...
    if (!pf?.hash || !f.hash || pf.hash === f.hash) continue
    changedFiles++
...
export function readabilityStatus(root: string, scanResult: ScanResult): ReadabilityStatus | null {
  ...
  try {
    ...
    if (opened) {
      ...
      const filesList = validFiles ? rawFiles as string[] : []
      const validHashes = validFiles && rawHashes && typeof rawHashes === 'object' && !Array.isArray(rawHashes) &&
        ...
        filesList.every((repoPath) => /^[0-9a-f]{40}$/u.test(((rawHashes as Record<string, unknown>)[repoPath] as string | undefined) ?? '')) &&
        Object.keys(rawHashes).every((repoPath) => filesList.includes(repoPath))
      const scopeLines = validHashes
        ? filesList.map((repoPath) => `${(rawHashes as Record<string, string>)[repoPath]}  ${repoPath}`).sort()
        : []
      ...
      if (index.format === 'atlas-audit-v1' && index.formatVersion === 1 &&
        ...
        if (summary && typeof summary === 'object') {
          ...
          if (value.formatVersion === 1) {
            if (typeof value.generatedAt === 'string') generatedAt = value.generatedAt
  ...
  if (!hashes) {
    try {
      ...
      const report = JSON.parse(opened.buffer.toString('utf8')) as unknown
      if (!isSupportedReadabilityReport(report)) return null
      const entries = Object.entries(report.files)
      if (entries.some(([repoPath, features]) => !validRepoPathForReadability(repoPath) || !/^[0-9a-f]{40}$/u.test(features?.hash ?? ''))) return null
  ...
  try {
    ...
    for (const [repoPath, previous] of Object.entries(hashes)) {
      ...
      if (hash && previous !== hash) changedFiles.push(repoPath)
    }
    return {
      file,
      generatedAt,
...
function validRepoPathForReadability(repoPath: string): boolean {
...
function safeAtlasDirectory(root: string, segments: string[], create: boolean): string {
...
function assertSafeAtlasFile(root: string, file: string, parentSegments: string[], createParent: boolean): void {
...
export function assertCanonicalReadabilityOutput(root: string): void {
  const file = path.join(atlasDir(root), 'readability.json')
  assertSafeAtlasFile(root, file, [], false)
}

export function assertReadabilityReportOutput(root: string, file: string): void {
  const target = path.resolve(file)
  const parent = path.dirname(target)
  const parentStat = fs.lstatSync(parent)
  if (!parentStat.isDirectory() || parentStat.isSymbolicLink()) throw new Error(`unsafe readability output directory: ${parent}`)
  ...
  if (fs.existsSync(target)) {
    ...
    if (!stat.isFile() || stat.isSymbolicLink()) throw new Error(`unsafe readability output: ${file} must be a regular file, not a symlink`)
  }
}
...
export function readReadabilityReport(root: string, file: string): unknown {
  ...
  if (target !== rootPath && target.startsWith(rootPath + path.sep)) {
    ...
    if (!opened) throw new Error(`unsafe readability report: ${file}`)
...
export function writeReadabilityReport(root: string, file: string, report: ReadabilityReport): string {
  assertReadabilityReportOutput(root, file)
  atomicWrite(path.resolve(file), JSON.stringify(report, null, 2) + '\n')
  return path.resolve(file)
}

export function writeCanonicalReadabilityReport(root: string, report: ReadabilityReport): string {
  const file = path.join(atlasDir(root), 'readability.json')
  assertCanonicalReadabilityOutput(root)
  return writeReadabilityReport(root, file, report)
}

export function assertReadabilityAuditOwnership(root: string): void {
  ...
  try {
    const opened = readRepoFile(root, '.atlas/audits/readability.json')
    if (!opened) throw new Error('unsafe readability audit ledger')
  ...
  const owned = existing !== null && typeof existing === 'object' &&
    ...
    (existing as Record<string, unknown>).slug === 'readability' &&
    (existing as Record<string, unknown>).ruleset === 'repo-atlas-readability-v1'
  if (!owned) throw new Error(`refusing to overwrite unrelated audit ledger: ${path.relative(root, file)}`)
}

export function writeReadabilityAuditLedger(root: string, report: ReadabilityReport): string | null {
  ...
  for (const [metric, rows] of Object.entries(report.outliers)) {
    for (const row of rows) {
      if (!metricsByFile.has(row.path)) metricsByFile.set(row.path, [])
...
export function computeReadability(root: string, config: AtlasConfig, topN = 10): ReadabilityReport {
  const scanResult = scan(root, { exclude: [...DEFAULT_EXCLUDE, ...(config.exclude ?? [])] })
  const files: Record<string, FileFeatures> = {}
  const shinglesByFile = new Map<string, Set<number>>()
  ...
  for (const rel of scanResult.files.keys()) {
    ...
    if (!lang) { skippedNonCode++; continue }
    let text: string
    let blobHash: string
    try {
      const opened = readRepoFile(root, rel, MAX_FILE_BYTES + 1)
      if (!opened) { skippedNonCode++; continue }
      const buf = opened.buffer
      if (opened.truncated || buf.length > MAX_FILE_BYTES) { skippedLarge++; continue }
      if (buf.subarray(0, 8192).includes(0)) { skippedNonCode++; continue }
      text = buf.toString('utf8')
      blobHash = createHash('sha1').update(`blob ${buf.length}\0`).update(buf).digest('hex')
    } catch { skippedNonCode++; continue }
  ...
  for (const [rel, set] of shinglesByFile) {
    ...
    let dup = 0
    for (const h of set) if ((shingleCounts.get(h) ?? 0) > 1) dup++
    files[rel].dupRatio = dup / set.size
  }

  // norms + outliers
  const norms: Record<string, { mean: number; sd: number }> = {}
  const outliers: Record<string, OutlierEntry[]> = {}
  for (const [metric, [get, tail]] of Object.entries(METRICS)) {
    ...
    if (sd === 0) { outliers[metric] = []; continue }
  ...
  const compositeOf = (f: FileFeatures): number => surfaceCompositeOf(f, norms)
  ...
  const dirMap = new Map<string, { files: number; sum: number; low: number; worst: string | null; worstV: number }>()
  for (const [p, f] of Object.entries(files)) {
    const parts = p.split('/')
    const key = parts.length >= 3 ? parts.slice(0, 2).join('/') : parts.length === 2 ? parts[0] : '(root)'
    if (!dirMap.has(key)) dirMap.set(key, { files: 0, sum: 0, low: 0, worst: null, worstV: Number.POSITIVE_INFINITY })
    const d = dirMap.get(key)!
    const c = compositeOf(f)
    d.files++
    d.sum += c
    if (compDist.sd > 0 && (c - compDist.mean) / compDist.sd <= -2) d.low++
    if (c < d.worstV) { d.worstV = c; d.worst = p }
  }
  const dirs: DirRollup[] = [...dirMap.entries()]
    .filter(([, v]) => v.files >= 5)
...
export function formatReadabilitySummary(report: ReadabilityReport, topN = 10): string {
  const out: string[] = []
  const r = report.repo
  out.push(
    `readability: ${r.files} files analysed · ${r.skippedNonCode} non-code skipped` +
  ...
  for (const [metric, rows] of Object.entries(report.outliers)) {
    ...
    const [label, show] = labels[metric] ?? [metric, (v: number) => fmt(v)]
    out.push('')
    out.push(`  ${label}:`)
    for (const row of rows) {
  ...
  if (report.longestLines.length) {
    ...
    for (const l of report.longestLines) out.push(`    ${l.path}:${l.line}  ${l.len} chars`)
  }
...
function markdownPathText(value: string): string {
 ...
 * artifacts.ts).
 */
export function writeReadabilityArtifacts(root: string, report: ReadabilityReport): number {
  if (!fs.existsSync(atlasDir(root))) return 0
  const base = safeAtlasDirectory(root, ['artifacts'], true)
  const wanted = new Set<string>()
  let written = 0
  const emit = (pageKey: string, body: string) => {
    ...
    if (existing && existing.buffer.toString('utf8') === body) return
    atomicWrite(file, body)
    written++
  }

  // per-file cards: every file appearing in any outlier list
  const flagged = new Map<string, { metric: string; value: number; z: number }[]>()
  for (const [metric, rows] of Object.entries(report.outliers)) {
    for (const r of rows) {
      if (!flagged.has(r.path)) flagged.set(r.path, [])
  ...
  const byDir = new Map<string, { path: string; metric: string; value: number; z: number }[]>()
  for (const [p, dims] of flagged) {
    const parts = p.split('/')
    const key = parts.length >= 3 ? parts.slice(0, 2).join('/') : parts.length === 2 ? parts[0] : '(root)'
    if (!byDir.has(key)) byDir.set(key, [])
    for (const d of dims) byDir.get(key)!.push({ path: p, ...d })
  }
  for (const d of report.dirs) {
    if (d.path === '(root)') continue
    const rows = byDir.get(d.path)
    if (!rows && d.lowFiles === 0) continue
    const table = (rows ?? [])
