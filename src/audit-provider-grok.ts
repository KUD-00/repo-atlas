...
const TOKEN_CAP_REFUSAL =
  /^(?:File content \(\d+ tokens\) exceeds maximum allowed tokens \(\d+ tokens\)\.|The requested line range \(offset=\d+, limit=\d+\) contains \d+ tokens, which exceeds the maximum allowed tokens \(\d+ tokens\)\.)/u
...
function copyGrokIdentity(context: AuditProviderContext, home: string): void {
...
function copyAgentIdentity(authRecordPath: string, home: string): void {
...
function proveTranscriptReadInterval(
  ...
  if (TOKEN_CAP_REFUSAL.test(content)) {
    ...
    if (call.limit <= 1) {
      throw new AuditProviderError(
        'preflight-rejected',
        `provider cannot read ${call.path}: one line alone exceeds read_file's ` +
 ...
 */
function renderReviewCorrection(unit: AuditProviderReviewUnit): string {
...
> {
  ...
  const correction =
    options.kind === 'review' && options.unit.attempt > 1
