...
import {
  ...
  type AnchoredAuditGitCapability,
} from './audit-core.js'
import { parseAuditDecisionPolicy } from './audit-decisions.js'
import type { AuditDecisionPolicyInputV1 } from './audit-v3-types.js'
import type {
  ...
  CoverageDiagnostic,
  HistoricalAuditUnitAssignmentV1,
  PortfolioDomain,
} from './types.js'
...
export const AUDIT_MATCH_OPERATION_LIMIT = 5_000_000n
...
function hasOwn(value: DataRecord, key: string): boolean {
  return Object.prototype.hasOwnProperty.call(value, key)
}

function dataRecord(value: unknown, pointer: string): DataRecord {
  if (
    value === null ||
    typeof value !== 'object' ||
    Array.isArray(value) ||
...
function dataArray(value: unknown, pointer: string): unknown[] {
  ...
  if (value.length > MAX_POLICY_ROWS) {
    fail(
      'policy-limit',
      `${pointer} exceeds the ${MAX_POLICY_ROWS}-item policy limit`,
    )
  }
  return value
}
...
function text(
  ...
  if (
    ...
    fail(
      'invalid-policy-text',
      `${pointer} must be nonempty bounded text without NUL`,
    )
  }
  if (options.identifier && !IDENTIFIER_RE.test(value)) {
    fail(
      'invalid-policy-identifier',
      `${pointer} must be a canonical lowercase identifier`,
    )
  }
  if (options.routeSlug && !ROUTE_SLUG_RE.test(value)) {
    fail(
      'invalid-policy-slug',
      `${pointer} must be a route-safe lowercase kebab-case slug`,
    )
  }
  return value
}
...
function uniqueSortedStrings(
  value: unknown,
  pointer: string,
  parse: (candidate: unknown, itemPointer: string) => string,
  nonempty: boolean,
): string[] {
...
function compileGlob(glob: string, pointer: string): (candidate: string) => boolean {
  try {
    return picomatch(glob, { dot: true })
  } catch (error) {
    fail(
      'invalid-policy-glob',
      `${pointer} is not a valid glob: ${
...
function glob(value: unknown, pointer: string): string {
  if (
    typeof value !== 'string' ||
    value.length === 0 ||
    ...
    fail(
      'invalid-policy-glob',
      `${pointer} must be a bounded nonempty POSIX glob`,
    )
  }
...
function globs(value: unknown, pointer: string, nonempty: boolean): string[] {
  return uniqueSortedStrings(value, pointer, glob, nonempty)
}

function isUniversalGlob(pattern: string): boolean {
...
function parseDomains(value: unknown, pointer: string): PortfolioDomain[] {
  const selected = uniqueSortedStrings(
    value,
    pointer,
    (candidate, itemPointer) => {
      if (candidate !== 'security' && candidate !== 'test') {
        fail(
          'invalid-policy-domain',
          `${itemPointer} must be security or test`,
        )
      }
      return candidate
    },
...
function parseRule(value: unknown, index: number): AuditReviewRuleV1 {
  const pointer = `/rules/${index}`
  const rule = dataRecord(value, pointer)
  exactKeys(
    rule,
    ['id', 'include', 'rationale'],
    ['except', 'domains', 'excluded'],
    pointer,
  )
  ...
  if (hasDomains === hasExcluded) {
    fail(
      ...
      `${pointer} must contain exactly one of domains or excluded`,
    )
  }
  const common = {
  ...
  for (const pattern of common.include) {
    if (isUniversalGlob(pattern)) {
      fail(
        'universal-exclusion',
        `excluded rule ${common.id} contains universal include glob ${pattern}`,
      )
    }
  }
  return {
...
function parseUnit(value: unknown, index: number): AuditReviewUnitV1 {
  ...
  if (unit.domain !== 'security' && unit.domain !== 'test') {
    fail(
      'invalid-policy-domain',
      `${pointer}/domain must be security or test`,
    )
  }
  return {
    domain: unit.domain,
...
function parseHistoricalAssignment(
  value: unknown,
  index: number,
): HistoricalAuditUnitAssignmentV1 {
  ...
  if (assignment.sourceKind !== 'relayos-security-scan/v1') {
    fail(
      ...
      `${pointer}/sourceKind must be relayos-security-scan/v1`,
    )
  }
  if (assignment.domain !== 'security') {
    fail(
      'invalid-historical-domain',
      `${pointer}/domain must be security`,
    )
  }
  return {
...
function withoutPolicyDigest(
  policy: ReturnType<typeof parseAuditDecisionPolicy>,
...
function parsePolicy(value: unknown): {
  ...
  if (
    ...
    fail(
      'invalid-policy-envelope',
      'review policy must be atlas-review-policy-v1 formatVersion 1',
    )
  }
  ...
  for (const rule of rules) {
    ...
    if (reservedId || reservedCategory) {
      if (
        ...
        fail(
          'invalid-generated-proof-rule',
          'generated-proof is reserved for the exact ' +
  ...
  const historicalUnitAssignments = hasOwn(
    root,
    'historicalUnitAssignments',
  )
    ? dataArray(
      root.historicalUnitAssignments,
      '/historicalUnitAssignments',
    )
  ...
  for (const assignment of historicalUnitAssignments) {
    if (historicalIds.has(assignment.id)) {
      fail(
        'duplicate-policy-id',
        `duplicate historical assignment ID ${assignment.id}`,
      )
    }
    historicalIds.add(assignment.id)
    if (!unitIds.has(`${assignment.domain}/${assignment.unit}`)) {
      fail(
        'invalid-historical-unit',
        `historical assignment ${assignment.id} names missing current unit ` +
  ...
  if (globCount > MAX_GLOBS) {
    fail(
      'policy-limit',
      `review policy exceeds the ${MAX_GLOBS}-glob limit`,
    )
  }

  let decisionInput: AuditDecisionPolicyInputV1
  try {
    const provisional = parseAuditDecisionPolicy(
      root.securityDecisions,
      ZERO_POLICY_DIGEST,
    )
    if (provisional.acceptedRulesets.includes('codex-contract')) {
      fail(
        'invalid-policy-ruleset',
        'acceptedRulesets must contain producer.ruleset IDs; ' +
    ...
    fail(
      'invalid-security-decision-policy',
      `securityDecisions is invalid: ${
        error instanceof Error ? error.message : String(error)
      }`,
    )
  }
  ...
  const hashInput = {
    ...
    format: 'atlas-review-policy-v1' as const,
    rules,
    units,
    historicalUnitAssignments,
  ...
  const policyHash = createHash('sha256')
    ...
    .digest('hex')
  const securityDecisions = parseAuditDecisionPolicy(
    decisionInput,
    `sha256:${policyHash}`,
  )
  return {
...
export interface AuditReviewPolicyLoad {
  policy: AuditReviewPolicyV1 | null
...
function validateTrackedPath(repoPath: string): string {
...
function readAuditTrackedInventoryWithCapability(
  capability: AnchoredAuditGitCapability,
): AuditTrackedInventory {
  const diagnostics: CoverageDiagnostic[] = []
  let objectFormat: 'sha1' | 'sha256'
  let stageBytes: Uint8Array
  try {
    const rawFormat = UTF8.decode(
      capability.gitBytes(
    ...
    objectFormat = rawFormat as 'sha1' | 'sha256'
    stageBytes = capability.gitBytes(
      ['ls-files', '--stage', '-z', '--'],
      MAX_GIT_OUTPUT_BYTES,
    )
  } catch (error) {
    return {
      ...
      diagnostics: [diagnostic(
        ...
        `unable to read tracked Git inventory: ${
          error instanceof Error ? error.message : String(error)
        }`,
      )],
    }
  ...
  try {
    ...
    return {
      ...
      files: [],
      diagnostics: [diagnostic(
        'invalid-git-path-encoding',
        'tracked Git inventory is not strict UTF-8',
      )],
    }
  }
  ...
  if (records.length > MAX_TRACKED_FILES) {
    return {
      ...
      diagnostics: [diagnostic(
        'inventory-limit',
        `tracked inventory exceeds the ${MAX_TRACKED_FILES}-file limit`,
      )],
    }
  }

  const expectedHashLength = objectFormat === 'sha1' ? 40 : 64
  const files: AuditTrackedFile[] = []
  ...
  for (const record of records) {
    ...
    if (!match) {
      diagnostics.push(diagnostic(
        'invalid-git-index-record',
        'Git index returned a malformed stage record',
      ))
      continue
    }
    ...
    try {
      ...
      diagnostics.push(diagnostic(
        ...
        `${JSON.stringify(rawPath)} is not a safe tracked path: ${
          error instanceof Error ? error.message : String(error)
        }`,
      ))
      continue
    ...
    if (rawPaths.has(repoPath)) {
      diagnostics.push(diagnostic(
        ...
        `tracked path has duplicate or unresolved index entries: ${JSON.stringify(repoPath)}`,
        repoPath,
      ))
      continue
    }
    ...
    if (collision !== undefined && collision !== repoPath) {
      diagnostics.push(diagnostic(
        'tracked-path-collision',
        `tracked paths collide by case or NFC normalization: ` +
          `${JSON.stringify(collision)} and ${JSON.stringify(repoPath)}`,
        repoPath,
      ))
    } else {
    ...
    if (stage !== '0') {
      diagnostics.push(diagnostic(
        ...
        `tracked path has unresolved index stage ${stage}: ${JSON.stringify(repoPath)}`,
        repoPath,
      ))
      continue
    }
    if (
      indexBlob.length !== expectedHashLength ||
      !/^[0-9a-f]+$/.test(indexBlob)
    ) {
      diagnostics.push(diagnostic(
        'invalid-index-object',
        `tracked path has an invalid ${objectFormat} blob ID: ${JSON.stringify(repoPath)}`,
        repoPath,
      ))
      continue
    }
    if (!REGULAR_GIT_MODES.has(indexMode)) {
      diagnostics.push(diagnostic(
        'unsafe-index-mode',
        `tracked path has unsupported non-regular Git mode ${indexMode}: ` +
    ...
    try {
      const currentFile = capability.hashWorktreeFile(
        repoPath,
        objectFormat,
      ...
      if (
        ...
        diagnostics.push(diagnostic(
          ...
          `tracked path executable mode drifted from ${indexMode} to ` +
            `${current.mode}: ${JSON.stringify(repoPath)}`,
          repoPath,
        ))
      }
      files.push({
      ...
      if (current.deleted) {
        diagnostics.push(diagnostic(
          'tracked-deletion',
          `tracked path is deleted from the worktree: ${JSON.stringify(repoPath)}`,
          repoPath,
        ))
      }
    } catch (error) {
      diagnostics.push(diagnostic(
        'unsafe-worktree-file',
        `unable to hash tracked path ${JSON.stringify(repoPath)} safely: ${
          error instanceof Error ? error.message : String(error)
        }`,
        repoPath,
      ))
...
export function readAuditTrackedInventory(root: string): AuditTrackedInventory {
  try {
    return withAnchoredAuditGitCapability(
      root,
      readAuditTrackedInventoryWithCapability,
    )
  } catch (error) {
    ...
    return {
      ...
      files: [],
      diagnostics: [diagnostic(
        /repository root.*(?:must|existing|safe)/i.test(message)
          ? 'invalid-repository-root'
          : 'git-inventory-failed',
        `unable to read one anchored tracked Git inventory: ${message}`,
      )],
    }
  }
}
...
interface CompiledRule {
  rule: AuditReviewRuleV1
  matches: (repoPath: string) => boolean
}

interface CompiledUnit {
  unit: AuditReviewUnitV1
  owns: (repoPath: string) => boolean
}
...
function isExclusionRule(
  rule: AuditReviewRuleV1,
): rule is AuditReviewExclusionRuleV1 {
...
function hazardousExclusionPath(file: AuditTrackedFile): boolean {
  if (file.indexMode === '100755' || file.currentMode === '100755') return true
  const lower = file.path.toLowerCase()
  const base = path.posix.basename(lower)
  if (GENERATED_LOCKFILES.has(base)) return false
  if (base === '.env' || base.startsWith('.env.')) return true
  if (lower.startsWith('.agents-src/')) return true
  if (lower.startsWith('.github/workflows/')) return true
  const extension = path.posix.extname(lower)
  if (INERT_TEXT_EXTENSIONS.has(extension)) return false
  if (OPERATIONAL_FILENAMES.has(base)) return true
  if (/^(?:dockerfile|containerfile)(?:\..+)?$/u.test(base)) return true
  if (
    /^(?:tsconfig|eslint|prettier|vitest|vite|playwright|babel|jest)(?:\.[^.]+)*\.json$/u.test(base)
  ) {
    return true
  }
...
function exclusionIsExactAndOwned(
  rule: AuditReviewExclusionRuleV1,
  repoPath: string,
): boolean {
  return (
    typeof rule.excluded.owner === 'string' &&
    rule.excluded.owner.length > 0 &&
    rule.except.length === 0 &&
    rule.include.some((pattern) =>
      !hasGlobMagic(pattern) && pattern === repoPath)
  )
}
...
function snapshotPlainData(
  ...
  if (budget.nodes > nodeLimit) {
    throw new Error(
      `${pointer} exceeds the aggregate ${nodeLimit}-node limit`,
    )
  }
  const chargeText = (text: string, kind: string): void => {
    if (text.length > MAX_TEXT_CODE_UNITS) {
      throw new Error(
        `${pointer} ${kind} exceeds the ${MAX_TEXT_CODE_UNITS}-code-unit limit`,
      )
    }
    ...
    if (budget.text > textLimit) {
      throw new Error(
        `${pointer} exceeds the aggregate string limit of ` +
  ...
  if (Array.isArray(value)) {
    ...
    if (
      ...
      throw new Error(
        `${pointer} has an invalid array length or exceeds the ` +
    ...
    for (const key of ownKeys as string[]) {
      ...
      if (
        ...
        throw new Error(
          `${pointer} plain array has unexpected own property ${JSON.stringify(key)}`,
        )
      }
    }
    ...
    for (let index = 0; index < arrayLength; index += 1) {
      ...
      if (
        ...
        throw new Error(
          `${pointer}/${index} must be an enumerable data property`,
        )
      }
      output.push(snapshotPlainData(
        descriptor.value,
        `${pointer}/${index}`,
        depth + 1,
        arrayLimits,
  ...
  const output: Record<string, unknown> = Object.create(null) as
  ...
  for (const key of Reflect.ownKeys(value)) {
    ...
    if (!descriptor?.enumerable) {
      throw new Error(
        `${pointer}/${key} must be an enumerable own property`,
      )
    }
...
function snapshotPublicPolicy(
  value: AuditReviewPolicyV1,
): { policy: AuditReviewPolicyV1; policyHash: string } {
  const snapshot = dataRecord(
    snapshotPlainData(value, '/policy'),
    '/policy',
  )
  const decisions = dataRecord(
    snapshot.securityDecisions,
...
export function normalizeAuditReviewPolicy(
  value: AuditReviewPolicyV1,
): { policy: AuditReviewPolicyV1; policyHash: string } {
  return snapshotPublicPolicy(value)
}

export interface AuditHistoricalExpansionInput {
  policy: AuditReviewPolicyV1
  sourceKind: 'relayos-security-scan/v1'
  historicalPaths: string[]
  currentPaths: string[]
  activeReceiptPaths: string[]
}

export interface AuditHistoricalExpandedAssignment {
  id: string
  sourceKind: 'relayos-security-scan/v1'
  domain: 'security'
  unit: string
...
export interface AuditHistoricalExpansionResult {
  assignments: AuditHistoricalExpandedAssignment[]
  unmapped: string[]
  diagnostics: CoverageDiagnostic[]
  expansionDigest: string
}

function historicalExpansionDigest(
  sourceKind: 'relayos-security-scan/v1',
  historicalUnitAssignments:
    readonly HistoricalAuditUnitAssignmentV1[],
  assignments: readonly AuditHistoricalExpandedAssignment[],
...
function emptyHistoricalExpansion(
  message: string,
): AuditHistoricalExpansionResult {
  const sourceKind = 'relayos-security-scan/v1' as const
  return {
    assignments: [],
    unmapped: [],
    diagnostics: [diagnostic(
      'invalid-historical-expansion-input',
      message,
    )],
...
function historicalResourceLimitExpansion(
  sourceKind: 'relayos-security-scan/v1',
  historicalUnitAssignments:
    readonly HistoricalAuditUnitAssignmentV1[],
  historicalPaths: string[],
  ...
  return {
    ...
    diagnostics: [diagnostic(
      'historical-resource-limit',
      `historical expansion requires ${operations.toString()} ` +
        `worst-case match operations; limit is ` +
        `${AUDIT_MATCH_OPERATION_LIMIT.toString()}`,
    )],
    expansionDigest: historicalExpansionDigest(
      sourceKind,
      historicalUnitAssignments,
      [],
      historicalPaths,
...
function historicalExpansionPaths(
  value: unknown,
  pointer: string,
): string[] {
  const parsed = dataArray(value, pointer).map((candidate, index) => {
    if (typeof candidate !== 'string') {
      fail(
        'invalid-historical-path',
        `${pointer}/${index} must be a repository path`,
      )
    }
    try {
  ...
  for (let index = 1; index < parsed.length; index += 1) {
    if (parsed[index] === parsed[index - 1]) {
      fail(
        ...
        `${pointer} contains duplicate path ${JSON.stringify(parsed[index])}`,
      )
    }
  }
  return parsed
 ...
 */
export function expandAuditHistoricalUnitAssignments(
  unsafeInput: AuditHistoricalExpansionInput,
): AuditHistoricalExpansionResult {
  let snapshot: unknown
  try {
    snapshot = snapshotPlainData(unsafeInput, '/historicalExpansion')
  } catch (error) {
    return emptyHistoricalExpansion(
      error instanceof Error ? error.message : String(error),
    )
  }
  try {
    ...
    if (input.sourceKind !== 'relayos-security-scan/v1') {
      fail(
        'invalid-historical-source',
        'historical expansion sourceKind must be relayos-security-scan/v1',
      )
    }
    const sourceKind = input.sourceKind
    const policy = snapshotPublicPolicy(
      input.policy as AuditReviewPolicyV1,
    ).policy
    const historicalPaths = historicalExpansionPaths(
      input.historicalPaths,
      '/historicalExpansion/historicalPaths',
    )
    const currentPaths = historicalExpansionPaths(
      input.currentPaths,
      '/historicalExpansion/currentPaths',
    )
    const activeReceiptPaths = historicalExpansionPaths(
      input.activeReceiptPaths,
      '/historicalExpansion/activeReceiptPaths',
    )
    const pathCount =
    ...
    const matchOperations =
      BigInt(pathCount) * BigInt(historicalGlobCount)
    if (matchOperations > AUDIT_MATCH_OPERATION_LIMIT) {
      return historicalResourceLimitExpansion(
        sourceKind,
    ...
    const unitIds = new Set(policy.units.map((unit) =>
      `${unit.domain}:${unit.slug}`))
    for (const candidate of compiled) {
      const unitId =
        `${candidate.assignment.domain}:${candidate.assignment.unit}`
      if (!unitIds.has(unitId)) {
        diagnostics.push(diagnostic(
          'historical-unit-domain-mismatch',
          `historical assignment ${candidate.assignment.id} names missing ` +
            `same-domain unit ${unitId}`,
          undefined,
        ))
      }
      for (const currentPath of currentPaths) {
        if (candidate.matches(currentPath)) {
          diagnostics.push(diagnostic(
            ...
            `historical assignment ${candidate.assignment.id} matches ` +
      ...
      for (const receiptPath of activeReceiptPaths) {
        if (candidate.matches(receiptPath)) {
          diagnostics.push(diagnostic(
            ...
            `historical assignment ${candidate.assignment.id} matches ` +
              `active receipt ${JSON.stringify(receiptPath)}`,
            receiptPath,
          ))
        }
      }
    ...
    for (const candidate of compiled) {
      if (candidate.paths.length === 0) {
        diagnostics.push(diagnostic(
          'historical-assignment-empty',
          `historical assignment ${candidate.assignment.id} matches no ` +
            'historical path',
        ))
      }
    }
    const assignments: AuditHistoricalExpandedAssignment[] = compiled
...
interface TrackedInventorySnapshot {
  files: AuditTrackedFile[]
  forcedConflicts: Set<string>
  diagnostics: CoverageDiagnostic[]
}

function snapshotTrackedInventory(
  ...
  if (!Array.isArray(snapshot)) {
    return {
      ...
      diagnostics: [diagnostic(
        'invalid-inventory-input',
        'inventory must be a plain array',
      )],
    }
  }
  ...
  for (let index = 0; index < snapshot.length; index += 1) {
    ...
    if (
      ...
      diagnostics.push(diagnostic(
        ...
        `inventory row ${index} must be a plain object`,
      ))
      continue
    }
    const row = candidate as Record<string, unknown>
    if (
      Object.keys(row).length !== exactFields.size ||
      Object.keys(row).some((key) => !exactFields.has(key))
    ) {
      diagnostics.push(diagnostic(
        'invalid-inventory-row',
        `inventory row ${index} must have exact tracked-file fields`,
      ))
      continue
    }
    let repoPath: string
    try {
      if (typeof row.path !== 'string') throw new Error('path must be text')
      repoPath = validateTrackedPath(row.path)
    } catch (error) {
      diagnostics.push(diagnostic(
        'invalid-inventory-row',
        `inventory row ${index} has an invalid path: ${
          error instanceof Error ? error.message : String(error)
        }`,
      ))
      continue
    ...
    const blobLength =
      typeof row.indexBlob === 'string' ? row.indexBlob.length : 0
    ...
    if (
      ...
      diagnostics.push(diagnostic(
        'invalid-inventory-row',
        `inventory row has invalid blob, mode, or deletion fields: ${JSON.stringify(repoPath)}`,
        repoPath,
      ))
      continue
    }
    const file: AuditTrackedFile = {
      path: repoPath,
      indexBlob: row.indexBlob,
      currentBlob: row.currentBlob as string | null,
      indexMode: row.indexMode as string,
      currentMode: row.currentMode as '100644' | '100755' | null,
      deleted: row.deleted,
    }
    files.push(file)
    ...
    if (collision !== undefined && collision !== repoPath) {
      ...
      diagnostics.push(diagnostic(
        ...
        `inventory paths collide by case or NFC: ` +
          `${JSON.stringify(collision)} and ${JSON.stringify(repoPath)}`,
        repoPath,
      ))
    } else {
    ...
    if (!coherent) {
      forcedConflicts.add(repoPath)
      diagnostics.push(diagnostic(
        'incoherent-inventory-deletion',
        `inventory deletion/currentBlob/currentMode fields are incoherent for ${JSON.stringify(repoPath)}`,
        repoPath,
      ))
    }
    if (file.deleted) {
    ...
    if (
      ...
      forcedConflicts.add(repoPath)
      diagnostics.push(diagnostic(
        'inventory-mode-drift',
        `inventory executable mode drifted for ${JSON.stringify(repoPath)}`,
        repoPath,
      ))
    }
  }
  for (const [repoPath, indexes] of seen) {
    ...
    diagnostics.push(diagnostic(
      ...
      `inventory contains duplicate path ${JSON.stringify(repoPath)}`,
      repoPath,
    ))
  }
...
export function classifyAuditInventory(
  inventory: readonly AuditTrackedFile[],
  policyInput: AuditReviewPolicyV1 | null,
): AuditInventoryClassification {
  ...
  if (policyInput === null) {
    return {
      ...
      diagnostics: [...diagnostics, diagnostic(
        ...
        'tracked inventory cannot be classified without a valid review policy',
      )],
    }
  }
  let policy: AuditReviewPolicyV1
  try {
    ...
    return {
      ...
      diagnostics: [...diagnostics, diagnostic(
        ...
        `review policy input is invalid: ${
          error instanceof Error ? error.message : String(error)
        }`,
      )],
    }
  ...
  const ruleGlobCount = policy.rules.reduce(
    (count, rule) => count + rule.include.length + rule.except.length,
    0,
  )
  const exclusionGlobCount = policy.rules.reduce(
    (count, rule) => count + (
      isExclusionRule(rule)
        ? rule.include.length + rule.except.length
        : 0
    ),
    0,
  )
  const unitGlobCount = policy.units.reduce(
    (count, unit) => count + unit.include.length + unit.except.length,
    0,
  )
  const historicalGlobCount = policy.historicalUnitAssignments.reduce(
    (count, assignment) => count + assignment.include.length,
  ...
  const matchOperations = BigInt(snapshot.files.length) * BigInt(
    ruleGlobCount +
    exclusionGlobCount +
    unitGlobCount +
    historicalGlobCount,
  ...
  if (matchOperations > AUDIT_MATCH_OPERATION_LIMIT) {
    return {
      ...
      diagnostics: [...diagnostics, diagnostic(
        ...
        `classification requires ${matchOperations.toString()} ` +
          `worst-case match operations; limit is ` +
          `${AUDIT_MATCH_OPERATION_LIMIT.toString()}`,
      )],
    }
  ...
  const compiledRules: CompiledRule[] = policy.rules.map((rule) => ({
    rule,
    matches: matcher(rule.include, rule.except),
  }))
  const compiledUnits: CompiledUnit[] = policy.units.map((unit) => ({
    unit,
    owns: matcher(unit.include, unit.except),
  }))
  const historicalMatchers = policy.historicalUnitAssignments.map(
    (assignment) => ({
      assignment,
      matches: matcher(assignment.include, []),
    }),
  )
  const universalSwallow = new Set<string>()
  if (snapshot.files.length > 0) {
    for (const compiled of compiledRules) {
      if (
        isExclusionRule(compiled.rule) &&
        ...
        diagnostics.push(diagnostic(
          'universal-exclusion',
          `excluded rule ${compiled.rule.id} swallows the entire tracked inventory`,
        ))
      }
    }
  }
  ...
  const files = snapshot.files.map((file) => {
    ...
    const selectedDomains = new Set<PortfolioDomain>()
    for (const rule of matchedRules) {
      if (!isExclusionRule(rule)) {
        for (const domain of rule.domains) selectedDomains.add(domain)
      }
    }

    let classification: CoverageClassification
    const historical = historicalMatchers.filter((candidate) =>
      candidate.matches(file.path))
    if (historical.length > 0) {
      classification = { kind: 'conflict' }
      diagnostics.push(diagnostic(
        'historical-current-overlap',
        `historical assignment(s) ${historical.map((item) =>
          item.assignment.id).join(', ')} match current tracked path ` +
      ...
      diagnostics.push(diagnostic(
        ...
        `tracked path is unclassified: ${JSON.stringify(file.path)}`,
        file.path,
      ))
    } else if (
      exclusions.length > 1 ||
      ...
      diagnostics.push(diagnostic(
        ...
        `tracked path ${JSON.stringify(file.path)} matches conflicting domain ` +
          `and/or exclusion rules: ${ruleIds.join(', ')}`,
        file.path,
      ))
    } else if (exclusions.length === 1) {
      const exclusion = exclusions[0]
      if (
        universalSwallow.has(exclusion.id) ||
        (
        ...
        classification = { kind: 'conflict' }
        diagnostics.push(diagnostic(
          'unsafe-broad-exclusion',
          `executable or configuration path ${JSON.stringify(file.path)} ` +
            `requires an exact exclusion with an owner`,
          file.path,
        ))
      } else {
        classification = {
      ...
      const domains: Partial<
        Record<PortfolioDomain, { unit: string }>
      > = {}
      let conflict = false
      for (const domain of DOMAINS) {
        ...
        const owners = compiledUnits
          .filter((compiled) =>
            compiled.unit.domain === domain &&
            compiled.owns(file.path))
          .map((compiled) => compiled.unit)
        if (owners.length !== 1) {
          conflict = true
          diagnostics.push(diagnostic(
            'unit-ownership-conflict',
            `tracked path ${JSON.stringify(file.path)} requires ${domain} ` +
  ...
  for (const unit of policy.units) {
    ...
    if (!ownedUnits.has(unitId)) {
      diagnostics.push(diagnostic(
        'empty-review-unit',
        `review unit ${unitId} owns no currently classified tracked paths`,
        undefined,
      ))
    }
  }
