...
import type { ImportGraph, ScanResult } from './types.js'
...
function extractSpecifiers(absFile: string, hash: string): string[] {
...
function workspacePackages(root: string, scanResult: ScanResult): Map<string, string> {
  ...
  for (const p of [...scanResult.files.keys(), ...scanResult.ignored]) {
    ...
    try {
      const pkg = JSON.parse(fs.readFileSync(path.join(root, p), 'utf8')) as { name?: string }
...
function resolveRelative(fromFile: string, spec: string, files: Map<string, string>): string | null {
...
export function buildImportGraph(root: string, scanResult: ScanResult): ImportGraph {
