...
# 0. bootstrap
repo-atlas status --json | jq -r '.missing[]' > /tmp/missing.txt
...
bun $QA/sweep.ts --file /tmp/missing.txt --concurrency 12

# 3. 人审概念主页（必做，见下"门的边界"）

# 4. 代码演进后：只重扫过时的
repo-atlas status --json | jq -r '.outdated[].path' > /tmp/outdated.txt
bun $QA/sweep.ts --file /tmp/outdated.txt --fresh
```
